                    ...
                    success: {
                        ...
                        callback: function () {
							...
							localStorage.removeItem('offer_total_discount');
                                ...
                                suspend_cancel();
                                // return false;
                                window.location.href = site.url+"pos/qsr";
    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
           ...
           tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price)) * parseFloat(item_qty)-parseFloat(manual_item_ds))) + '</span></td>';
            ...
            tr_html += '<td class="text-center"><div class="text-center"><span class="payment_status  label label-danger posdel" id="' + row_no + '" title="Remove" style="cursor:pointer;"><i class="fa fa-remove"></i></span></div></td>';
            ...
            if (pos_settings.remote_printing != 1) {
               ...
               var recipe_variant=(item.row.variant)?'['+item.row.variant+']':'';

                var  bprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td class="no-border">'+(an-1)+"."+ item_name +recipe_variant+'</td>';
              /*  for (var i = 0, len = comments.length; i < len; i++) {
       ...
       var bill_table_head = '<tr><th class="text-right">Description</th><th class="text-right" ><span>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</span>Price</th><th >Qty</th><th class="text-right">Sub Total</th>';
