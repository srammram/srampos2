        ...
        if (protect_delete == 1) {
                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            if(pos_pin == pos_settings.pin_code) {
                                ...
                                $('#modal-loading').show();
                                window.location.href = site.url+"pos/pos/home";
            ...
            bootbox.confirm(lang.r_u_sure, function (result) {
                if (result) {
                    ...
                    $('#modal-loading').show();
                    window.location.href = site.url+"pos/pos";
                }
            });
        }
    });
        ...
        $.each(addon_check, function(k) {
            ...
            $.each(item.addons, function () { 
               ...
               $.each(addonidqty, function (key, val) {
                ...
                $style=($addonenteredqty !='')?'style="background-color:#200202"':"";
				console.log($style);
                AddonTr = $('<tr id="row_' + row_no + '" class="item_' + this.id + ' addon_roow" data-item-id="' + row_no + '" '+$style+'>');
                $checked =''; 
             ...
             //   addon_tr_html = '<td></td>';
                ...
                addon_tr_html += '<td><input type="checkbox" name="addon_id"  style="opacity:0;" class=" recipeaddon"  data="'+this.id+'" '+$checked+' >'+$addname+ '';    
                addon_tr_html += '<div><input type="hidden" class="recipe_addon_id recipe_addon_id'+this.recipe_id +this.variant_id+' kb-pad"  name="recipe_addon_id[]" value="'+this.recipe_id +this.variant_id+'">';

                addon_tr_html += '<input type="hidden" class="addon_native_name addon_native_name'+this.recipe_id +this.variant_id+' kb-pad"  name="addon_native_name[]" value="'+$addname+'"><div></td>';

                 addon_tr_html += '<td><input type="text" maxlength="2" class="addon_quantity numberonly add_kb-pad addon_quantity_'+this.recipe_id +this.variant_id+'"  name="addon_quantity[]" value="'+$addonenteredqty+'"  '+$readonly+' ><span style="color: red;"></span>';
                 ...
                 addon_tr_html += '<td>'+formatMoney(this.price)+'<input type="hidden" class="addon_price kb-pad" name="addon_price[]" style="border-color: #4c0a0a;" value="'+formatDecimal(this.price)+'" ></td></tr>';
           ...
           $(document).on('change', '.recipeaddon', function () {        
            ...
            if ($(this).prop('checked')==true){ 
			 $(this).closest('tr').css('background-color', '#200202');
                $(this).parent().parent().find('.addon_quantity').attr("readonly",false);
				$(this).parent().parent().find('.addon_quantity').trigger("click");
				$(this).parent().parent().find('.addon_quantity').val('');
            }else{
				   $(this).parent().parent().find('.addon_quantity').unbind('focusout');
				$(this).closest('tr').css('background-color', '#4c0a0a');
                $(this).parent().parent().find('.addon_quantity').attr("readonly",true); 
                $(this).parent().parent().find('.addon_quantity').unbind("click");
                $(this).parent().parent().find('.addon_quantity').val('');
                
            }
    });    
	 $(document).on('change', '.addon_quantity', function () {  
	 if( $(this).closest('tr').find('.recipeaddon').is(':checked')){
		 return true;
       }else{
       $(this).parent().parent().find('.addon_quantity').val('');
	   return false;
...
/*new fnction for add-on end*/
	  ...
	  $(document).on('click', '.addon_roow', function () {
		$(this).closest('tr').find('.recipeaddon').trigger('click', {
                        nonUI : true
        })
	...
	$(document).on('click', '.iclose_customized', function() {
...
/*     $(document).on('click', '.customize', function() {
        ...
        $('#cuModalLabelname').text('customize ='+item.row.code + ' - ' + item.row.name);               
        var CustomizeTr='';
        var recipe_variant_addon = 0;   
       $(".customized_ingredients").empty();  
		$('#categorieslist').hide();
        ...
        $.each(item.customizable_ingrediends, function () { 
            ...
            customize_tr_html ='<div class="modal-body" id="pr_popover_content"><h4 id="cuModalLabelname"></h4>  <form class="form-horizontal" role="form"><div class="form-group text-center"><div class="col-sm-12"><table class="table table-bordered table-striped n1"><thead><tr><th>Check</th><th>Name</th>  <th>Qty</th><th>UOM</th></tr></thead><tbody id="pocustomize-div">'+CustomizeTr;
			...
			customize_tr_html +='</tbody></table></div></div></form></div><input type="hidden" id="row_id1" value=""/><div class="modal-footer mod_footer"><span class="payment_status pull-left label label-danger iclose" style="padding:10px 12px;font-size:14px;margin-right:5px;border-radius:0px;font-weight:normal;cursor:pointer;">void</span><button type="button" class="btn btn-primary customized_saved" id="CustomizeItem">Submit</button></div>' ;			
			
            //CustomizeTr.html(customize_tr_html);
           // CustomizeTr.appendTo(".customized_ingredients");                        
        }); 
		$('.customized_ingredients').html(customize_tr_html);
        }else{
            recipe_variant_addon = 0;
        }
         */
		  $(document).on('click', '.customize', function() {
		 $('.customized_ingredients').css({"display":"block"});
		 $('.table_middle_s').hide();
		  $('.category_se').hide();
      	 $('.table_bottom_s').hide();
  ...
  /*   $(document).on('change', '.recipecustomize', function () {        
    ...
    $(document).on('click', '#CustomizeItem', function () {
       ...
       $('.sub_category_gr').show();
		...
		$('#pocustomize-div').empty();
		$('.customized_ingredients').css({"display":"none"});
	    $('#popup_id_s').hide();
	    $('#category-list').show();
	    $('.sub_category_gr').show();
	    $('#cpinner').show();
...
//console.log(localStorage.getItem('positems'));
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var get_item_variant_name=item.row.get_variant_name ? item.row.get_variant_name : '';   
			var get_item_variant_id=item.row.get_variant_id ? item.row.get_variant_id : '';   
			...
			if(manual_item_ds>unit_price){
				item.row.manual_item_ds_val=0;
				bootbox.alert('Please enter the valid discount');
				manual_item_ds=0;
				manual_item_ds_val=0;
			}
...
/*variant check*/
		 ...
		 //buy x get y part end///
            ...
            if (pos_settings.item_addon == 1 && item.addons != false) {
                tr_html += '<div class="col-xs-3"><div class="text-center"><span class="label label-primary fa-bx'+(item_comment != '' ? '' :'-o')+' tip pointer edit1 addon_css" id="' + row_no + '" data-item="' + item_id + '"  title="Edit" style="font-size:10px;'+$addondidbledstyle+'" >Addon</span></div></div>';
            }
            
            if(pos_settings.order_item_customization == 1 && item.customizable_ingrediends !=false){
                tr_html += '<div class="col-xs-3" style=""><div class="text-center"><span class="label label-primary fa-bx'+(item_comment != '' ? '' :'-o')+' tip pointer customize customize_css" id="' + row_no + '" data-item="' + item_id + '"  title="Customize" style="font-size:10px;" >Customized</span></div></div>';
            }

            if (pos_settings.item_comment ==1) {
                tr_html += '<div class="col-xs-3" style=""><div class="text-center"><span class="label label-warning fa-bx'+(item_comment != '' ? '' :'-o')+' tip pointer comment comment_css" id="' + row_no + '" data-item="' + item_id + '" title="Comment" style="cursor:pointer;margin-right:5px;">comment</span></div></div>';
            }
...
tr_html  +='<td class="text-center"><button type="button" class="btn btn_remove posdel"><i class="fa fa-remove"></i></button></td>';
            newTr.html(tr_html);
            if (pos_settings.item_order == 1) {
	...
	//			console.log(oprTr);
	  ...
	  //total order tax customer customization  .only for display purpose in screen //
	  $default_taxs = (total) * (default_tax.rate / 100);   
	  
	   if(pos_settings.tax_type != 0){
                           $final_val = (total + $default_taxs);
                           $sub_val = $final_val;
                        }
                        else{
                            $sub_val = total/(($default_taxs/total)+1);
                            $sub_val =  $sub_val;
                            $default_taxs = ($sub_val) * (default_tax.rate / 100);
                            $final_val = $sub_val+$default_taxs; 
                            $sub_val =  $final_val;
                        } 
	  ...
	  ///
		...
		$('#gtotal').text(formatMoney($sub_val)); 
		$('.totalPayable').text(formatMoney($sub_val,site.settings.symbol));
		console.log(gtotal);
       // $('#titems').text((an - 1) + ' (' + formatQty(parseFloat(count) - 1) + ')');
		$('#total_items').text((an - 1));
		$('#titems').text(formatQty(parseFloat(count) - 1) );
		$('#total_items').text((an - 1) + ' (' + formatQty(parseFloat(count) - 1) + ')');
        //  $('#total_items').val((parseFloat(count) - 1));
        $('#tds').text(manual_ds);
        if (site.settings.tax2 != 0) {
            $('#ttax2').text(formatMoney($default_taxs));
        }
        $('#tship').text(parseFloat(shipping) > 0 ? formatMoney(shipping) : '');
		  $('#total').text(formatMoney(gtotal-manual_ds));
        if (pos_settings.remote_printing != 1) {
...
$(document).on('keypress',".avoidsepcialcharacter",function (event){   
      if ((event.which != 46 || $(this).val().indexOf('.') != -1)&&(event.which != 37 || $(this).val().indexOf('%') != -1) && (event.which < 48 || event.which > 57)) {
                event.preventDefault();
   }
});