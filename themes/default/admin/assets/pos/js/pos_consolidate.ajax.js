...
//window.location.href = site.base_url+"pos/consolidate/?order=1&table=5";		
//return false;
		//alert('test');
		$("#sent_to_kitchen").prop("disabled", true);
		$('.v12').empty();
		//$('.v12').hide();
		$('.ad12').show();
		$('#popup_id_s').show();
	...
	$('#item-list').hide();
	 //console.log('.edit1');
        ...
        $('.ad1').text(item.row.code + ' - ' + item.row.name);
        ...
        var addonidqty = {};
		addon_tr_html ="<table class='table table-bordered table-striped'><thead><th colspan='1'></th><th>Recipe Addon</th><th>Quantity</th><th>Price</th> </th></thead><tfoot><td colspan='4' ><button type='button' class='btn btn-primary pull-right AddonItem' id='AddonItem'>Submit</button><span class='payment_status pull-left label label-danger iclose' style='padding:10px 12px;font-size:14px;margin-right:5px;border-radius:0px;font-weight:normal;cursor:pointer;' id='' + row_no + '' title='Remove' style='cursor:pointer;'>void</span></td></tfoot>";  
		
        $.each(addon_check, function(k) {
			
			
			
			//addon_tr_html ="<tr><td><div class='modal-header'><button type='button' class='close' data-dismiss='modal'><span aria-hidden='true'><iclass='fa fa-2x'>&times;</i></span><span class='sr-only'>Close</span></button><h4 class='modal-title' id='prModalLabel'></div></h4></td></tr>";
			...
			//alert(this.addon);
               ...
               $.each(addonidqty, function (key, val) {
                ...
                AddonTr = $('<tr id="row_' + row_no + '" class="item_' + this.id + '" data-item-id="' + row_no + ' a2">');
                $checked =''; 
                $readonly ='readonly'; 
                if (recipe_addon.indexOf(this.id) !== -1) {
                        ...
                        $readonly ='';
                            ...
                            addon_tr_html +='<tr class="a2" id="row_' + row_no + '"><td style="border-right-color:transparent"><input type="checkbox" style="opacity:0;" value="1" name="addon_id" class="recipeaddon "  data="'+this.id+'" '+$checked+' ></td><td   > '+ this.addon + '</td><td ><input type="text" class="form-control addon_quantity kb-pad a1"  name="addon_quantity[]" value="'+$addonenteredqty+'"  '+$readonly+' ></td><td>'+formatMoney(this.price)+'<input type="hidden" class="form-control addon_price kb-pad a1" name="addon_price[]" value="'+formatDecimal(this.price)+'" readonly></td> </tr>';        
              ...
              //  addon_tr_html += '<div><input type="hidden" class="form-control recipe_addon_id recipe_addon_id'+this.recipe_id +this.variant_id+' kb-pad"  name="recipe_addon_id[]" value="'+this.recipe_id +this.variant_id+'">';
               ...
               // addon_tr_html += '<input type="hidden" class="form-control addon_native_name addon_native_name'+this.recipe_id +this.variant_id+' kb-pad"  name="addon_native_name[]" value="'+$addname+'"><div></td>';
                ...
                // addon_tr_html += '<td><input type="text" maxlength="2" class="form-control addon_quantity numberonly add_kb-pad addon_quantity_'+this.recipe_id +this.variant_id+'"  name="addon_quantity[]" value="'+$addonenteredqty+'"  '+$readonly+' ><span style="color: red;"></span>';
                 ...
                 //addon_tr_html += '<td><input type="text" class="form-control addon_price kb-pad" name="addon_price[]" value="'+formatDecimal(this.price)+'" readonly></td>';
				 
           //  addon_tr_html +="</tbody></table></div></div><div class='form-group' style='display: none'><label for='addonamount' class='col-sm-4 control-label'><?=lang('addonamount')?></label><div class='col-sm-8'><input type='hidden' class='form-control kb-pad' name='addonamount[]' id='addonamount'> </div></div><input type='hidden' id='punit_price' value=''/> <input type='hidden' id='old_tax' value=''/><input type='hidden' id='old_qty' value=''/> <input type='hidden' id='old_price' value=''/><input type='hidden' id='row_id' value=''/></form></div> ";
              ...
              //  AddonTr.appendTo("#poaddon-div");
			     // AddonTr.appendTo(".ad12-div");
				 $("#category-list").css({"display":"none"});
				// $("#category-list").css("background-color", "yellow");
				 //$("#bill").css({"border":"1px solid red"});
				
				 //$(" #nav nav-pills1").css({"display":"none"})
				  //$("#n1").css({"display":"none"})
				 $("#item-list").css({"display":"none"})
				//$("#subcategory-2").css({"display":"block"});
				
				$("#subcategory-list").css({"display":"none"});
			//	$("#cp").css({"display":"block"});
				 $('.ad12').html(addon_tr_html);
				
//addon_tr_html +="<tfoot><tr id="row_ + row_no + '"><td colspan='4'><button type='button' class='btn btn-primary pull-right AddonItem' id='AddonItem'>Submit</button></td></tr></tfoot>"; 
...
$(document).ready(function(){
    $("#row_id").delegate("tr.rows", "click", function(){
        alert("Click!");
    });
});
...
//alert('test');	
	...
	var check = $('#'+rowid).children('td').children('input[name="addon_id"]:checked').val();
	//alert('test');
	 var c = check ? check : 0;
	if(c != 0){
		alert(1);
		$('#'+rowid).children('td').css('background-color','#428bca');
	}else{
		alert(2);
		//alert(check);
		$('#'+rowid).children('td').css('background-color','#fff');
	}
	//$('.recipeaddon').trigger('click'); 
  ...
  if($(this).val()>0){
	 return false;
	  ...
	  $('#'+rowid).children('td').css({"background-color": "#fff", "color": "#333"});
  }
	var check = $('#'+rowid).children('td').children('input[name="addon_id"]:checked').val();
	 var c = check ? check : 0;
	if(c != 0){
		$('#'+rowid).children('td').children('input[name="addon_id"]').attr('checked', false);
		$('#'+rowid).children('td').css({"background-color": "#fff", "color": "#333"});
		// $(this).parent().find('.addon_quantity').attr("readonly",true);
		  $(this).parent().find('.addon_quantity').val('');
	}else{
		$('#'+rowid).children('td').children('input[name="addon_id"]').attr('checked', true);
		$('#'+rowid).children('td').css({"background-color": "#428bca", "color": "#fff"});
		$(this).focus();
                $(this).parent().find('.addon_quantity').attr("readonly",false);
                $(this).parent().find('.addon_quantity').val('');
	}
	}); 
	 $(document).on('click', '.AddonItemdel', function () {
        var row = $('#' - $('#row_id1').val());
	 });
...
/* $(document).on('click', '#customized_saved', function() {
...
/*variant check*/
		 ...
		 //buy x get y part end///
            ...
            if (pos_settings.item_addon == 1 && item.addons != false) {
                tr_html += '<div class="col-xs-4" padding-left:15px;><div class="text-center"><span class="label label-primary fa-bx'+(item_comment != '' ? '' :'-o')+' tip pointer edit1 addon_css" id="' + row_no + '" data-item="' + item_id + '"  title="Edit" style="font-size:10px;'+$addondidbledstyle+'" >Addon</span></div></div>';
            }
