...
$('#posdiscount').focus(function () {
    ...
    --------------------------- */
     $(document).on("click",".rquantity",function() {
        $(this).select();
    });
    ...
    $(document).on("focus", '.rquantity', function () {
        ...
        localStorage.setItem('positems', JSON.stringify(positems));
         // $('.manual_dis_text').trigger('change');
        loadItems();
    });
// end ready function

/*manual discount start*/
    $(document).on("click", '.manual_dis_text', function () {
       /* var row = $(this).closest('tr');
        var manualitem_id = row.attr('id');
        var arr = manualitem_id.split('_');
        var idval = arr[1];
        alert($('#manual_item_discount_val_'+ idval));
        $('#manual_item_discount_val_'+ idval).select(); */
        // manual_item_discount_1548681600498
    // $(this).select();
    });
    var manualdistext = 0;
    $(document).on("focus", '.manual_dis_text', function () {
        ...
        /*if (manualdistext.indexOf("%") !== -1) {
                var pds = manualdistext.split("%");
                if (!isNaN(pds[0])) {
                    manual_item_ds = formatDecimal((parseFloat(((total) * parseFloat(pds[0])) / 100)), 4);
                } else {
                    manual_item_ds = parseFloat(manualdistext);
                }
            } else {
                manual_item_ds = parseFloat(manualdistext);
            }*/
       ...
       item_id = row.attr('data-item-id');
        // positems[item_id].row.manual_item_ds = manual_item_ds;
...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var item_cost = item.row.item_price;
