...
function loadItems() {
    if (localStorage.getItem('reqitems')) {
        ...
        $.each(sortedItems, function () {
		    ...
		    cm_id = item.row.cm_id ? item.row.cm_id :0,
		...
		if (item.units) {
            ruom = '<select name="uom[]" class="form-control p_uom" style="width:72px;"><option value="">UOM</option>';
            $.each(item.units, function () {    
			$selected=(item.row.product_uom==this.id)? 'selected':'';
                ruom +='<option value="'+this.id+'" '+$selected+'>'+this.name+'</option>';
            });
			ruom +='</select>';
        }
	       ...
	       if (category_name==null) {category_name ='';}if (category_id==null) {category_id =0;}
		    ...
		    if (brand_name==null) {brand_name ='';}if (brand_id==null) {brand_id =0;}
	         tr_html +='<td>'+
		     '<input name="brand_id[]" type="hidden" class="bid" value="' + brand_id + '"><input name="cm_id[]" type="hidden" class="cm_id" value="' + cm_id + '">'+
...
$(document).on("change", '.p_uom', function () {
	    $uom=$(this).val();
		var row = $(this).closest('tr');
		item_id = row.attr('data-item-id');
		var new_qty = parseFloat(reqitems[item_id].row.qty);
            $.each(reqitems[item_id].units, function(){
                if (this.id == $uom) {
                    reqitems[item_id].row.base_quantity = unitToBaseQty(new_qty, this);
                }
            });
		reqitems[item_id].row.product_uom=$(this).val();
        localStorage.setItem('reqitems', JSON.stringify(reqitems));
        loadItems();
	});
