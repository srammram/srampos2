    ...
    $('#powarehouse').select2("val", powarehouse);
        ...
        $('#ponote').redactor({
            ...
            changeCallback: function (e) {
                var v = this.get();
                localStorage.setItem('pi_note', v);
            }
        });
...
$('#pi_discount').focus(function () {
	...
	$(document).on("change", '.rtax2', function () {
		...
		item_id = row.attr('data-item-id');
		pi_items[item_id].tax_rate = current_tax;
		localStorage.setItem('pi_items', JSON.stringify(pi_items));
		loadItems();
	});
    ...
    $(document).on("change", '.rdiscount', function () {
        ...
        item_id = row.attr('data-item-id');
        pi_items[item_id].row.discount = current_discount;
        localStorage.setItem('pi_items', JSON.stringify(pi_items));
        loadItems();
    });       
    /* --------------------------
...
function loadItems() {
    ...
    if (localStorage.getItem('pi_items')) {
       ...
       // sortedItems = (site.settings.item_addition == 1) ? _.sortBy(pi_items, function(o){return [parseInt(o.order)];}) : pi_items;
        ...
        $.each(sortedItems, function () {			
           ...
           /* tr_html += '<td>'+$i+'</td>';
            tr_html += '<td><input class="form-control rcode" name="rcode[]" type="text" value="' + item_code + '" data-id="' + row_no + '" data-item="' + item_id + '" id="rcode_' + row_no + '"></td>';

            tr_html += '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input class="form-control rname" name="product_name[]" type="text" value="' + item_name + '" data-id="' + row_no + '" ><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><span class="sname" id="name_' + row_no + '">' + item_code +' - '+ item_name +(sel_opt != '' ? ' ('+sel_opt+')' : '')+' <span class="label label-default">'+item_supplier_part_no+'</span></span><i class="pull-right fa fa-eye tip delivery_store" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';*/
        ...
        $('#purchase_invoicesTable tfoot').html(tfoot);
