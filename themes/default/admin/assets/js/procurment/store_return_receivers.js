...
function loadItems() {
	 if (localStorage.getItem('store_rtnrec_items')) {
	    ...
	    var item = this;
			...
			$received_qty =v.received_qty ? v.received_qty : 0;
			$landingCost =v.landing_cost ? v.landing_cost : 0;
			$return_qty =v.return_qty ? v.return_qty : v.received_qty;
			$product_gross_amt = $received_qty*$product_price;
			...
			$batch_html +='<th>return received qty</th>';
			...
			$batch_html += '<td style="width: 78px;font-size: 13px;">'+$product_batch_label+'<input type="hidden" name="batch['+item_id+']['+n+'][stock_id]" value="'+$product_stock_id+'"><input type="hidden" name="batch['+item_id+']['+n+'][batch_no]" value="'+$product_batch+'"><input type="hidden" name="batch['+item_id+']['+n+'][vendor_id]" value="'+$product_vendor+'"><input type="hidden" name="batch['+item_id+']['+n+'][invoice_id]" value="'+v.invoice_id+'"><input type="hidden" name="batch['+item_id+']['+n+'][itemid]" value="'+v.itemid+'"></td>';
			...
			$batch_html += '<td><select name="batch['+item_id+']['+n+'][r_type]"><option value="damaged">damaged</option><option value="exist Qty">exist Qty</option></select></td>';
			$batch_html +='</tr>';
			$batch_html +'</tbody>';
			$batch_html +='</table>';
			$html +=$batch_html;
	    ...
	    $html +='</td>';
	    $html +='</tr>';
        $('#store_return_receiversTable>tbody').append($html);
	    $total_no_items++;
        });
...
if($('#store_rtnrec_from_store_id').val()!=''){
          $val = $('#store_rtnrec_from_store_id').val();
          if ($val!='') {
              load_receiver_no($val);
          }
     }
$('#store_rtnrec_from_store_id').change(function(){
          $val = $(this).val();
          if ($val!='') {
              load_receiver_no($val);
          }
});
function load_receiver_no($val) {
          $("#receiver_id").select2("val", "");
          $("#receiver_id").empty();
          if ($val!='') {
              $.ajax({
                    type: 'get',
                    url: site.base_url+'procurment/store_return_receivers/getstore_receiver_no',
                    dataType: "json",
                    data: { store_id: $val},
                    success: function (data) {
                        console.log(data);
						...
						$('#receiver_id').prepend('<option selected=""></option>').select2({placeholder: "Select  "});
                         if(localStorage.getItem('receiver_id')){
                              $receiver_id = localStorage.getItem('receiver_id');
                              $('#receiver_id').select2("val", $receiver_id);
                         }
                    }
                });
			  
          }
          localStorage.setItem('store_rtnrec_from_store_id',$val);
}

 $("#receiver_id").on("select2-selecting", function(e) {
          $val = e.choice.id;
          if ($val!='') {
              $.ajax({
                    type: 'get',
                    url: site.base_url+'procurment/store_return_receivers/get_store_receivers_data',
                    dataType: "json",
                    data: {
                        store_id: $('#from_store_id').val(),
                        'store_receivers_id':$val
                    },
                    success: function (data) {
                        localStorage.setItem('store_rtnreceivers_date',data.date);
                        $('#store_rtnreceivers_date').val(data.date);
          ...
          localStorage.setItem('receiver_id',$val);
     });