...
$(document).ready(function(){
	 $(document).on('click', '.set-unique-discount', function(e) {
        e.preventDefault();
       
        var $disid = $(this).attr('data-id');
        $url =  $(this).attr('data-url');
        $.ajax({type: "post", data:{disid:$disid},url: $url, dataType: 'json',
            success: function(data) {
                if (data.error == 1) { addPOSAlert(data.msg, 'danger'); }
                else {
			$('#multi-unique-discount').html('<div class="unique-dis-set-success"><span>'+data.msg+'</span></div>');
			setTimeout(function(){ $('#multiunique_discount').modal('hide');},2000);
		} 
            },
            error: function(data) { addPOSAlert('Ajax call failed', 'danger'); }
        });
        return false;
    });
});
function addPOSAlert(message, type) {
