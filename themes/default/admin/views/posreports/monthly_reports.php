...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('category_wise_monthly_sales_report'); ?> <?php
            if ($this->input->post('start_date')) {
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.monthlysale_report', function () {
            $url = '<?=site_url('api/v1/posreports/get_monthly_reports');?>';
            GetData($url);
        });
    ...
    if (start !='') {                  
        ...
        /*$('#kot').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');*/
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {'api-key':api_key,start: start, warehouse_id: warehouse_id,pagelimit:pagelimit},
                        dataType: "json",
                        cache: false,
                        success: function (data) {
