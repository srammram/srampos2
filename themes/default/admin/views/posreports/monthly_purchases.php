                ...
                <?php if (!empty($warehouses) && !$this->session->userdata('warehouse_id')) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?=site_url('api/v1/posreports/monthly_purchases/0/'.$year)?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            <li class="divider"></li>
                            <?php
                                foreach ($warehouses as $warehouse) {
                                        echo '<li><a href="' . site_url('api/v1/posreports/monthly_purchases/'.$warehouse->id.'/'.$year) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                                    }
                                ?>
                        </ul>
                    </li>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped dfTable reports-table">
                        ...
                        <tr class="year_roller">
                            <th><a class="white" href="<?= site_url('api/v1/posreports/monthly_purchases/'.($warehouse_id ? $warehouse_id : 0).'/'.($year-1).'?api-key='.$_GET['api-key']); ?>">&lt;&lt;</a></th>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('api/v1/posreports/monthly_purchases/'.($warehouse_id ? $warehouse_id : 0).'/'.$year.'/pdf')?>";
            return false;
        });
