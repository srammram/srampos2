...
<script>
    /*$(document).ready(function () {
        oTable = $('#KOTData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('api/v1/posreports/getRecipeReport/?v=1' . $v) ?>',
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.user_report', function () {
            $offset = false;
            $url = '<?=site_url('api/v1/posreports/get_user_reports');?>';
            GetData($url);
        });
    ...
    if (start !='' && end !='' && user !='' ) {                  
        ...
        $('#user').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {'api-key':api_key,start: start, end: end, user: user,pagelimit:pagelimit,group:group},
                        dataType: "json",
                        success: function (data) {
                            ...
                            $('#USERData > tbody').append('<tr><td colspan="7" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                            }
