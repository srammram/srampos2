    ...
    <div id="payments-con" class="tab-pane fade in">
        ...
        $(document).ready(function () {
            ...
            oTable = $('#PayRData').dataTable({
                ...
                "aoColumns": [null,null, null, null, {"bVisible": false}, {"mRender": paid_by}, {"mRender": currencyFormat}, {"mRender": row_status}],
                'fnRowCallback': function (nRow, aData, iDisplayIndex) {
                    nRow.id = aData[8];
...
<div id="quotes-con" class="tab-pane fade in">
    ...
    $(document).ready(function () {
        oTable = $('#QuRData').dataTable({
            ...
            "iDisplayLength": <?= $Settings->rows_per_page ?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?= site_url('api/v1/posreports/getQuotesReport/?v=1&customer='.$user_id) ?>',
            'fnServerData': function (sSource, aoData, fnCallback) {
                aoData.push({
                    "name": "<?= $this->security->get_csrf_token_name() ?>",
                    "value": "<?= $this->security->get_csrf_hash() ?>"
            ...
            "aoColumns": [null,{"mRender": fld}, null, null, null, {
                "bSearchable": false,
                "mRender": pqFormat
...
<div id="deposits-con" class="tab-pane fade in">
    ...
    $(document).ready(function () {
        oTable = $('#DepData').dataTable({
                ...
                "iDisplayLength": <?= $Settings->rows_per_page ?>,
                'bProcessing': true, 'bServerSide': true,
                'sAjaxSource': '<?= site_url('api/v1/posreports/get_deposits/'.$user_id) ?>',
                'fnServerData': function (sSource, aoData, fnCallback) {
                    aoData.push({
                        "name": "<?= $this->security->get_csrf_token_name() ?>",
                        "value": "<?= $this->security->get_csrf_hash() ?>"
                ...
                "aoColumns": [null,{"mRender": fld}, {"mRender": currencyFormat}, null, null, {"mRender": decode_html}],
                'fnRowCallback': function (nRow, aData, iDisplayIndex) {
                    var oSettings = oTable.fnSettings();
                    $index = oSettings._iDisplayStart+parseInt(iDisplayIndex) +parseInt(1) ;
                    $("td:first", nRow).html($index);
