...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.submit_hourlyreport', function () {
            $url = '<?=site_url('api/v1/posreports/get_HourlySummaryreports');?>';
            GetData($url);
        });
...
function GetData($url){ 
            ...
            //alert(time_range);           
            if (start_date !='' && end_date !='' && time_range !='') {
                 $('#start_date,#end_date').css('border-color', '#ccc'); 
                 $('#time_range').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        type: 'POST',
                        url: $url,                    
                        data: {'api-key':api_key,start_date: start_date, end_date: end_date, warehouse_id: warehouse_id,time_range:time_range,pagelimit:pagelimit, printlist:printlist},
                        dataType: "json",
                        success: function (data) {
                            $('#HRData').empty(); 
                ...
                if (time_range == '') {  
                    $('#time_range').siblings(".select2-container").find('.select2-choice').css('border-color', 'red');
                } 
                else
                {
                    $('#time_range').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
