...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('api/v1/posreports/get_settlementreports/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('api/v1/posreports/get_settlementreports/0/xls/?v=1'.$v)?>";
            return false;
        });
        ...
        $(document).on('click', '.report_settlement', function () {
            $url = '<?=site_url('api/v1/posreports/get_settlementreports');?>';
            GetData($url);
        });
            ...
            if (start_date !='' && end_date !='' ) {
                 $('#start_date,#end_date').css('border-color', '#ccc'); 
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {'api-key':api_key,start_date: start_date, end_date: end_date, warehouse_id: warehouse_id, defalut_currency:defalut_currency,pagelimit:pagelimit, printlist:printlist},
                        dataType: "json",
                        success: function (data) {
                             $('#SlRData > tbody').empty();
                                ...
                                $.each(data.settlements, function (a,b) 
                                {
                                    ...
                                    var $row_index = 1;
                                     ...
                                     {  
                                      ...
                                      $('#SlRData > tbody').append('<tr style="font-weight:bold" class="text-right"><td colspan="7" class="text-center">User Total: </td><td >'+formatMoney(cash)+'</td><td>'+formatMoney(cc)+'</td><td>'+formatMoney(credit)+'</td><td>'+formatMoney(foreign,for_currency)+'('+formatMoney(forusd)+')</td><td>'+formatMoney(bal)+'</td><td>'+formatMoney(amt)+'</td></tr>');             
                                });
