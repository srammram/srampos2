...
<script>
    $(document).ready(function () {
        ...
        oTable = $('#PayRData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('api/v1/posreports/getPaymentsReport/?v=1' . $v) ?>',
            ...
            "aoColumns": [null,null,{"mRender": fld}, null,null, {"mRender": paid_by}, {"mRender": currencyFormat}, {"mRender": currencyFormat},{"mRender": row_status}, {"bVisible": false}],
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                //nCells[3].innerHTML = $paidby_data;
            }
    ...
    $(document).ready(function () {
        ...
        $('#rsupplier').val(<?= $this->input->post('supplier') ?>).select2({
            ...
            initSelection: function (element, callback) {
                $.ajax({
                    ...
                    url: "<?= site_url('api/v1/possuppliers/getSupplier') ?>/" + $(element).val(),
                    dataType: "json",
        ...
        $('#rcustomer').val(<?= $this->input->post('customer') ?>).select2({
            ...
            initSelection: function (element, callback) {
                $.ajax({
                    type: "get", async: false,
                    url: "<?=site_url('api/v1/poscustomers/getCustomer') ?>/" + $(element).val(),
                    dataType: "json",
                    success: function (data) {
                        callback(data[0]);
                    }
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('end_date', (isset($_POST['end_date']) ? $_POST['end_date'] : ""), 'class="form-control date" autocomplete="off"  id="end_date"'); ?>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('api/v1/posreports/getPaymentsReport/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('api/v1/posreports/getPaymentsReport/0/xls/?v=1'.$v)?>";
            return false;
        });
