...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('home_delivery_report'); ?> <?php
            if ($this->input->post('start_date')) {
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.home_delivery', function () {
            $offset = false;
            $url = '<?=site_url('api/v1/posreports/get_homedelivery_reports');?>';
            GetData($url);
        });
        ...
        $('#kot').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if(data.home_delivery =='empty' || data.home_delivery == 'error'){
                             
                            $('#HomeDelieryData > tbody').append('<tr><td colspan="10" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                            }
                            else{ $('.dataTables_paginate').html(data.pagination);
                                    var amt = 0;                                   
                                    var round = 0;
                                    var $row_index = ($offset) ?parseInt($offset)+parseInt(1):1;
                                    $.each(data.home_delivery, function (a,b) 
                                    {        
                                       
                                       if(b.Orderdate != null)
                                       { 
                                        amt +=parseFloat(b.grand_total);         
                                        round = parseFloat(b.round);                         
                                        $('#HomeDelieryData > tbody').append('<tr class="text-center"><td>'+$row_index+'</td><td >'+b.Orderdate+'</td><td >'+b.Bill_No+'</td><td >'+b.branch+'</td><td>'+b.group_name+'</td><td>'+b.name+'</td><td>'+b.address+'</td><td>'+b.delivery_person+'</td><td>'+paid_by(b.paid_by)+'</td><td class="text-right">'+formatMoney(b.grand_total)+'</td></tr>');
                                         }    
                                        else{
                                    ...
                                    if(round != 0 )
                                    {
                                    $('#HomeDelieryData > tbody').append('<tr style="font-weight:bold"><td><strong>Round:</strong></td><td colspan="9" class="text-right">'+formatMoney((round))+'</td></tr>');
                                    }
                                     if(parseFloat((amt)+(parseFloat(round))) !=0)
                                    {
                                     $('#HomeDelieryData > tbody').append('<tr style="font-weight:bold"><td><strong>Grand Total:</strong></td><td colspan="9" class="text-right">'+formatMoney(amt)+'</td></tr>');
                                    } 
                           } 
                        }
                    });
...
function paid_by(x) {
            ...
            if (x) {
                ...
                if ($CC[1]!=0) {
                   $paidby_data +='</br> CC - '+formatMoney($CC[1]);
                }
                if ($Credit[1]!=0) {
                   $paidby_data +='</br> Credit - '+formatMoney($Credit[1]);
                }
