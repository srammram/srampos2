...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.user_report', function () {
            $offset = false;
            $url = '<?=site_url('api/v1/posreports/get_shifttime_reports');?>';
            GetData($url);
        });
    ...
    if (start !='' && end !='' && user !='' ) {                  
        ...
        $('#user').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {'api-key':api_key,start: start, end: end, user: user,pagelimit:pagelimit,shift:shift, defalut_currency:defalut_currency, printlist:printlist},
                        dataType: "json",
                        success: function (data) {
                             $('#SlRData > tbody').empty();
