...
<script>
    $(document).ready(function () {
        oTable = $('#PQData').dataTable({
            ...
            'sAjaxSource': '<?= site_url('api/v1/posreports/getQuantityAlerts' . ($warehouse_id ? '/' . $warehouse_id : '')) ?>',
                ...
                <?php if (!empty($warehouses)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li>
                                <a href="<?= site_url('api/v1/posreports/quantity_alerts') ?>">
                            ...
                            foreach ($warehouses as $warehouse) {
                                echo '<li ' . ($warehouse_id && $warehouse_id == $warehouse->id ? 'class="active"' : '') . '><a href="' . site_url('api/v1/posreports/quantity_alerts/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            }
                            ?>
                        </ul>
                    </li>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=site_url('api/v1/posreports/getQuantityAlerts/'.($warehouse_id ? $warehouse_id : '0').'/pdf')?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('api/v1/posreports/getQuantityAlerts/'.($warehouse_id ? $warehouse_id : '0').'/0/xls')?>";
            return false;
        });
