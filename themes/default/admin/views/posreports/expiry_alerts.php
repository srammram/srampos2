...
<script>
    $(document).ready(function () {
        oTable = $('#PExData').dataTable({
            ...
            'sAjaxSource': '<?=site_url('api/v1/posreports/getExpiryAlerts' . ($warehouse_id ? '/' . $warehouse_id : '')) ?>',
                ...
                <?php if (!empty($warehouses)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li>
                                <a href="<?=site_url('api/v1/posreports/expiry_alerts') ?>">
                            ...
                            foreach ($warehouses as $warehouse) {
                                echo '<li ' . ($warehouse_id && $warehouse_id == $warehouse->id ? 'class="active"' : '') . '><a href="' .site_url('api/v1/posreports/expiry_alerts/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            }
                            ?>
                        </ul>
                    </li>
