...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('pro_quotation_reports'); ?> <?php
            if ($this->input->post('start_date')) {
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.home_delivery', function () {
            $offset = false;
            $url = '<?=site_url('api/v1/posreports/get_pro_quotation');?>';
            GetData($url);
        });
                               ...
                               $('.dataTables_paginate').html(data.pagination);                                    
                                var $row_index = ($offset) ?parseInt($offset)+parseInt(1):1;
                                    ...
                                    $.each(data.report, function (a,b) 
                                    {
                                        ...
                                        cost_price +=parseFloat(b.cost)*parseFloat(b.quantity);

                                        // profit += (parseFloat(b.profit));

                                        $('#ReportData > tbody').append('<tr><td>'+$row_index+'</td><td class="text-center">'+b.reference_no+'</td><td class="text-center">'+b.quote_request+'</td><td class="text-center">'+b.store_request+'</td><td class="text-center">'+b.supplier+'</td><td class="text-center">'+b.product_code+'</td><td class="text-center">'+b.product_name+'</td><td class="text-center">'+b.category_name+'</td><td class="text-center">'+b.subcategory_name+'</td><td class="text-center">'+b.brand_name+'</td><td class="text-center">'+b.quantity+'</td><td class="text-center">'+b.cost+'</td></tr>');
                                        $row_index++;
                                    });
