...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('pro_purchase_order_reports'); ?> <?php
            if ($this->input->post('start_date')) {
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.home_delivery', function () {
            $offset = false;
            $url = '<?=site_url('api/v1/posreports/get_pro_purchase_order');?>';
            GetData($url);
        });
                            ...
                            if(data.report =='empty' || data.report == 'error'){
                               ...
                               $('.dataTables_paginate').html(data.pagination);                                    
                                    ...
                                    $.each(data.report, function (a,b) 
                                    {
                                        ...
                                        cost_price +=parseFloat(b.cost);
                                        item_dis +=parseFloat(b.item_disc_amt);
                                        item_bill_disc +=parseFloat(b.item_bill_disc_amt);
                                        tax_amt +=parseFloat(b.item_tax);
                                        selling_price +=parseFloat(b.selling_price);
                                        landing_cost +=parseFloat(b.landing_cost);
                                        gross_amt +=parseFloat(b.gross);
                                        margin +=parseFloat(b.margin);
                                        // profit += (parseFloat(b.profit));

                                        $('#ReportData > tbody').append('<tr><td>'+$row_index+'</td><td class="text-center">'+b.reference_no+'</td><td class="text-center">'+b.supplier+'</td><td class="text-center">'+b.product_code+'</td><td class="text-center">'+b.product_name+'</td><td class="text-center">'+b.category_name+'</td><td class="text-center">'+b.subcategory_name+'</td><td class="text-center">'+b.brand_name+'</td><td class="text-center">'+b.quantity+'</td><td class="text-center">'+b.cost+'</td><td class="text-center">'+b.item_disc_amt+'</td><td class="text-center">'+b.item_bill_disc_amt+'</td><td class="text-center">'+b.tax_rate+'</td><td class="text-center">'+b.item_tax+'</td><td class="text-center">'+b.selling_price+'</td><td class="text-center">'+b.landing_cost+'</td><td class="text-center">'+b.margin+'</td><td class="text-center">'+b.gross+'</td></tr>');
                                        $row_index++;
                                    });

                                  $('#ReportData > tbody').append('<tr style="font-weight:bold"><td colspan="8" >Total: </td><td class="text-right">'+formatDecimal(quantity)+'</td><td class="text-right">'+formatMoney(cost_price)+'</td><td class="text-right">'+formatMoney(item_dis)+'</td><td class="text-right">'+formatMoney(item_bill_disc)+'</td><td></td><td class="text-right">'+formatMoney(tax_amt)+'</td><td class="text-right">'+formatMoney(selling_price)+'</td><td class="text-right">'+formatMoney(landing_cost)+'</td><td>'+formatMoney(margin)+'</td><td class="text-right">'+formatMoney(gross_amt)+'</td></tr>');
