...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.bill_details', function () {
            $url = '<?=site_url('api/v1/posreports/get_bill_details_reports');?>';
            GetData($url);
        });
    ...
    if (start !='' && end !='') {                  
        ...
        $('#kot').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {'api-key':api_key,start: start, end: end, bill_no : bill_no, warehouse_id : warehouse_id,pagelimit:pagelimit, printlist:printlist},
                        dataType: "json",
                        success: function (data) {
                            ...
                            $('#BillDetailsData > tbody').append('<tr><td colspan="4" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                            }
                            else{
                                ...
                                {   console.log(b)
                                    ...
                                    $table_whitelisted = (d.table_whitelisted==1)?'table_whitelisted':'';
                                     ...
                                     else
                                     {
                                        ...
                                        $('#BillDetailsData > tbody').append('<tr class="text-center '+$table_whitelisted+'"><td>'+d.bill_time+'</td><td>'+d.branch+'</td><td>'+d.bill_number+'</td><td>'+d.kitchenno+'</td><td>'+d.item+'</td><td >'+formatQuantity(d.quantity)+'</td><td class="text-right">'+formatMoney(billamt)+'</td></tr>');                 
                                    });
...
$(document).ready(function(){
    ...
    $(document).on('change', '#end_date', function () {        
        ...
        $.ajax({
        type: 'POST',
        url: '<?=site_url('api/v1/posreports/get_bill_no');?>',
