                    ...
                    <table id="ReportData"
                       ...
                       <thead>
                          <tr>
                            <th><?= lang("s_no"); ?></th>
                            <th><?= lang("po_no"); ?></th>
                            <th><?= lang("Quotation_request_no"); ?></th>
                            <th><?= lang("purchase_quotation_no"); ?></th>
			    <th><?= lang("supplier"); ?></th>                            
                            <th><?= lang("gross"); ?></th>
                            <th><?= lang("item_discount"); ?></th>
                            <th><?= lang("bill_discount"); ?></th>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.home_delivery', function () {
            $offset = false;
            $url = '<?=site_url('api/v1/posreports/get_pro_purchase_order_summary');?>';
            GetData($url);
        });
                  ...
                  $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if(data.report =='empty' || data.report == 'error'){
                                  ...
                                  $('#ReportData > tbody').append('<tr><td colspan="8" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                            }
                            else{
                               ...
                               $('.dataTables_paginate').html(data.pagination);                                    
                                    ...
                                    $.each(data.report, function (a,b) 
                                    {
                                        ...
                                        gross +=parseFloat(b.gross);
                                        item_dis +=parseFloat(b.item_discount);
                                        
                                        item_bill_disc +=parseFloat(b.bill_discount);
                                        tax_amt +=parseFloat(b.total_tax);
                                        round_off +=parseFloat(b.round_off);
                                        grand_total +=parseFloat(b.grand_total);

                                        // profit += (parseFloat(b.profit));

                                         $('#ReportData > tbody').append('<tr><td>'+$row_index+'</td><td class="text-center">'+b.reference_no+'</td><td class="text-center">'+b.quotation_req_no+'</td><td class="text-center">'+b.quotation_no+'</td><td class="text-center">'+b.supplier+'</td><td class="text-center">'+b.gross+'</td><td class="text-center">'+b.item_discount+'</td><td class="text-center">'+b.bill_discount+'</td><td class="text-center">'+b.total_tax+'</td><td class="text-center">'+b.round_off+'</td><td class="text-center">'+b.grand_total+'</td></tr>');$row_index++;
                                    });

                                  $('#ReportData > tbody').append('<tr style="font-weight:bold"><td colspan="5" >Total: </td><td class="text-right">'+formatMoney(gross)+'</td><td class="text-right">'+formatMoney(item_dis)+'</td><td class="text-right">'+formatMoney(item_bill_disc)+'</td><td class="text-right">'+formatMoney(tax_amt)+'</td><td class="text-right">'+formatMoney(round_off)+'</td><td class="text-right">'+formatMoney(grand_total)+'</td></tr>');
