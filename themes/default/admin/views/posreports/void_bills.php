...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.voidmail', function () {
            $url = '<?=site_url('api/v1/posreports/get_voidbills_reports');?>';
            GetData($url);
        });
                  ...
                  $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if(data.void_bills =='empty'){ 
                                ...
                                $('#VoidBillData > tbody').append('<tr><td colspan="10" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');
                                ...
                                $.each(data.void_bills, function (a,b) 
                                {
                                    total  += parseFloat(b.bill_amt);
                                     ...
                                     $('#VoidBillData > tbody').append('<tr class="text-center"><td>'+b.bill_number+'</td><td>'+b.date+'</td><td>'+b.branch+'</td><td>'+b.order_item_cancel_note+'</td><td>'+b.created_by+'</td><td>'+b.Canceled+'</td><td>'+b.kot+'</td><td>'+b.recipename+'</td><td>'+b.quantity+'</td><td class="text-right">'+formatMoney(b.bill_amt)+'</td></tr>');
                                });

                                 $('#VoidBillData > tbody').append('<tr style="font-weight:bold" ><td><strong>Total: </strong></td><td class="text-right" colspan="9">'+formatMoney(total)+'</td></tr>');
                            }
                        }
                    });  
            }
