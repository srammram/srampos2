...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.stock_audit', function () {
            $url = '<?=site_url('api/v1/posreports/get_StockAuditreports');?>';
            GetData($url);
        });
            ...
            if (start_date !='' ) {
                 ...
                 /*$('#product_id').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc'); */
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {'api-key':api_key,start_date: start_date,product_id: product_id,warehouse_id: warehouse_id,pagelimit:pagelimit},
                        dataType: "json",
                        cache: false,
                        success: function (data) {
