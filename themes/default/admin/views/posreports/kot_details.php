...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.kot_report', function () {
            $url = '<?=site_url('api/v1/posreports/get_kotdetailsreports');?>';
            GetData($url);
        });
    ...
    if (start_date !='' && end_date !='' && kot !='' ) {                  
        ...
        $('#kot').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {'api-key':api_key,start_date: start_date, end_date: end_date, kot: kot, warehouse_id: warehouse_id,pagelimit:pagelimit},
                        dataType: "json",
                        success: function (data) {
                            $('#KOTData > tbody').empty();  
                            $('#KOTData > thead').empty();  
                            ...
                            $('#KOTData > tbody').append('<tr><td colspan="8" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                                ...
                                if(kot == 'kot_cancel')
                                {                                      
                                    $('#KOTData > thead').append('<tr><th><?= lang("s.no"); ?></th><th><?= lang("kot_no"); ?></th><th><?= lang("kot_date"); ?></th><th><?= lang("user"); ?></th><th><?= lang("recipe"); ?></th><th><?= lang("table"); ?></th><th><?= lang("status"); ?></th></tr>');
                                    $('#KOTData > tbody').empty(); 
                                    $.each(data.kotdetails, function (a,b) 
                                    {   
                                        $('#KOTData > tbody').append('<tr class="text-center"><td>'+$row_index+'</td><td>'+b.id+'</td><td>'+b.date+'</td>><td>'+b.username+'</td><td>'+b.recipename+'</td><td>'+b.table_name+'</td><td>'+b.order_item_cancel_note+'</td></tr>');
                                    $row_index++;
                                    });
                                }
                                    ...
                                    $('#KOTData > thead').append('<tr><th><th><?= lang("s.no"); ?></th><?= lang("kot_no"); ?></th><th><?= lang("date"); ?></th><th><?= lang("table_no"); ?></th><th><?= lang("item"); ?></th><th><?= lang("user"); ?></th><th><?= lang("qty"); ?></th><th><?= lang("amount"); ?></th></tr>');
                                     $.each(data.kotdetails, function (c,d) 
                                        ...
                                        if(d.table_name == null)
                                        {
                                            ...
                                            $('#KOTData > tbody').append('<tr class="text-center"><td>'+$row_index+'</td><td>'+d.id+'</td><td >'+d.Orderdate+'</td><td>'+table_name+'</td><td>'+d.recipename+'</td><td>'+d.username+'</td><td class="text-right">'+formatQuantity(d.quantity)+'</td><td class="text-right">'+formatMoney(d.subtotal)+'</td></tr>');
                                        $row_index++;
                                        });
                                     $('#KOTData > tbody').append('<tr style="font-weight:bold"><td>Total</td><td colspan="5"></td><td  class="text-right">'+formatQuantity(pen_qty)+'</td><td class="text-right">'+formatMoney(pen_tot)+'</td></tr>');
                               }
                               else
                                {   

                                    $('#KOTData > thead').append('<tr><th><?= lang("s.no"); ?></th><th><?= lang("kot_no"); ?></th><th><?= lang("kot_date"); ?></th><th><?= lang("time"); ?></th><th><?= lang("bill_no"); ?></th><th><?= lang("user"); ?></th><th><?= lang("table"); ?></th><th><?= lang("order_type"); ?></th><th><?= lang("steward"); ?></th><th><?= lang("item"); ?></th><th><?= lang("qty"); ?></th><th><?= lang("value"); ?></th></tr>');
                                    var grd_amt = 0;
                                    var round = 0;
                                    var grand_qty = 0;
                                    ...
                                    { 
                                       ...
                                       {  
                                        ...
                                        if(h.table_name == null)
                                        {
                                         ...
                                         $('#KOTData > tbody').append('<tr class="text-center"><td>'+$row_index+'</td><td>'+h.kitchenno+'</td><td>'+h.kot_date+'</td><td>'+h.kot_time+'</td><td>'+h.Bill_No+'</td><td>'+h.username+'</td><td>'+table+'</td><td>'+h.order_type+'</td><td>'+h.steward+'</td><td>'+h.item+'</td><td class="text-right">'+formatQuantity(h.quantity)+'</td><td class="text-right">'+formatMoney(h.Bill_amt)+'</td></tr>');      
                                        $row_index++;
                                       });

                                     $('#KOTData > tbody').append('<tr style="font-weight:bold"><td>User Total:</td><td colspan="9"></td><td  class="text-right">'+formatQuantity(qty)+'</td><td class="text-right">'+formatMoney(amt)+'</td></tr>');
                                  ...
                                  $('#KOTData > tbody').append('<tr style="font-weight:bold"><td><strong>Grand Total:</strong></td><td colspan="9"></td><td  class="text-right">'+formatQuantity((grand_qty))+'</td><td class="text-right">'+formatMoney(grd_amt)+'</td></tr>');    
                            }
                           } 
                        }
                    });
