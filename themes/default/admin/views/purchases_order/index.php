...
<script>
    $(document).ready(function () {
        oTable = $('#purchasesorderTable').dataTable({
            "aaSorting": [[1, "desc"], [2, "desc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?=lang('all')?>"]],
            "iDisplayLength": <?=$Settings->rows_per_page?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?=admin_url('purchases_order/getPurchasesOrder' . ($warehouse_id ? '/' . $warehouse_id : ''))?>',
            ...
            "aoColumns": [{"bSortable": false,"mRender": checkbox}, {"mRender": fld}, null,null, {"mRender": currencyFormat},{"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat},   {"bSortable": false}],
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
                var oSettings = oTable.fnSettings();
                nRow.id = aData[0];
                nRow.className = "purchases_order_link";
                return nRow;
            },
...
<?php if ($Owner || $GP['bulk_actions']) {
	    echo admin_form_open('purchases_order/purchases_order_actions', 'id="action-form"');
	}
?>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-star"></i><?=lang('purchases_order') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')';?>
        </h2>
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?=admin_url('purchases_order/add')?>">
                                <i class="fa fa-plus-circle"></i> <?=lang('add_purchases_order')?>
                            </a>
                        </li>
                        <!-- <li>
                        ...
                        <li class="divider"></li>
                        <li>
                            <a href="#" class="bpo" title="<b><?=lang("delete_purchases_order")?></b>"
                                data-content="<p><?=lang('r_u_sure')?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?=lang('i_m_sure')?></a> <button class='btn bpo-close'><?=lang('no')?></button>"
                                data-html="true" data-placement="left">
                                <i class="fa fa-trash-o"></i> <?=lang('delete_purchases_order')?>
                            </a>
                        </li> -->
                    </ul>
                </li>
                <?php if (!empty($warehouses)) {
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?=admin_url('purchases_order')?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            <li class="divider"></li>
                            <?php
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table id="purchasesorderTable" cellpadding="0" cellspacing="0" border="0"
