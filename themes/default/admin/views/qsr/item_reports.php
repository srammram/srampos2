            ...
            <div>
              ...
              $todate = $this->input->get('todate');
                ...
                if(!empty($recipes)){   
                    ...
                    <th><?= lang("amount"); ?></th>
                     ...
                     foreach($table->split_order as $recipesa){ 
                        ...
                        foreach ($recipesa->order as  $value) {
                             ...
                             $qty+= $value->quantity;
                             $total+= (((($value->subtotal-$value->item_discount)-$value->off_discount)-$value->input_discount)+$value->tax);

                             $group_qty += $value->quantity; 
                             $group_total += (((($value->subtotal-$value->item_discount)-$value->off_discount)-$value->input_discount)+$value->tax);

                             $grand_qty+= $value->quantity;
                             $grand_total+= (((($value->subtotal-$value->item_discount)-$value->off_discount)-$value->input_discount)+$value->tax);                             

                             echo '<tbody>
                                <tr>
                                <td>'.$value->name.'</td>
                                <td class="text-right">'.$this->sma->formatQuantity($value->quantity).'</td>
                                <td class="text-right">'.$this->sma->formatMoney((((($value->subtotal-$value->item_discount)-$value->off_discount)-$value->input_discount))+$value->tax).'</td></tr>';
                           }
                        ?>
                         <tr>
                    ...
                    ?>
                     <tr>
                            <td><strong><?= lang("group_total:"); ?></strong></td>
                     ...
                     ?>
                      ...
                      <tr>
                            <td><strong><?= lang("grand_total:"); ?></strong></td>
                            <td class="text-right"><?php echo $this->sma->formatQuantity($grand_qty); ?></td>
                            <td class="text-right"><?php echo $this->sma->formatMoney($grand_total); ?>                            
                            </td>
                        </tr> 
...
<script type="text/javascript">
    $('#submit-sale').click(function () {
            ...
            window.location.href= url +'/?type=1'+'&fromdate='+from_date+'&todate='+to_date; 
    });
