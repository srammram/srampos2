...
if ($pos_settings->remote_printing != 1) {
    ...
    <script type="text/javascript">
        ...
        function receiptData() {
            ...
            receipt.items += "<?= printLine("   ".($this->sma->formatQuantity($row->	).' '.$row->product_unit_code)." x ".$this->sma->formatMoney($row->unit_price) . ":  ". $this->sma->formatMoney($row->subtotal), $char_per_line, ' '); ?>" + "\n";
            <?php $r++; endforeach; ?>
            <?php
            if ($return_rows) { ?>
                receipt.items += "\n" + "<?=lang('returned_items');?>" + "\n";
