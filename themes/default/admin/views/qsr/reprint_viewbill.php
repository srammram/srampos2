                    ...
                    <tfoot>
                        ...
                        if ($Settings->indian_gst) {
                            if ($inv->cgst > 0) {
                                ...
                                echo '<tr><td colspan="4">' . lang('cgst') .'</td><td class="text-right">' . ( $Settings->format_gst ? $this->sma->formatMoney($cgst) : $cgst) . '</td></tr>';
                            }
                            if ($inv->sgst > 0) {
                                $sgst = $return_sale ? $inv->sgst + $return_sale->sgst : $inv->sgst;
                                echo '<tr><td colspan="4">' . lang('sgst') .'</td><td class="text-right">' . ( $Settings->format_gst ? $this->sma->formatMoney($sgst) : $sgst) . '</td></tr>';
                            }
                            if ($inv->igst > 0) {
    ...
    left: 0px;" class="no-print">
            ...
            if ($modal) {
                ...
                <span class="col-xs-12">
                    <a class="btn btn-block btn-warning" href="<?= admin_url('qsr/reprinter'); ?>"><?= lang("back_to_reprinter"); ?></a>
                </span>
                <?php
            }
            if ($pos->remote_printing == 1) {
