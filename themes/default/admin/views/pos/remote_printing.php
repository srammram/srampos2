...
if ($pos_settings->remote_printing != 1) {
    ...
    <script type="text/javascript">
        ...
        function receiptData() {
            ...
            receipt.items = "Description         Price   Qty  Sub Total\n";
			receipt.items += "<?=str_replace("\n", "", drawLine($char_per_line));?>"; + "\n"; 
            <?php $r = 1; foreach ($rows as $row): ?>
            receipt.items += "<?= printLine(recipe_name(addslashes("#".$r." ".$row->recipe_name), 15)."  ".($this->sma->formatMoney($row->net_unit_price)." ".($row->quantity)." ".($this->sma->formatMoney($row->subtotal))), $char_per_line, ' '); ?>" + "\n";
            //receipt.items += "<?= $row->item_tax != 0 ? '   '.lang('tax').' ('.($Settings->indian_gst ? $row->tax : $row->tax_code).') '.$this->sma->formatMoney($row->item_tax).' ('.lang('hsn_code').': '.$row->hsn_code.')' : ''; ?>" + "\n";
           // receipt.items += "<?= printLine("   ".($this->sma->formatQuantity($row->unit_quantity).' '.$row->recipe_unit_code)." x ".$this->sma->formatMoney($row->unit_price) . ":  ". $this->sma->formatMoney($row->subtotal), $char_per_line, ' '); ?>" + "\n";
            <?php $r++; endforeach; ?>
            <?php
            if ($return_rows) { ?>
                receipt.items += "\n" + "<?=lang('returned_items');?>" + "\n";
                <?php $r = 1; foreach ($return_rows as $row): ?>
                receipt.items += "<?= printLine(recipe_name(addslashes("#".$r." ".$row->recipe_code." - ".$row->recipe_name).' '.($row->variant ? ' (' . $row->variant . ')' : ''), $char_per_line).": ".($row->tax_code ? '*'.$row->tax_code : ''), $char_per_line, ' '); ?>" + "\n";
                receipt.items += "<?= $row->item_tax != 0 ? '   '.lang('tax').' ('.($Settings->indian_gst ? $row->tax : $row->tax_code).') '.$this->sma->formatMoney($row->item_tax).' ('.lang('hsn_code').': '.$row->hsn_code.')' : ''; ?>" + "\n";
                receipt.items += "<?= printLine("   ".($this->sma->formatQuantity($row->unit_quantity).' '.$row->recipe_unit_code)." x ".$this->sma->formatMoney($row->unit_price) . ":  ". $this->sma->formatMoney($row->subtotal), $char_per_line, ' ') . ""; ?>" + "\n";
                <?php $r++; endforeach; ?>
                <?php
            ...
            receipt.totals = "";
            receipt.totals += "<?= printLine(lang("total") . ": " . $this->sma->formatMoney($return_sale ? (($inv->total + $inv->recipe_tax)+($return_sale->total + $return_sale->recipe_tax)) : ($inv->total + $inv->recipe_tax)), $char_per_line); ?>" + "\n";
            <?php
			if ($inv->total_discount != 0) {
			?>
				receipt.totals += "<?= printLine(lang("discount") . ": " . $this->sma->formatMoney($inv->total_discount), $char_per_line); ?>" + "\n";
				
			<?php
			}
			
			if ($inv->tax_rate != 0) {?>
				receipt.totals += "<?= printLine(lang("tax (".$inv->tax_name.")") . ": " . $this->sma->formatMoney($return_sale ? ($inv->order_tax+$return_sale->order_tax) : $inv->total_tax), $char_per_line); ?>" + "\n";
			<?php
			}
            ...
            if ($inv->total_discount != 0) { ?>
                receipt.totals += "<?= printLine(lang("discount") . ": (" . $this->sma->formatMoney($return_sale ? ($inv->recipe_discount+$return_sale->recipe_discount) : $inv->recipe_discount) . ") " . $this->sma->formatMoney($return_sale ? ($inv->order_discount+$return_sale->order_discount) : $inv->order_discount), $char_per_line); ?>" + "\n";
                <?php
            }
            if ($inv->shipping != 0) { ?>
                receipt.totals += "<?= printLine(lang("shipping") . ": ". $this->sma->formatMoney($inv->shipping), $char_per_line); ?>" + "\n";
            ...
            if ($Settings->invoice_view > 0) {
                if (!empty($tax_summary)) {
                    ...
                    <?php endforeach;?>
                    receipt.footer += "<?=str_replace("\n", "", drawLine($char_per_line));?>"; + "\n";
                    receipt.footer += "\n<?=printLine(lang("total_tax_amount") . ":" . $this->sma->formatMoney($inv->recipe_tax), $char_per_line);?>" + "\n";
                    receipt.footer += "<?=str_replace("\n", "", drawLine($char_per_line));?>" + "\n\n";
                    <?php
                }
            }
