        ...
        <div class="row">
            <div class="col-lg-12">
             ...
             echo admin_form_open("reports/recipe", $attrib);?> -->
                    ...
                    <div class="col-sm-3">
                        <div class="form-group">
                            <?= lang("bill_action", "bill_action"); ?></label>
                            <?php $action = ($this->session->userdata('bill_action'))?$this->session->userdata('bill_action'):'all';?>
                            <select name="bill_action" class="form-control select" id="bill_action" style="width:100px">
...
<script type="text/javascript">var $offset = false;
    $(document).ready(function () {
        ...
        $(document).on('click', '.bill_details', function () {
            $offset = false;
            $url = '<?=admin_url('pos/get_archived_bills');?>';
            GetData($url);
        });
                        ...
                        success: function (data) {
                            ...
                            $('#BillDetailsPrintData > tbody').append('<tr><td colspan="12" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                                ...
                                $.each(data.bill_details, function (a,b) 
                                {
                                    ...
                                    $t_data = '<tr class="text-center"><td>'+$row_index+'</td><td>'+b.date+'</td><td>'+b.bill_number+'</td><td>'+b.grand_total+'</td><td></td><td></td>';
                                   ...
                                   if (b.action==1) {
                                    $t_data +='<li><a href="<?=admin_url('pos/restore_modified_bill/')?>'+b.id+'" target="_blank"><i class="fa fa-money"></i>restore</a></li>';
                                   }else if (b.action==2) {
                                    $t_data +='<li><a href="<?=admin_url('pos/restore_deleted_bill/')?>'+b.id+'" target="_blank"><i class="fa fa-money"></i>restore</a></li>';
                                   }
                                ...
                                $('#BillDetailsPrintData > tbody').append('<tr class="text-right"><td colspan=4>'+formatMoney(grand_total)+'</td><td></td><td></td><td></td></tr>');
                           }
