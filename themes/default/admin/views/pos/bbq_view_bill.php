...
<div class="modal-dialog no-modal-header" role="document"><div class="modal-content"><div class="modal-body">
                ...
                <table class="table table-striped table-condensed">
                    ...
                    <tfoot>
                        ...
                        if($inv->birthday_discount != 0) {                  
                            echo '<tr><th colspan="4" class="text-right">' . lang("birthday_discount") .'(' . $this->pos_settings->birthday_discount . ')'.'</th><th class="text-right">' . $this->sma->formatMoney($inv->birthday_discount) . '</th></tr>';
                        } ?>

                            <?php
                            // var_dump($inv->service_charge_amount );die;
                            if($inv->service_charge_amount != 0)
                            {  $ServiceCharge = $this->site->getServiceChargeByID($inv->service_charge_id); ?>
                             <tr>
                               <th colspan="4" class="text-right"><?= $ServiceCharge->name;?></th>
                           ...
                           <?php  }
                            ...
                            {
                                $taxname = 'Inclusive';
                                $grandtotal = $inv->total-$inv->total_discount-$inv->bbq_cover_discount-$inv->birthday_discount-$inv->bbq_daywise_discount;
                            }
                            else
                            {
                                $taxname = 'Exclusive';
                                $grandtotal = $inv->total-$inv->total_discount-$inv->bbq_cover_discount-$inv->birthday_discount-$inv->bbq_daywise_discount+$inv->total_tax;
                            }                            
							?>
                            <tr>
                       ...
                       // } ?>
						...
						//if ($pos_settings->display_tax==1 && $inv->tax_rate != 0) {
							...
							echo '<tr><th colspan="6" class="text-left">*<small>' . lang("tax ".$taxname." (".$inv->tax_name.")") . '</small></th></tr>';
                        //}
						?>
						</tr>
                    </tfoot>
    ...
    left: 0px;" class="no-print">
            ...
            if (!empty($modal)) {
                ...
                <span class="col-xs-12">
                    <a class="btn btn-block btn-warning" href="<?= admin_url('pos/biller_bbqtable/'); ?>"><?= lang("back_to_BBQ_Bills"); ?></a>
                </span>
                <?php
            }
    ...
    <script type="text/javascript">
        ...
        <?php
        if ($pos_settings->remote_printing == 2) {
            ?>
