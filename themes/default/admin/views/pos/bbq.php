...
<head>
    ...
    <link rel="stylesheet" href="<?=$assets?>pos/css/posbbqajax.css" type="text/css"/>
    ...
    <?php if(@$_GET['bbqtid']): ?>
        <script>var curr_page="bbqadd";var curr_func="update_bbqtables";var tableid = '<?=$_GET['bbqtid']?>';</script>
    <?php endif; ?>
</head>
<body>
                              ...
                              <div class="no-print">
                                 ...
                                 <div class="form-group">
                                        <?php
                                            $st[''] = '';
                                                ...
                                                echo form_dropdown('order_type', $st, (isset($_POST['order_type']) ? $_POST['order_type'] : $get_order_type), 'id="posorder_type" class="form-control"   data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("type") . '" required="required" style="width:100%; pointer-events:none;" ');
                                            ?>
                                    </div>
								...
								?>
                                 <div class="form-group">
                                        ...
                                        <select  class="form-control" data-placeholder="Select Tables" id="postable_list" required style="width:100%; pointer-events:none;" <?php if($this->sma->actionPermissions('table_edit')){ echo ''; }else{  echo 'disabled'; }  ?>>
                                        	<?php
											if(!empty($areas)){
                                ...
                                <div class="form-group">
                                <input type="hidden" name="seats_id" id="seats_id" class="form-control  kb-pad text-center " placeholder="<?=lang('how_many_people')?>" >
                                </div>
                                <?php
								}
								?>
...
<div class="modal fade in" id="mModal" tabindex="-1" data-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="mModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true"><i
...
<script type="text/javascript">
    var recipe_variant = 0, shipping = 0, p_page = 0, per_page = 0, tcp = "<?=$tcp?>", pro_limit = <?=$pos_settings->pro_limit?>,
        brand_id = 0, obrand_id = 0, cat_id = "<?=$pos_settings->default_category?>", bbq_set_id ="<?=$bbq_set_id?>", ocat_id = "<?=$pos_settings->default_category?>", sub_cat_id = 0, osub_cat_id,
    ...
    $(document).on('click', '.recipe-varient', function (e) {
        ...
        $(document).on('click', '.category', function () {
            if (cat_id != $(this).val()) {
                ...
                var split = "<?php echo $this->input->get('split'); ?>";
                $.ajax({
                    type: "get",
                    url: "<?=admin_url('pos/ajaxcategorydata');?>",
                    data: {category_id: cat_id, warehouse_id: warehouse_id, recipe_standard: 2,order_type:order_type,split:split},
                    dataType: "json",
                    success: function (data) {
                        $('#item-list').empty();
                        var newPrs = $('<div></div>');
        ...
        $(document).on('click', '.subcategory', function () {
            if (sub_cat_id != $(this).val()) {
                ...
                $.ajax({
                    type: "get",
                    url: "<?=admin_url('pos/ajaxbbqrecipe');?>",
                    data: {category_id: cat_id, warehouse_id: warehouse_id, subcategory_id: sub_cat_id, per_page: p_page,order_type:order_type,split:split},
                    dataType: "html",
                    success: function (data) {
                        $('#item-list').empty();
                        var newPrs = $('<div></div>');
        ...
        $('#next').click(function () {
            ...
            if (tcp >= pro_limit && p_page < tcp) {
                ...
                $.ajax({
                    type: "get",
                    url: "<?=admin_url('pos/ajaxbbqrecipe');?>",
                    // data: {bbq_set_id: bbq_set_id, warehouse_id: warehouse_id, per_page: p_page},
                    data: {category_id: cat_id, warehouse_id: warehouse_id, subcategory_id: sub_cat_id, per_page: p_page,order_type :order_type,split:split},
                    dataType: "html",
                    success: function (data) {
                        $('#item-list').empty();
    ...
    $(document).ready(function () {
		...
		$('#sent_to_kitchen').click(function () {
          	...
          	if (count == 1) {
                ...
                if($this->pos_settings->kot_print_lang_option ==1 || $this->pos_settings->kot_print_lang_option ==2 ){  ?>
...
<script type="text/javascript" src="<?=$assets?>pos/js/posbbq.ajax.js?v=1"></script>
