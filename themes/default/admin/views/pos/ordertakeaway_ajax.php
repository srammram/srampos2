...
<div class="col-xs-12 kitchen_section">
	...
	<ul>
        ...
        <li><a href="<?php echo base_url().'admin/pos/order_takeaway'; ?>" class="active"><?=lang('take_away')?></a></li>
        <?php } ?>
...
<ul class="col-xs-12 ul_main">
    ...
    <ul class="col-xs-12">
        <?php
        foreach($takeaway as $takeaway_row){
            if($this->site->splitCheckSalestable($takeaway_row->split_id) == FALSE){
                $count_item = $this->site->splitCountcheck($takeaway_row->split_id);
        ?>
        <div class="row">
            <li class="col-xs-6 text-left split">
               <h2> <?php echo $takeaway_row->split_id; ?></h2>
            </li>
            
            <li class="col-xs-6 text-right new_split">
            	<?php if($this->sma->actionPermissions('new_order_create')){ ?>
                <a href="<?=admin_url('pos').'/?order=2&split='.$takeaway_row->split_id.'&same_customer='.$takeaway_row->customer_id.''?>"> <button class="btn btn-info"><?php echo lang("order_item") ?></button></a> 
                <?php }  if($this->pos_settings->kot_enable_disable == 1){ ?>
                    
                <button class="btn btn-danger" id="order_cancel_<?php echo $table->id;  ?>"  OnClick="send_kot('<?php echo $takeaway_row->split_id;  ?>');" ><?php echo lang("kot_print") ?></button>
                <?php } ?>

                <button class="btn btn-danger" id="order_cancel_<?php echo $takeaway_row->split_id;  ?>"  OnClick="CancelAllOrderItems('<?php echo $takeaway_row->split_id;  ?>','<?php echo $this->GP['pos-cancel_order_remarks'];?>');" <?php if($this->sma->actionPermissions('cancel_order_items')){ echo ''; }else{  echo 'disabled'; }  ?> ><?php echo lang("cancel_all") ?></button>
		
                  <button OnClick="bilGeneratorTakeaway('<?php echo $takeaway_row->split_id;  ?>', '<?php echo $count_item; ?>');" class="btn btn-warning" id="main_split_<?php echo $takeaway_row->split_id;  ?>" <?php if($this->sma->actionPermissions('bil_generator')){ echo ''; }else{  echo 'disabled'; }  ?>><?php echo lang("bill_generator") ?></button>
            ...
            <div style="clear:both;"></div>
            <?php
            if(!empty($takeaway_row->order)){
            ?>
            <li class="col-xs-12 ">
                <ul class="col-xs-12 item_list">
                    <?php
                    foreach($takeaway_row->order as $order){
                    ?>
						...
						<?php 
                                ...
                                <div class="col-xs-4 text-right">
                                     ...
                                     <?php
                                        ...
                                        <input type="checkbox" name="status_update_<?php echo $takeaway_row->split_id; ?>[]" value="<?php echo $item->id;  ?>" title="<?php echo $item->item_status; ?>" data-type="<?php echo $item->id;  ?>" data-split="<?php echo $takeaway_row->split_id; ?>" class="multiple_status status_<?php echo $takeaway_row->split_id; ?>">
                    ...
                    ?>
                     <button data-status="Ready" data-id="" data-split-id = "<?php echo $takeaway_row->split_id; ?>"  type="button" class="btn btn-success kitchen_status preparing_<?php echo $takeaway_row->split_id; ?> pull-right" style="display:none;">Served</button>
                            <button data-status="Served" data-id=""  data-split-id = "<?php echo $takeaway_row->split_id; ?>"   type="button" class="btn btn-success kitchen_status ready_<?php echo $takeaway_row->split_id; ?> pull-right" style="display:none;" >Closed</button>
                </ul>
            </li>
            <?php
            }
