...
<table class="table table-striped table-condensed" style="font-size:14px!important;">
                    ...
                    <tr>
                        ...
                        foreach ($bill_items['item'] as $bill) {                            
                            ...
                            echo '<tr><td colspan="2" style="display: table-cell;text-align: -webkit-match-parent;text-decoration:'.$underline.'"  class="no-border">' . ($star)  . ($recipe_name) ; 
                        ...
                        <tr>
                            ...
                            <th class="text-right"><?=$this->sma->formatMoney($bill_items['total']-$manual_discount_amount)?></th>
                        </tr>
                       ...
                       /* if ($manual_discount_amount != 0) {
                        ...
                        if ($pos_settings->display_tax == 1) {  
                            ...
                            $tax_rate = $this->site->getTaxRateByID($pos_settings->default_tax);
                            echo '<tr>';
                            echo '<th colspan="1"></th>';
                            echo '<th>' . $pos_settings->tax_caption. '</th>';
                            foreach ($tax_splits as $taxsplits) {
                                echo '<th>' . $taxsplits->name . '(Rs)</th>';
                            ...
                            echo '<tr>';
                            echo '<th colspan="1"></th>';
                            echo '<th>' . $this->sma->formatDecimal($tax_rate->rate). '</th>';
                            foreach ($tax_splits  as $tax_split) {                            
                                echo "<br>";
                                $tax_amount = ((($bill_items['grand_total']-$bill_items['tax']) * $tax_split->rate) / 100);                            
                                echo '<th>' . $this->sma->formatMoney($tax_amount) . '</th>';

                            }
                            echo '<th colspan="1" class="text-right">' . $this->sma->formatMoney($bill_items['tax']) . '</th>';
                            echo '</tr>';
                        }    
                        /*if ($pos_settings->display_tax == 1) {                            
                      ...
                      /*  if ($bill_items['service_amount'] != 1) {     
