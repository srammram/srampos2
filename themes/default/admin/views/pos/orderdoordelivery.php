...
function ajaxData()
{
	...
	$.ajax({
	  url: "<?=admin_url('pos/ajaxorder_doordelivery');?>",
	  type: "get",
	  success: function(response) {
			$("#orderdoordelivery_box").html(response);
	  }
	});
}
var ajaxDatatimeout;
...
<script type="text/javascript">
	...
	$(document).on('click','.kitchen_status',function(e){		
		...
		/*function bilGenerator( table_id, split_id )
		{
			$('#modal-loading').show();
			$.ajax({
				type: "get",
				url: "<?=admin_url('pos/ajaxBildata');?>",
				data: {table_id: table_id, split_id: split_id},
				dataType: "json",
				success: function (data) {
					if(data.status == true){
						alert('bil generator success');
						location.reload();
					}else{
...
$(document).ready(function() {
		...
		function bilGeneratordoordelivery( split_id, count_id )
	{    
	...
	$(document).on('click','#updateBil',function(){
		 ...
		 if(bils > 0){
			 window.location.href= url +'/billing/?order_type=3&bill_type='+bil_type+'&bils='+bils+'&splits='+split_id;
		 }else{
