...
function ajaxData(type_id)
{
	...
	$.ajax({
	  url: "<?=admin_url('pos/ajaxbbqconsolidated');?>",
	  type: "get",
	  data: { 
		type: type_id
	  },
    ...
    <div class="modal-dialog modal-md">
		...
		<h4 class="modal-title" id="payModalLabel"><?=lang('finalize_sale');?></h4>
            ...
            <div class="modal-body" id="payment_content">
             ...
             echo admin_form_open("pos/bbqconsolidatedpaymant", $attrib);
             $type = 4;
             ?>
                <div class="row">
    ...
    $(document).ready(function () {
       ...
       /* $('#paymentModal').on('change', '#amount_<?=$i?>', function (e) {
	    ...
	    $('#paymentModal').appendTo("body").modal('show', {backdrop: 'static', keyboard: false});        
            ...
            var customer_id = $thisObj.siblings('.customer-id_1').val(); 
        ...
        var customer_discount =  '<?php echo $this->Settings->customer_discount; ?>';
		...
		$.ajax({
			...
			success:function(data){
			    ...
			    if ("<?php echo $Settings->bbq_discount ?>" == "bbq"  || "<?php echo $Settings->customer_discount ?>" == "customer") {
			       console.log(data);                   
                ...
                if(bbq_discount == "bbq" ){
                    ...
                    $dropdown1 +='</select>';
                    if (data.cus_dis.customer_discount_id != 0) {
                        $msg1 = "<div id='discount-confirm-msg'>Do You want to apply \"<span id='discount-name'>"+data.bbq_dis.name+"</span>\"?</div>";
                    } else {
                        $msg1 = "<div id='discount-confirm-msg'>Do You want to apply any discount?</div>";
                    }
				    ...
				    bootbox.confirm({
					message: $dropdown+$msg+$dropdown1+$msg1,					
					 buttons: {
					     confirm: {
						 label: 'Apply',
					 ...
					 callback: function (result) {
                        ...
                        bbq_dis_id  = $('#bbq-discount').val();
						 $.ajax({
							 ...
							 requestBill(ordersplit);//payment_popup($thisObj);
						     }
						 });
					     }else{  requestBill(ordersplit);}
	    ...
	    var billid = $(this).siblings('.billid_1').val(); 
			...
			success:function(data){
			    ...
			    if (bbq_discount == "bbq"  ||  customer_discount == "customer") {
			       console.log(data);	
...
$(document).on('change', '.credit-max', function () {
    ...
    $(document).on('click', '.cancel_bill', function(e) {
        e.preventDefault();
        var cancel_id = $(this).siblings('.sales_split_id').val();
...
$(document).on('click','#cancel_orderitem',function(){
     ...
     var split_id = $('#sale_id').val(); 
     if($.trim(cancel_remarks) != ''){
        
        $.ajax({
            type: "get",
            url:"<?=admin_url('pos/consolidcancel_sale');?>",                
            data: {cancel_remarks: cancel_remarks, split_id: split_id},
            dataType: "json",
            success: function (data) {
                if(data.msg == 'success'){
