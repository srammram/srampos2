...
<script>
    $(document).ready(function () {
        oTable = $('#MaterialRequestData').dataTable({
            "aaSorting": [[2, "desc"], [3, "desc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?=lang('all')?>"]],
            "iDisplayLength": <?=$Settings->rows_per_page?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?=admin_url('material_request/getMaterialrequest' . ($warehouse_id ? '/' . $warehouse_id : ''))?>',
            ...
            "aoColumns": [null,{"bSortable": false,"mRender": checkbox}, {"mRender": fld}, null,null,null, {"mRender": currencyFormat},{"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat},   {"bSortable": false}],
...
<?php if ($Owner || $GP['bulk_actions']) {
	    echo admin_form_open('material_request/quotes_actions', 'id="action-form"');
	}
?>
<div class="box">
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?=admin_url('material_request/add')?>">
                                <i class="fa fa-plus-circle"></i> <?=lang('add_material_request')?>
                            </a>
                        </li>
                       <!--  <li>
                ...
                <?php if (!empty($warehouses)) {
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?=admin_url('material_request')?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            <li class="divider"></li>
                            <?php
                            	foreach ($warehouses as $warehouse) {
                            	        echo '<li ' . ($warehouse_id && $warehouse_id == $warehouse->id ? 'class="active"' : '') . '><a href="' . admin_url('material_request/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            	    }
                                ?>
                        </ul>
                    </li>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table id="MaterialRequestData" cellpadding="0" cellspacing="0" border="0"
