        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="well well-sm no-print">
                    ...
                    <?= admin_form_open("recipe/print_barcodes", 'id="barcode-print-form" data-toggle="validator"'); ?>
                    <div class="controls table-controls">
                        <table id="bcTable"
                               class="table items table-striped table-bordered table-condensed table-hover">
                            <thead>
                            <tr>
                                <th class="col-xs-4"><?= lang("recipe_name") . " (" . $this->lang->line("recipe_code") . ")"; ?></th>
                        ...
                        <div class="form-group">
                            ...
                            <input name="recipe_name" type="checkbox" id="recipe_name" value="1" checked="checked" style="display:inline-block;" />
                            <label for="recipe_name" class="padding05"><?= lang('recipe_name'); ?></label>
                            ...
                            <input name="recipe_image" type="checkbox" id="recipe_image" value="1" style="display:inline-block;" />
                            <label for="recipe_image" class="padding05"><?= lang('recipe_image'); ?></label>
                            ...
                            if (!empty($barcodes)) {
                                ...
                                foreach ($barcodes as $item) {
                                    for ($r = 1; $r <= $item['quantity']; $r++) {
                                        ...
                                        if($item['image']) {
                                            echo '<span class="recipe_image"><img src="'.base_url('assets/uploads/thumbs/'.$item['image']).'" alt="" /></span>';
                                        }
                                        if($item['site']) {
                                        ...
                                        echo '<span class="barcode_image"><img src="'.admin_url('recipe/barcode/'.$item['barcode'].'/'.$item['bcs'].'/'.$item['bcis']).'" alt="'.$item['barcode'].'" class="bcimg" /></span>';
                                        if ($style == 50) {
                                            echo '</div>';
                                        }
                                        echo '</div>';
                                ...
                                echo '<h3>'.lang('no_recipe_selected').'</h3>';
                            }
                        }
                    ?>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#add_item").autocomplete({
            source: '<?= admin_url('recipe/get_suggestions'); ?>',
            minLength: 1,
            autoFocus: false,
            delay: 250,
        ...
        $(document).on('ifChecked', '#recipe_name', function(event) {
            localStorage.setItem('bcrecipe_name', 1);
        });
        $(document).on('ifUnchecked', '#recipe_name', function(event) {
            localStorage.setItem('bcrecipe_name', 0);
        });
        if (recipe_name = localStorage.getItem('bcrecipe_name')) {
            if (recipe_name == 1)
                $('#recipe_name').iCheck('check');
            else
                $('#recipe_name').iCheck('uncheck');
        }
        ...
        $(document).on('ifChecked', '#recipe_image', function(event) {
            localStorage.setItem('bcrecipe_image', 1);
        });
        $(document).on('ifUnchecked', '#recipe_image', function(event) {
            localStorage.setItem('bcrecipe_image', 0);
        });
        if (recipe_image = localStorage.getItem('bcrecipe_image')) {
            if (recipe_image == 1)
                $('#recipe_image').iCheck('check');
            else
                $('#recipe_image').iCheck('uncheck');
        }
        ...
        $('#reset').click(function (e) {
            ...
            bootbox.confirm(lang.r_u_sure, function (result) {
                if (result) {
                    ...
                    if (localStorage.getItem('bcrecipe_name')) {
                        localStorage.removeItem('bcrecipe_name');
                    }
                    ...
                    $('#modal-loading').show();
                    window.location.replace("<?= admin_url('recipe/print_barcodes'); ?>");
                }
            });
        });
    ...
    function loadItems () {
        ...
        if (localStorage.getItem('bcitems')) {
            ...
            $.each(bcitems, function () {
                ...
                tr_html = '<td><input name="recipe[]" type="hidden" value="' + item.id + '"><span id="name_' + row_no + '">' + item.name + ' (' + item.code + ')</span></td>';
