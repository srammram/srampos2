 ...
 var oTable;
    $(document).ready(function () {
        oTable = $('#PRData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('recipe/getrecipe'.($warehouse_id ? '/'.$warehouse_id : '').($supplier ? '?supplier='.$supplier->id : '')) ?>',
		...
		var oSettings = oTable.fnSettings();
			 ...
			 "aoColumns": [
			 	null,{"bSortable": false, "mRender": checkbox}, null,null, null,null,null, {"bSortable": false, "mRender": recipe_img},  {"bSortable": false, "mRender": recipe_stock},null,null,  {"mRender": currencyFormat}, {"mRender": recipe_status},  {"bSortable": false}
			 ],
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-barcode"></i><?= lang('recipe') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')'.($supplier ? ' ('.lang('supplier').': '.($supplier->company && $supplier->company != '-' ? $supplier->company : $supplier->name).')' : ''); ?>
        </h2>
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?= admin_url('recipe/add') ?>">
                                <i class="fa fa-plus-circle"></i> <?= lang('add_recipe') ?>
                            </a>
                        </li>
                        <?php if(!$warehouse_id) { ?>
                        <!--<li>
