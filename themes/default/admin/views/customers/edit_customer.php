...
<div class="modal-dialog modal-lg">
    <div class="modal-content close_pop">
       ...
       // echo admin_form_open_multipart("customers/add_pos", $attrib); ?>
        <form data-url="<?=admin_url('customers/edit_customer/'. $customer->id)?>" data-toggle="validator" role="form"  method="post" accept-charset="utf-8" novalidate class="bv-form" id="edit-customer-form">
        <div class="modal-body1">
          ...
          /*  echo form_dropdown('supplier_type', $types, $customer->supplier_type, '1', 'class="form-control select" id="supplier_type" required="required"');*/
           echo form_dropdown('supplier_type', $types, $customer->supplier_type, 'class="form-control select" id="supplier_type" style="width:100%;" ');
             ?>
                </div>
                    <div class="form-group">
                    <label class="control-label" for="customer_group"><?php echo $this->lang->line("customer_group"); ?></label>
                        ...
                        echo form_dropdown('customer_group', $cgs, $customer->customer_group, 'class="form-control select" id="customer_group" style="width:100%;" required="required"');
                        ?>
                    </div>
            ...
            <div class="form-group person">
                        <?= lang("name", "name"); ?>
                        <?php echo form_input('name', $customer->name, 'class="form-control tip alphabetsOnly" id="name" data-bv-notempty="true"'); ?>
                       <!--  <span id='alp' style="color: #a94442;">Alphabets Only Type</span> -->
                    </div>
                    <div class="form-group">
                        <?= lang("phone", "phone"); ?>
                        <input type="tel" name="phone" class="form-control numberonly"  maxlength="10" id="phone"   value="<?= $customer->phone ?>"/>
                    </div>
            ...
            <option value="0" <?php if($customer->allow_loyalty==0) {echo 'selected="selected"';}?>><?=lang('No')?></option>
            <option value="1" <?php if($customer->allow_loyalty==1) {echo 'selected="selected"';}?>><?=lang('Yes')?></option>
             </select>
                    </div>
        ...
        <div class="modal-footer">
            ...
            <input type="hidden" name="edit_customer" value="<?=lang('edit_customer')?>">
