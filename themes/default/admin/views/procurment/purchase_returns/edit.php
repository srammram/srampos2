    ...
    <div class="box-content">
        <div class="row">
            ...
            <div class="col-lg-12">
                ...
                <?php
                $attrib = array('data-toggle' => 'validator1', 'role' => 'form','id'=>'add-purchase-return');
                echo admin_form_open_multipart("procurment/purchase_returns/edit/".$inv->id, $attrib)
                ?>
                <div class="row">
						...
						<i class="fa fa-2x fa-plus-square" id="addIcon1"></i>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                           $po[''] = '';
                                            foreach ($purchaseinvoice as $purchaseinvoice_row) {
                        ...
                        <input type="hidden" name="warehouse" id="pi_warehouse" value="<?php echo $Settings->default_warehouse ?>">  
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <?= lang("status", "pi_status"); ?>
                                    <?php $st['process'] = lang('process');
								if($this->siteprocurment->GETaccessModules('purchase_return_approved')){
									$st['approved'] = lang('approved');	
								}
                    ...
                    <table class="table custom_tables">
                            <tbody>
                                <tr>                                    
                                    ...
                                    <td width="150px">
                                        <input  name="logged_by" id="logged_by" readonly class="form-control" value="<?= $this->session->userdata('username') ?>">
                                    </td>
                                     <td width="100px">                                    
                                    </td>
                                    <td width="100px">                                    
...
$(document).ready(function(e) {
	...
	if(supplierid != ''){
		$.ajax({
			type: 'get',
			url: '<?= admin_url('procurment/purchase_returns/supplier'); ?>',
			dataType: "json",
...
$(document).on('change', '#pi_requestnumber', function(){
	...
	$.ajax({
		type: 'get',
		url: '<?= admin_url('procurment/purchase_returns/purchase_invoices_list'); ?>',
		dataType: "json",
		data: {
		...
		success: function (data) {
			...
			$(this).removeClass('ui-autocomplete-loading');
			if (data.value['purchase_invoicesitem']!=null) {
			   var items = JSON.stringify(data.value['purchase_invoicesitem']);
			}else{
			    bootbox.alert('<?= lang('no_product_found_for_this_store') ?>', function () {
				    $('#pi_requestnumber').focus();
			    });
