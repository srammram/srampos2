...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#purchase_invoicesTable').dataTable({
            ...
            'sAjaxSource': '<?=admin_url('procurment/purchase_returns/getPurchase_returns' . ($warehouse_id ? '/' . $warehouse_id : ''))?>',
            ...
            "aoColumns": [null,{"bSortable": false,"mRender": checkbox}, {"mRender": fld}, null,null, null, {"mRender": currencyFormat},{"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat},  {"mRender": row_status},{"bSortable": false,"mRender": procurment_attachment},  {"bSortable": false}],
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
                var oSettings = oTable.fnSettings();
                nRow.id = aData[0];
                nRow.className = "purchase_invoices_link";
                return nRow;
            },
...
<?php if ($Owner || $GP['bulk_actions']) {
	    echo admin_form_open('purchase_invoices/purchase_invoices_actions', 'id="action-form"');
	}
?>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-star"></i><?=lang('purchase_returns') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')';?>
        </h2>
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?=admin_url('procurment/purchase_returns/add')?>">
                                <i class="fa fa-plus-circle"></i> <?=lang('add_purchase_return')?>
                            </a>
                        </li>
                        <!-- <li>
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            ...
                            <?php
                            	foreach ($warehouses as $warehouse) {
                            	        echo '<li ' . ($warehouse_id && $warehouse_id == $warehouse->id ? 'class="active"' : '') . '><a href="' . admin_url('procurment/purchase_invoices/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            	    }
                                ?>
                        </ul>
                    </li>-->
