            ...
            <div class="well">
                <table class="table table-borderless" style="margin-bottom:0;">
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <strong class="text-right"><?php echo $this->sma->formatMoney($payment->total_pay); ?></strong>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <strong class="text-right"><?php echo $this->sma->formatMoney($payment->balance); ?></strong>
                        </td>
                    </tr>
                    <?php endif; ?>
                    <tr>
                        <td><strong><?= lang("paid_by"); ?></strong></td>
                        <td class="text-right"><strong class="text-right">
                        <?php
                        $paid_by = '';
                            if($payment->Cash!=0) $paid_by .= lang('Cash').' | ';                            
                            if($payment->amt_exchange!=0) $paid_by .= lang('Amount Exchnage').' | ';
                            if($payment->CC!=0) {
                                $paid_by .= lang('Credit Card');
                                if($payment->cc_no!='') $paid_by .= ' (' . substr($payment->cc_no, -4) . ')';
                                $paid_by .=' | ';
                            }
                        
                        echo trim($paid_by,' | ');
                         //if ($payment->paid_by == 'gift_card' || $payment->paid_by == 'CC' || $payment->paid_by == 'ppp' || $payment->paid_by == 'stripe' || $payment->paid_by == 'authorize') {
                                if ($payment->paid_by == 'gift_card' || $payment->paid_by == 'ppp' || $payment->paid_by == 'stripe' || $payment->paid_by == 'authorize') {
