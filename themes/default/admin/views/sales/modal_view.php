            ...
            <div class="well well-sm">
                <div class="row bold">
                    ...
                    <p class="bold">
                        ...
                        <?= lang("sale_status"); ?>: <?= lang($inv->payment_status); ?><br>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped print-table order-table">
                    ...
                    <tr>
                        ...
                        <?php
                        if ($Settings->tax1 && $inv->total_tax > 0) {
                            echo '<th>' . lang("tax") . '</th>';
                        }
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="vertical-align:middle;">
                                <?= $row->recipe_code.' - '.$row->recipe_name . ($row->variant ? ' (' . $row->variant . ')' : ''); ?>
                            ...
                            <?php } ?>
                            <td style="width: 80px; text-align:center; vertical-align:middle;"><?= $this->sma->formatQuantity($row->quantity).' '.$row->unit_price; ?></td>
                    ...
                    if ($Settings->product_discount && $inv->total_discount != 0 && $Settings->tax1 && $inv->total_tax > 0) {
                        $tcol = $col - 2;
                    } elseif ($Settings->product_discount && $inv->total_discount != 0) {
                        $tcol = $col - 1;
                    } elseif ($Settings->tax1 && $inv->total_tax > 0) {
                        $tcol = $col - 1;
                    } else {
                        $tcol = $col;
                    }
                    ...
                    <?php if ($inv->grand_total != $inv->total) { 
                        ...
                        <tr>
                            ...
                            <?php
                            if ($Settings->tax1 && $inv->total_tax > 0) {
                                echo '<td style="text-align:right;">' . $this->sma->formatMoney($return_sale ? ($inv->subtotal+$return_sale->product_tax) : $inv->total_tax) . '</td>';
                            }
                            if ($Settings->product_discount && $inv->total_discount != 0) {
                                echo '<td style="text-align:right;">' . $this->sma->formatMoney($return_sale ? ($inv->total_discount+$return_sale->product_discount) : $inv->total_discount) . '</td>';
                            }
                            ?>
                            <td style="text-align:right; padding-right:10px;"><?= $this->sma->formatMoney($return_sale ? (($inv->total + $inv->total_tax)+($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->total_tax)); ?></td>
                        </tr>
                    <?php } ?>
                    <?php
                    if ($return_sale) {
