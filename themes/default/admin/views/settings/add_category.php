      ...
      $('#add-category')
	    ...
	    $.ajax({
		    ...
		    contentType: false,
			    ...
			    if ($isSub==0 && $page=='add_sale') {
				
			    
			    $("#myModal .close").trigger('click');
			    $category_id = data.category.id;
			    $category_name = data.category.name;
			    $newCategory = '<li class="level-1-menu-li">'+
					    '<div class="level-1-menu-div">'+
						'<input type="hidden" name="" value="'+$category_id+'">'+
						'<div class="category-name-container">'+
						    '<input type="checkbox" name="" value="'+$category_id+'" class="recipe-group new-recipe-group" data-index="0">'+
						    '<label for="pos-door_delivery_bils" class="category-name padding05">'+$category_name+'</label>'+
						    '<span class="subgroup_hide_show"><i class="fa fa-plus-circle" aria-hidden="true"></i></span>'+
						'</div>'+					
					    ...
					    '</div>'+
					'</li>';
			    $('.recipe-group-list .level-1-menu').append($newCategory);
			    $('.new-recipe-group').not('.skip').iCheck({
			    ...
			    return false;
			    }if ($isSub==1 && $page=='add_sale') {
				
			    
			    $("#myModal .close").trigger('click');
			    $category_id = data.category.id;
			    $category_name = data.category.name;
			    $parentCat = '<?=$_GET['parent']?>';
			    $newSubCategory = '<li class="level-2-menu-li">'+
						'<div class="subgroup-strip">'+
						    '<input type="hidden" name="" value="'+$category_id+'">'+
						    '<input type="checkbox" name="" value="'+$category_id+'" class="recipe-subgroup new-recipe-subgroup" data-index="0">'+
							'<label for="pos-door_delivery_bils" class="subgroup-name padding05">'+$category_name+'</label>'+
							'<span class="recipe_hide_show"><i class="fa fa-plus-circle fa-minus-circle" aria-hidden="true"></i></span>'+ 
						'</div>'+
						'<ul class="level-3-menu">'+
						    '<div class="items-title">brands</div>'+
						'</ul>';
			    $('.parent-cate-'+$parentCat+'.level-2-menu').append($newSubCategory);
			    $('.new-recipe-subgroup').not('.skip').iCheck({
				checkboxClass: 'icheckbox_square-blue',
				radioClass: 'iradio_square-blue',
				increaseArea: '20%'
			    });
...
<div class="modal-dialog">
    ...
    cursor: pointer;">
            ...
            <div class="form-group">
                ...
                echo form_dropdown('parent', $cat, (isset($_GET['parent']) ? $_GET['parent'] : ''), 'class="form-control select select-parent-category" id="parent" style="width:100%"')
                ?>
            </div>
