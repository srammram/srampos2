...
<div class="box">
    ...
    <div class="box-content">
        <div class="row">
            ...
            // echo admin_form_open("reports/daywise");?>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.daywise', function () {
            $url = '<?=admin_url('reports/get_DaySummaryreports');?>';
            GetData($url);
        });
            ...
            if (start_date !='') {
                 $('#start_date').css('border-color', '#ccc'); 
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {start_date: start_date,warehouse_id: warehouse_id,pagelimit:pagelimit},
                        dataType: "json",
                        cache: false,
                        success: function (data) {
                            $('#DAYData').empty();
...
//            'sAjaxSource': '<?= admin_url('reports/get_DaySummaryreports/?v=1'.$v) ?>',
