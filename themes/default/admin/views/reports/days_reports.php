...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('reports/get_daysreports/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/get_daysreports/0/xls/?v=1'.$v)?>";
            return false;
        });
        ...
        $(document).on('click', '.report_settlement', function () {
            $url = '<?=admin_url('reports/get_daysreports');?>';
            GetData($url);
        });
            ...
            if (start_date !='' && end_date !='' ) {
                 $('#start_date,#end_date').css('border-color', '#ccc'); 
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {start_date: start_date, end_date: end_date, warehouse_id: warehouse_id, day: day, defalut_currency:defalut_currency,pagelimit:pagelimit},
                        dataType: "json",
                        success: function (data) {
                             $('#SlRData > tbody').empty();
