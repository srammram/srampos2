    ...
    if (start !='' && end !='') {                  
        ...
        $('#kot').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {start: start, end: end, bill_no : bill_no, warehouse_id : warehouse_id,varient_id: varient_id,pagelimit:pagelimit,table_whitelisted:table_whitelisted},
                        dataType: "json",
                        success: function (data) {
                            ...
                            $('#BillDetailsData > tbody').append('<tr><td colspan="5" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                                ...
                                {   console.log(b)
                                    ...
                                    $table_whitelisted = (d.table_whitelisted==1)?'table_whitelisted':'';
                                     ...
                                     if(d.tax_type ==0)
                                     {   
                                       bill_amt += parseFloat(d.Bill_amt)-parseFloat(d.item_discount)-parseFloat(d.off_discount)-parseFloat(d.input_discount)+parseFloat(d.service_charge_amount);
                                       billamt = parseFloat(d.Bill_amt)-parseFloat(d.item_discount)-parseFloat(d.off_discount)-parseFloat(d.input_discount)+parseFloat(d.service_charge_amount);       
                                     } 
                                     else
                                     {
                                        bill_amt += parseFloat(d.Bill_amt)-parseFloat(d.item_discount)-parseFloat(d.off_discount)-parseFloat(d.input_discount)+parseFloat(d.tax)+parseFloat(d.service_charge_amount);

                                        billamt = parseFloat(d.Bill_amt)-parseFloat(d.item_discount)-parseFloat(d.off_discount)-parseFloat(d.input_discount)+parseFloat(d.tax)+parseFloat(d.service_charge_amount); 
                                     }
                                        $('#BillDetailsData > tbody').append('<tr class="text-center '+$table_whitelisted+'"><td>'+d.bill_time+'</td><td>'+d.bill_date+'</td><td>'+d.table_name+'</td><td>'+d.branch+'</td><td>'+d.bill_number+'</td><td>'+d.kitchenno+'</td><td>'+d.item+'</td><td>'+d.variant+'</td><td >'+formatQuantity(d.quantity)+'</td><td class="text-right">'+formatMoney(billamt)+'</td></tr>');                 
                                    });
                                    $('#BillDetailsData > tbody').append('<tr class="'+$table_whitelisted+'" style="font-weight:bold"><td>Bill Total</td><td colspan="7"></td><td  class="text-right">'+formatQuantity(qty)+'</td><td  class="text-right">'+formatMoney(bill_amt)+'</td></tr>');
                                });

                                $('#BillDetailsData > tbody').append('<tr style="font-weight:bold"><td>Grand Total</td><td colspan="7" ></td><td class="text-right">'+formatQuantity(grand_qty)+'</td><td class="text-right">'+formatMoney(grand_total)+'</td></tr>');
                           } 
                        }
                    });
    }
