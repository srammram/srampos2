...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.popular_analy', function () {
            $url = '<?=admin_url('reports/get_popular_reports');?>';
            GetData($url);
        });
    ...
    if (start_date !='' && end_date !='' && popular != '') {                  
       ...
       $('#popular').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {start_date: start_date, end_date: end_date, popular:popular, warehouse_id:warehouse_id,pagelimit:pagelimit},
                        dataType: "json",
                        success: function (data) {
                            $('#PopularData > tbody').empty();
                            if(data.popular_non_popular =='empty' || data.popular_non_popular == 'error'){ 
                                 
                                $('#PopularData > tbody').append('<tr><td colspan="6" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');
                            }
                            else{
                                        ...
                                        {  
                                            ...
                                            var sub_total = 0;
                                             ...
                                             {  
                                               ...
                                               sub_total += (parseFloat(f.subtotal) - (parseFloat(f.item_discount) + parseFloat(f.off_discount) + parseFloat(f.input_discount))+(parseFloat(f.service_charge_amount))+(parseFloat(f.tax)));
                                             /*SUB END*/
                                             ...
                                             grp_total +=(parseFloat(f.subtotal) - (parseFloat(f.item_discount) + parseFloat(f.off_discount) + parseFloat(f.input_discount))+(parseFloat(f.service_charge_amount))+(parseFloat(f.tax)));
                                            /*GROUP END*/
                                          ...
                                          grand_rate += (parseFloat(f.subtotal) - (parseFloat(f.item_discount) + parseFloat(f.off_discount) + parseFloat(f.input_discount)));
                                            ...
                                            grand_discount += parseFloat(f.item_discount)+parseFloat(f.off_discount)+ parseFloat(f.input_discount);
                                               ...
                                               grand_total += (parseFloat(f.subtotal) - (parseFloat(f.item_discount) + parseFloat(f.off_discount) + parseFloat(f.input_discount))+(parseFloat(f.service_charge_amount))+(parseFloat(f.tax)));
                                             /*GRAND END*/
                                             ...
                                             /*
                                               var rate = (parseFloat(f.subtotal) - (parseFloat(f.item_discount) + parseFloat(f.off_discount) + parseFloat(f.input_discount))+(parseFloat(f.tax)));*/
                                                ...
                                                $('#PopularData > tbody').append('<tr  class="text-right"><td class="text-center">'+f.name+'</td><td class="text-center">'+f.variant+'</td><td>'+formatQuantity(f.quantity)+'</td><td>'+formatMoney(f.subtotal)+'</td><td>'+formatMoney(dis)+'</td><td  style="display:<?php echo $display; ?>" >'+formatMoney(f.service_charge_amount)+'</td><td>'+formatMoney(f.tax)+'</td><td>'+formatMoney(rate)+'</td></tr>');
                                            }); 
                                             $('#PopularData > tbody').append('<tr style="font-weight:bold" class="text-right"><td class="text-left">Sub Total:</td><td>&nbsp;</td><td>'+formatQuantity(sub_qty)+'</td><td>'+formatMoney(sub_rate+sub_discount)+'</td><td>'+formatMoney(sub_discount)+'</td><td style="display:<?php echo $display; ?>" >'+formatMoney(sub_service_charge)+'</td><td>'+formatMoney(sub_tax)+'</td><td>'+formatMoney(sub_total)+'</td></tr>');
                                       });

                                        $('#PopularData > tbody').append('<tr style="font-weight:bold"  class="text-right"><td class="text-left">Group Total:</td><td>&nbsp;</td><td>'+formatQuantity(grp_qty)+'</td><td>'+formatMoney(grp_rate+grp_discount)+'</td><td>'+formatMoney(grp_discount)+'</td><td style="display:<?php echo $display; ?>" >'+formatMoney(grp_service_charge)+'</td><td>'+formatMoney(grp_tax)+'</td><td>'+formatMoney(grp_total)+'</td></tr>');
                                });
                                /* $('#PopularData > tbody').append('<tr style="font-weight:bold"  class="text-right"><td class="text-left" colspan="4">Round :</td><td>'+formatMoney(round)+'</td></tr>');*/
                                  ...
                                  $('#PopularData > tbody').append('<tr style="font-weight:bold"  class="text-right"><td class="text-left">Grand Total:</td><td>&nbsp;</td><td>'+formatQuantity(grand_qty)+'</td><td>'+formatMoney(grand_rate+grand_discount)+'</td><td>'+formatMoney(grand_discount)+'</td><td>'+formatMoney(grand_service_charge)+'</td><td>'+formatMoney(grand_tax)+'</td><td>'+formatMoney(grand_total)+'</td></tr>');
                            }
                        }
                    });
    }
