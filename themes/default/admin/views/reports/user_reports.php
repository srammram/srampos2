...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.user_report', function () {
            $offset = false;
            $url = '<?=admin_url('reports/get_user_reports');?>';
            GetData($url);
        });
        ...
        $('#user').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $('#USERData > thead').append('<tr><th><?= lang("s.no"); ?><th><?= lang("date"); ?></th><th><?= lang("user"); ?></th><th><?= lang("order_type"); ?></th><th><?= lang("table_name"); ?></th><th><?= lang("bill_no"); ?></th><th><?= lang("grand_total"); ?></th></tr>'); 
                            ...
                            $('#USERData > tbody').append('<tr><td colspan="6" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                                    ...
                                    $.each(data.user_report, function (a,b) 
                                    {
                                        if(b.tax_type !=0){
                                          ...
                                          grand_total = parseFloat(b.total)-parseFloat(b.total_discount)+parseFloat(b.total_tax)+parseFloat(b.service_charge_amount);

                                          amt += parseFloat(b.total)-parseFloat(b.total_discount)+parseFloat(b.total_tax)+parseFloat(b.service_charge_amount);
                                        }
                                        else{
                                            grand_total =parseFloat(b.total)-parseFloat(b.total_discount)+parseFloat(b.service_charge_amount);

                                            amt += parseFloat(b.total)-parseFloat(b.total_discount)
                                            +parseFloat(b.service_charge_amount);
                                        }
                                                                          
                                        $('#USERData > tbody').append('<tr class="text-center"><td>'+$row_index+'</td><td >'+b.billdate+'</td><td >'+b.username+'</td><td>'+b.order_type+'</td><td>'+b.table_name+'</td><td>'+b.bill_number+'</td><td  class="text-right">'+formatMoney(grand_total)+'</td></tr>');                 
                                        $row_index++;
                                    });

                                     $('#USERData > tbody').append('<tr style="font-weight:bold"><td ><strong>Grand Total:</strong></td><td colspan="6"   class="text-right">'+formatMoney(amt)+'</td></tr>');
                           } 
                        }
                    });
    }
