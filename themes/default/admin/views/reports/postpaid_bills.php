...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.postpaid_bills_report', function () {
            $url = '<?=admin_url('reports/postpaid_bills_report');?>';
            GetData($url);
        });
                                 ...
                                 $('#MonthlyData').append('<tbody><tr><td colspan="6" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr></tbody>');
                                   ...
                                   $url = '<?=admin_url()?>';
                                    ...
                                    $.each(data.postpaid_bills,function(n,v){
                                        ...
                                        $tbody += '<td><a href="'+$url+'reports/customer_postpaid_bills/'+v.customer_id+'" ><?=lang('bill_details')?></a></td>';
                                        if(v.amount!=0){
                                            $tbody += '<td><a style="text-decoration: none" href="'+$url+'reports/postpaid_payment/'+v.customer_id+'" data-toggle="modal" data-target="#myModal"><div class="text-center"><span class="payment_status label label-warning"><?=lang('make_payment')?></span></div></a></td>';
