...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.home_delivery', function () {
            $offset = false;
            $url = '<?=admin_url('reports/get_take_away_reports');?>';
            GetData($url);
        });
    ...
    if (start !='' && end !='') {                  
        ...
        $('#kot').siblings(".select2-container").find('.select2-choice').css('border-color', '#ccc');
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {start: start, end: end, warehouse_id: warehouse_id,pagelimit:pagelimit},
                        dataType: "json",
                        success: function (data) {
                            ...
                            $('#HomeDelieryData > tbody').append('<tr><td colspan="5" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                                    ...
                                    {   
                                       ...
                                       round = parseFloat(b.round);      
                                        $('#HomeDelieryData > tbody').append('<tr class="text-center"><td>'+$row_index+'</td><td >'+b.Orderdate+'</td><td >'+b.Bill_No+'</td><td >'+b.branch+'</td><td>'+b.name+'</td><td>'+paid_by(b.paid_by)+'</td><td class="text-right">'+formatMoney(b.paid)+'</td></tr>');
                                        }    
                                        else{
