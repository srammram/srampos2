...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('bill_reprint_report'); ?> <?php
            if ($this->input->post('start_date')) {
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.bill_details', function () {
            $url = '<?=admin_url('reports/get_bill_reprint_reports');?>';
            GetData($url);
        });
                  ...
                  $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $('#BillDetailsData > tbody').append('<tr><td colspan="5" class="dataTables_empty"><?= lang('sEmptyTable') ?></td></tr>');    
                                ...
                                $.each(data.reports, function (a,d) 
                                {
                                    
                                                             
                                    bill_total += parseFloat(d.bill_total);
                                    round = parseFloat(d.bill_total); 
                                    console.log('gt'+bill_total)
                                    var bill_amt = 0;    
                                    var billamt = 0;
                                    var qty = 0;                  
                                    ...
                                    $table_whitelisted = (d.table_whitelisted==1)?'table_whitelisted':'';
                                        ...
                                        $('#BillDetailsData > tbody').append('<tr class="text-center '+$table_whitelisted+'"><td>'+d.bill_time+'</td><td>'+d.bill_date+'</td><td>'+d.table_name+'</td><td>'+d.branch+'</td><td>'+d.bill_number+'</td><td >'+d.print_count+'</td><td class="text-right">'+formatMoney(d.bill_total)+'</td></tr>');                 
                                    
                                    
                                });

                                $('#BillDetailsData > tbody').append('<tr style="font-weight:bold"><td colspan="6" style="text-align:right;">Grand Total</td><td style="text-align:right;">'+formatMoney(bill_total)+'</td></tr>');
                           } 
                        }
                    });
    }
