...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('NC_Kot_report'); ?> <?php
            if ($this->input->post('start_date')) {
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.report_settlement', function () {
            $url = '<?=admin_url('reports/get_ncKotreports');?>';
            GetData($url);
        });
                                ...
                                $.each(data.settlements, function (a,b) 
                                {
                                    ...
                                    var $row_index = 1;
                                     $.each(b.user, function (c,d) {  
										...
										$nc_details=JSON.parse( d.nc_kot_details );
										$ncKot_details='';
										jQuery.each($nc_details, function(index, item) {
                                          $ncKot_details +=item.type_name+":"+item.details+'<br>';
});
                                        /*usd += parseFloat(d.USD);*/
                                        //amt += (parseFloat(d.Credit_Card) + parseFloat(d.Cash) + parseFloat(d.For_Ex) -parseFloat(d.return_balance));
										amt += (parseFloat(d.grand_total));
                                        /*total_usd += parseFloat(d.USD);*/
                                        total_amt += (parseFloat(d.grand_total));
                                        ...
                                        $('#SlRData > tbody').append('<tr class="text-right"><td>'+$row_index+'</td><td class="text-center">'+d.bill_date+'</td><td class="text-center">'+d.table_name+'</td><td class="text-center">'+d.seats+'</td><td class="text-center">'+d.username+'</td><td class="text-center">'+d.warehouse+'</td><td class="text-center">'+d.Bill_No+'</td><td class="text-center">'+d.bill_type+'</td><td class="text-center">'+d.bill_time+'</td><td>'+d.nc_kot_type_name+'</td><td>'+$ncKot_details+'</td><td>'+formatMoney((parseFloat(d.grand_total)))+'</td></tr>');                     $row_index++;
                                     });  
                                      $('#SlRData > tbody').append('<tr style="font-weight:bold" class="text-right"><td colspan="9" class="text-center">User Total: </td><td></td><td></td><td>'+formatMoney(amt)+'</td></tr>');             
                                });

                                 $('#SlRData > tbody').append('<tr style="font-weight:bold" class="text-right"><td colspan="9" class="text-center">Grand Total: </td><td></td><td></td><td>'+formatMoney(total_amt)+'</td></tr>');
                            }
                        }
                    });  
            }
...
$(".excel_report").click(function(){
      ...
      $("#SlRData1").table2excel({
        ...
        filename: "NC Kot Reports  " //do not include extension
