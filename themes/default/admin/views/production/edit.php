...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('Edit Production'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo admin_form_open_multipart("production/edit/".$production->id, $attrib)
                ?>
                ...
                <div class="col-md-4">
                 ...
                 <div class="col-md-4">   
                   <div class="form-group">
                        ...
                        <?= form_input('production_date', (isset($_POST['production_date']) ? $_POST['production_date'] : $production->production_date), 'class="form-control tip totay_date" id="production_date" required="required"') ?>
                    </div>
                ...
                <table id="productionTable"  class="table items  table-bordered table-condensed sortable_table" style="background:#fff">
                 ...
                 <div class="col-md-12">
					...
					<?php if($production->production == ''){ echo form_submit('add_purchases_order', $this->lang->line("submit"), 'id="add_purchases_order" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"');  } ?>
    ...
    $(document).ready(function () {
        ...
        function add_production_item(item) {
			if (item == null) {
             ...
             item_id = item.id;
			 if (pod_items[item_id]) {
				 
			 }else{
			 	pod_items[item_id] = item;       
			 }
			localStorage.setItem('pod_items', JSON.stringify(pod_items));
    		loadItems();
            return true;
        }
		...
		function loadItems() {
			...
			if (localStorage.getItem('pod_items')) {
				pod_items = JSON.parse(localStorage.getItem('pod_items'));
				 $("#productionTable tbody").empty();
				$.each(pod_items, function () {
				...
				tr_html += '<td><input name="product_qoh[]" id="product_qoh_' + row_no + '" type="hidden" class="form-control" value="'+this.qoh+'"><span id="qoh_' + row_no + '">' + this.qoh + '</span></td>';			
		...
		localStorage.setItem('pod_items', JSON.stringify(<?=$production_item;?>));
		<?php
		}
		?>
		if (localStorage.getItem('pod_items')) {
		...
		$(document).on('change', '.given_quantity', function () {
			...
			var qoh = $('#product_qoh_'+item_id).val();
			if(parseInt(qoh) >= $(this).val()){
				pod_items[item_id].given_quantity = $(this).val();
				localStorage.setItem('pod_items', JSON.stringify(pod_items));
			}else{
				bootbox.alert('<?= lang('your_quantity_value_large') ?>');
				$('#given_quantity_'+item_id).val('');
			}
		});
		$(document).on('change', '.given_unit', function () {
			var item_id = $(this).closest('tr').attr('data-item-id');
			pod_items[item_id].given_units = $(this).val();
			localStorage.setItem('pod_items', JSON.stringify(pod_items));
		});
		...
		$('#reset').click(function (e) {
			bootbox.confirm(lang.r_u_sure, function (result) {
				if (result) {
					if (localStorage.getItem('pod_items')) {
						localStorage.removeItem('pod_items');
					}
