...
<link rel="stylesheet" href="<?=$assets?>styles/jquery-ui.css" type="text/css">
...
<style>
	...
	.reprint_bill_list tbody tr td:last-child{text-align: right;padding-right: 19px;}
	#req_1{position: absolute;top: 5px;right:56%;}
	.print_bill{position: absolute;}
	.payment-list-container{position: relative;}
	.date_div{padding-left:0px;}
	#consolidation-reprint-data{background-color: #810819;border-color: #000;}
...
<body>
	...
	<section class="drop_down_list">
		<div class="container">
		  ...
		  <?php echo form_open("pos/pos/reprint", $attrib);?>
			<div class="row">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 drop_down_list_s" style="padding: 0px;">
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
						<div class="form-group">
							<label for="" class="col-sm-5">Date</label>
							<div class="col-sm-7">
								<input type="text" name="from_date"  class="form-control" placeholder="Date " id="from_date" value="<?php  echo date('Y-m-d',strtotime($date));  ?>">
							</div>
						</div>
					</div>
					<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
						<div class="form-group">
							<label for="" class="col-sm-5">Bill Number</label>
							<div class="col-sm-7">
								<input type="text" name="bill_no" value="" class="form-control " placeholder="Bill Number"  autocomplete="off" >
							</div>
						</div>
					</div>
						<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
						<div class="form-group">
						  <label for="method" class="col-sm-5"><?php echo $this->lang->line("type"); ?></label>
						  	<div class="col-sm-7">
								<select class="form-control" name="type" style="height:37px;" id="type">
					...
					<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
						 <input type="submit" class="tablebtn pull-right" value="Submit">
					</div>
				</div>
			</div>
				<?php   echo form_close();  ?>
    ...
    <div class="form-group  col-lg-2 date_div" style="margin-top: 30px;">  
			...
			<table class="table reprint_bill_list">
        <?php
        if(!empty($sales)){ ?>
        <ul>
            <?php
          ...
          <!--   <div style="clear:both; height:5px;"></div>
                     ...
                     <?php if(!empty($sales_row->bils)){
                        ...
                        foreach($sales_row->bils as $split_order){
							...
							<td style="color:#fff;font-size:18px;">
							 <?=lang('bill_no')?>: <?php echo $split_order->bill_number; ?>
							</td>
							<td>
				...
				<?php }      
                				...
                				<input type="hidden"  class="customer-name" value="<?php echo $split_order->customer; ?>">
                               ...
                               <button type="button" class="tablebtn request_bil" 
								...
								<?php   if($this->sma->actionPermissions('resettlement')){  ?>
								  <button type="button" class="tablebtn request_bil_new" 
                                 data-bil="req_<?=$k; ?>"id="" >
                                <i class="fa fa-money" ></i><?=lang('Repayment');?> 
                                </button>
								<?php   }  ?>
								   <input type="hidden"  class="billid" value="<?php echo $split_order->id; ?>">
                                <input type="hidden"  class="order_split" value="<?php echo $sales_row->sales_split_id; ?>">
    ...
    <div class="modal-dialog modal-md">
		...
		<h4 class="modal-title" id="payModalLabel"><?=lang('make_payment');?>(<span id="new_customer_name"></span>)</h4>
            ...
            <div class="modal-body" id="payment_content" >
             ...
             echo form_open("pos/pos/repaymant", $attrib);
    ...
    $(document).ready(function () {
       ...
       /* $('#paymentModal').on('select2-close', '#paid_by_<?=$i?>', function (e) {
		...
		$('#discount-name').text($('#choose-discount option:selected').text());
          ...
          $thisObj = $(this);
		  var type=<?php  echo $reprint_type ;  ?>;
          ...
          var url = '<?php echo  base_url('pos/pos/reprint_view') ?>';
          window.location.href= url +'/?bill_id='+billid+'&type='+type;
		 ...
		 function requestBill(billid){
            ...
            if (billid != '') {
                $.ajax({
                    ...
                    url: '<?= base_url('pos/pos/reprint_view');?>',
                    dataType: "json",
                    data: {
                        billid: billid
                    },
	...
	//	$('.check_all').change(function(){
		...
		if($(this).is(':checked')){
		  ...
		  var type=<?php  echo $reprint_type ;  ?>;
            ...
            if(bill_ids!=''){
                var url = '<?php echo  base_url('pos/pos/multiple_reprint_view') ?>';
			
                window.location.href= url +'/?bill_id='+bill_ids+'&type='+type;   
            }else{
