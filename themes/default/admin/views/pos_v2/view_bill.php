        ...
        <div id="receiptData">
            ...
            <div id="receipt-datareceipt-data">
                <div class="text-center">
                    ...
                    echo '</h4>';
                    ?>
                    <h3 style="font-weight:bold;"><?=lang('RECEIPT');?></h3>                    
                </div>
                <?php
               ...
               if($pos_settings->order_no_display == 1){
				...
				// echo "Cashier :";echo $cashier->sales_associate . "<br>";
				 echo lang("cashier") . ": " . $cashier->sales_associate. "</br>";
               ...
               // echo "<pre>";
                ...
                <table class="table table-striped table-condensed" style="font-size:14px!important;">
                        ...
                        if($inv->manual_item_discount != 0){
                         ...
                         ?>
                            <th class="text-right"><?=lang($dis);?></th>
                        <?php } ?>
                        ...
                        foreach ($billi_tems as $bill) {
						   ...
						   $underline='';
                            ...
                            $itemaddonamt =0;
                             ...
                             if($bill->comment_price !=0) : 
                                    echo  '<span style="color: #1778dc;font-weight: bold;">Comments</span>';
                                    echo  '<br><span style="color: red;font-weight: bold;"> '.$bill->comment.' </span>';
                             endif;
                            echo '</td>';
                           ...
                           if($itemaddonamt != 0) : 
							...
							// echo '<td  class="no-border">'.$this.$addons->qty.'';
                             ...
                             if($bill->comment_price !=0) :                                     
                                    echo  '<br><span style="color: red;font-weight: bold;"> '.$this->sma->formatMoney($bill->comment_price).' </span>';
                             endif;

                            echo '</td>';


                            echo '<td class="no-border text-center">'.$bill->quantity.'';
							 if($itemaddonamt != 0) : 
							echo '<br><span style="color: red;text-align: right;">'.$this.$bill->quantity.'</span>';
							 endif;
							echo '</td>';
                            ...
                            if($inv->manual_item_discount != 0){
                                ...
                                echo '<td class="no-border text-right">'.floor($bill->manual_item_discount_per_val).'</td>';
                                    ...
                                    echo '<td class="no-border">'.$this->sma->formatQuantity($bill->manual_item_discount).'</td>';
                                }
                            }
                            
                            echo '<td " class="no-border text-right">'.$this->sma->formatMoney($bill->net_unit_price -$bill->manual_item_discount).'';
                       ...
                       <?php 
						...
						<?php
						 if ($pos_settings->discount_note_display_option==1) {      
                             if ($bill_items['discount'] != 0 || $manual_dis_itemwise !=0) {
                                if($this->settings->customer_discount != 'none' || $manual_dis_itemwise !=0){
                    ...
                    foreach ($payments as $payment) {
					  echo '<tr>';
                        if (($payment->paid_by == 'cash' || $payment->paid_by == 'credit' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
                            ...
                            echo '<td>' . lang("") . ' ' . lang(card) . '</td>';
                            echo '<td>' . lang("") . ' ' . $this->sma->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
                ...
                bootbox.prompt({
                    ...
                    callback: function (email) {
                        if (email != null) {
                            $.ajax({
                                type: "post",
                                url: "<?=  base_url('pos/pos/email_receipt') ?>",
		...
		?>
			bootbox.confirm('This customer has been eligible to issue a loyalty card. click yes to issue loyalty card.', function (result) {
				if (result) {
					...
					window.open("<?= base_url('pos/pos/loyalty');?>", '_blank');
				}
			});
		 <?php
		}
