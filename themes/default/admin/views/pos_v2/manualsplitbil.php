...
<style>
	...
	#submit-sale{background-color:#cf1f10;height: 34px;width: 100%;padding: 0px; border-color:#cf1f10; }
	.container {width: 1170px!important;}
	.drag_table_head {
    background-color: #380404;
    border-color:#380404;}
	.example_manual_single{background-color: #380404;}
	.left_dragtable{margin-top: 15%;border-bottom: none;}
	.draggable{border-top: none;border: 1px solid #380404;}
	.ttax2_old{font-size: 11px!important;}
	.drag_items div{color: #fff;font-weight: normal;font-size: 16px;border-right: 1px solid #380404;}
	.drag_items{border: 1px solid #380404;}
	.manual_click_btn  .draggable div{color: #fff;}
	.pos-variant-name1{     color: #e47345; position: relative;}
	#content{background-color: #6a0202;}
	.tableright {background-color: #6a0202;}
	#pos {background-color: #6a0202;}
	.leftdrag_table_item{background-color: transparent!important;}
	.leftdrag_table_item .draggable div{color: #fff;}
	.droporder{    border-right: 1px solid #380404; border-left: 1px solid #380404;}
</style>
</head>
<body>
        ...
        <div class="c1">
            <div class="pos">
                ...
                <div id="pos">
                    ...
                    echo form_open("pos/pos/billing?order_type=".$order_type."&bill_type=3&bils=".$bils."&table=".$table_id."&splits=".$split_id, $attrib);?>
                    ...
                    <div class="tableleft col-sm-3 col-xs-3">
                        <div id="left-middle">
                                ...
                                <div class="col-lg-12 col-sm-12 col-xs-12 leftdrag_table_item orders draggable " id="origin" style="min-height:200px; background:#fff;">
                                    <?php
                                           ...
                                           <?php }
                                            if(!empty($discount)){
                                                if($discount[2]== 'percentage_discount'){
                                                 ...
                                                 $dis = $price_value;
                                                 $price_total = $total_p/$item->quantity;
                                                }else{
                                                 $price_total = $item->subtotal/$item->quantity;
                                                }
                                    ...
                                    <div class="col-lg-12 col-sm-12 col-xs-12 draggable orders_item">
                                        ...
                                        <div class="item_id no_pad"><input type="hidden" name="split[<?=$k?>][recipe_id][]"  value="<?php echo $item->recipe_id; ?>" class="recipe-id"></div>
										  ...
										  <div class="item_variants no_pad"><input type="hidden" name="split[<?=$k;?>][recipe_variant][]" value="<?php echo (!empty($item->variant))? $item->variant: "";?>"></div>
										  <div class="item_addon_id no_pad">
										   <input type="hidden" name="split[<?php echo $k;?>][addon_id][]" value="<?php echo $item->addon_id;?>" class="addon_id">
										  </div>
										  <div class="item_addon_qty no_pad">
										  <input type="hidden" name="split[<?php echo $k;?>][addon_qty][]" value="<?php echo $item->addon_qty;?>" class="addon_qty">
										  </div>
                                        ...
                                        <input type="hidden" name="split[<?=$k?>][manual_item_discount][]"  value="<?php echo $item->manual_item_discount;?>" class ="form-control pos-input-tip manual_item_discount text-right" readonly>
                                        </div>

                                        <div class="rec_price3 item_quantity"><?php echo $item->quantity/$item->quantity;?><input type="hidden" name="split[<?=$k?>][quantity][]"  value="<?php echo $item->quantity/$item->quantity; ?>" class="recipe-quantity"></div>                                        
                    ...
                    ?>
                    <div class="tableright col-sm-5 col-xs-12 manual_click_btn split-form-<?=$i?>">
                            ...
                            <div class="col-lg-12 col-sm-12 col-xs-12 rightdrag_table_item<?=$i?> orders droporder" id="drop_<?php echo $i; ?>" style="min-height:150px;">
...
function myFunction(id) {
    ...
    $('#drop_'+splitid+' .item_recipe input').attr('name',$('.item_recipe input').attr('name').replace(/\[\d+](?!.*\[\d)/, '['+splitid+']'));
            ...
            $('#drop_'+splitid+' .item_id input').attr('name',$('.item_id input').attr('name').replace(/\[\d+](?!.*\[\d)/, '['+splitid+']'));
			 $('#drop_'+splitid+' .item_variants input').attr('name',$('.item_variants input').attr('name').replace(/\[\d+](?!.*\[\d)/, '['+splitid+']'));
			  
			  $('#drop_'+splitid+' .item_addon_id input').attr('name',$('.item_addon_id input').attr('name').replace(/\[\d+](?!.*\[\d)/, '['+splitid+']'));
			   $('#drop_'+splitid+' .item_addon_qty input').attr('name',$('.item_addon_qty input').attr('name').replace(/\[\d+](?!.*\[\d)/, '['+splitid+']'));
...
$(document).on('click', '#submit-sale', function () {
	var totalbils=<?php echo  $_GET['bils'];  ?>;
	for (var i = 1; i <=totalbils; i++) {
	var rightorderitem =$(".rightdrag_table_item"+i+" > div").length
	if(rightorderitem ==0){
		bootbox.alert('Split '+i+' Is Empty');
		return false;
	}
	}
	var leftorderitem =$(".leftdrag_table_item > div").length;
	if(leftorderitem ==0){
    $('#pos-manualsplitbill-form').submit();
  
	}else{
   bootbox.alert('Balance Item Found In Split');
	}
