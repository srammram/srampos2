<style>
	* {
	...
	table tr.tfoot_last_tr th{background: #000!important;color: #fff!important;border: none;border-bottom: 1px solid #ddd;}
</style>
<?php  $biller = $this->site->getCompanyOrderByID($bill_items['biller_id']);  ?>
    ...
    <div id="wrapper1">
		...
		<div class="text-center">
               ...
               if($this->Settings->time_format == 12){
                ...
                <br><?=lang("customer")?>: <?=$bill_items['customer_name']?>
				<?php
				 ...
				 echo "<br>";
				  ...
				  if(in_array(1,json_decode($pos_settings->nop_print))&& $pos_settings->number_of_people_print ==1){
					 echo "People :";echo !empty($seats_id)? $seats_id:$tableno->max_seats ;
					 echo "<br>";
				 }
            ...
            <?php } ?>
					...
					//	echo $get_free_item;
                            echo '<tr><td colspan="2" style="display: table-cell;text-align: -webkit-match-parent;text-decoration:'.$underline.'"  class="no-border">' . ($star).($discount_star)  . ($recipe_name) . ($recipe_variant ? ' ' . $recipe_variant . '' : ''); 
                       ...
                       /* if ($manual_discount_amount != 0) {
						...
						if ($dis != 0) {
                            echo '<tr><th colspan="'.$cols.'" class="text-right">' .$discounnames. '</th><th class="text-right">' . $this->sma->formatMoney($dis) . '</th></tr>';
							 echo '<tr><th colspan="'.$cols.'" class="text-right">Sub Total</th><th class="text-right">' . $this->sma->formatMoney($bill_items['total']-$dis) . '</th></tr>';
                        }
                        ...
                        if ($pos_settings->discount_note_display_option==1) {    
                             if ($bill_items['discount'] != 0 || $manual_itemwis_discount !=0) {
                                if($this->settings->customer_discount != 'customer' || $manual_itemwis_discount !=0){
				...
				<?php  if(in_array(2,json_decode($pos_settings->cus_print))&& $pos_settings->customer_name ==1){    ?>
