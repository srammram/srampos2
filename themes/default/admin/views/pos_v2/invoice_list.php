...
<head>
	...
	#choose-discount{ color:black; }
		...
		.select2-drop{background: #380404;}
		.select2-drop-active{border: 1px solid #380404;}
		.select2-dropdown-open .select2-choice{box-shadow: 0 1px 0 #380404 inset;}
		.select2-results li{color: #fff;font-size: 16px;}
		#req_2,#req_1{position: absolute;}
		.ui-keyboard{top: 0px!important;}
		.ui-keyboard div{width: 325px;position: absolute;right: 4%;top: 75%;}
		#payment-list,#userd_tender_list{text-align: center;}
	...
	<header class="logo_header">
		<div class="container-fluid">
			<div class="row">
                ...
                <a href="<?php echo base_url('pos/pos/'); ?>"><img src="<?=base_url()?>assets/uploads/logos/<?=$this->Settings->logo3?>" alt="<?=$this->Settings->site_name?>" class="sram_table_logo" /></a>
           <?php   } ?>
				</div>
			</div>
		</div>
...
<section class="kitchen_section">
	<div class="container">
		<div class="row">
			<ul class="split_list_sec">
				...
				<li><a href="<?php echo base_url().'pos/pos/invoice_list/?type=1'; ?>"><?=lang('dine_in')?></a></li>
				<?php } ?>

				<?php if($this->sma->actionPermissions('takeaway_orders')){  ?>
				<li><a href="<?php echo base_url().'pos/pos/invoice_list/?type=2'; ?>" class="active"><?=lang('take_away')?></a></li>
				<?php } ?>

				<?php if($this->sma->actionPermissions('door_delivery_orders')){  ?>
				<li><a href="<?php echo base_url().'pos/pos/invoice_list/?type=3'; ?>"><?=lang('door_delivery')?></a></li>
				<?php } ?>
	...
	<section class="drop_down_list">
		...
		<?php   $attrib = array( 'role' => 'form', 'id' => 'Sinvoicelist');  ?>
		  <?php echo form_open("pos/pos/invoice_list", $attrib);?>
			<div class="row">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 drop_down_list_s" style="padding: 0px;">
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
						<div class="form-group">
							...
							<div class="col-sm-7">
								<select class="form-control select tables" name="table_id">
					...
					<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
						<div class="form-group">
							...
							<div class="col-sm-7">
								<select class="form-control select stewards" name="steward">
									<option value="">All</option>
								<?php if($steward){ foreach($steward as $user) { ?>
										<option value="<?php  echo $user->id ?>"><?php  echo !empty($user->first_name)?$user->first_name:$user->username;   ?></option>
								<?php   } }  ?>
								</select>
							</div>
						</div>
        ...
        <ul>
            ...
            ?>
                     ...
                     /*    echo "<pre>";
						...
						$k=1;
                        foreach($sales_row->bils as $split_order){  if (count($split_order->id) > 0) {
                            ?>
							<tr>
						...
						<?php   }  ?>
							<td style="color:#fff;font-size:18px;">
							  <?php echo $sales_row->reference_no; ?>
							</td>
							<td>
                        ...
                        ?>
							...
							<td style="color:#fff;font-size:18px;">
							 <?=lang('sale_ref_no')?>: <?php echo $split_order->bill_sequence_number; ?>
							</td>
							<td>
                            <div class=" payment-list-container">
                                <?php // if($split_order->payment_status == NULL) {  ?>
                                <button type="button" class="tablebtn request_bil_new" data-item="payment" id="BNO<?php echo $split_order->bill_number; ?>"  <?php if($this->sma->actionPermissions('bil_payment')){ echo ''; }else{  echo 'disabled'; }  ?>>
				                  ...
				                  <?php }
				                  ?>
				              <button type="button" class="tablebtn rough-tender-payment" data-item="rough-tender" id="RT-BNO<?php echo $split_order->bill_number; ?>" <?= $RT_disabled?> <?php if($this->sma->actionPermissions('bil_payment')){ echo ''; }else{  echo 'disabled'; }  ?>>
				            ...
				            <?php } ?>
                                ...
                                <input type="hidden"  class="loyalty_available" value="<?php  echo $this->site->getCheckLoyaltyAvailable($split_order->customer_id); ?>">
                                 <button type="button" class="tablebtn request_bil" 
                                 data-bil="req_<?=$k; ?>"id="" <?php if($this->sma->actionPermissions('bil_print')){ echo ''; }else{  echo 'disabled'; }  ?>>
                    ...
                    <div class="col-md-12 col-sm-12 text-center">
                        <?php if ($Owner || $Admin || !$this->session->userdata('biller_id')) { ?>
                            <div class="form-group col-sm-12 "  style="margin-bottom: 5px;">
                                <div class="col-sm-2 text-left"><?=lang("biller", "biller");?></div>
                                <div class="col-sm-7 text-left biller_list_s">
                                <?php
                        ...
                        <div class="form-group bill_sec_head" style="color: #1F73BB!important;font-size: 20px!important;text-align: center;margin-bottom: 5px;">
...
if (site.pos_settings.bill_print_format==2) {
    bill_totals += '<td class="no-border text-right">'+ formatMoney(b.subtotal-b.manual_item_discount-b.input_discount) +'</td></tr>';
               ...
               <?php if($pos_settings->discount_note_display_option == 1){?>
                    ...
                    if(data.billdata.manual_item_discount != 0){
                       bill_totals += '<tr><th colspan="'+$cols1+'" class="text-left"><small>* Items are manually Discount is applied.</small></th></tr>';
                     }  
					      if(get_free_item !=0){
...
<script>
/* 	$('.stewards, .tables').change(function(){
		if($(this).val() ==''){
			return false; 
		}
				$.ajax({
                    type: 'post',
                    url: '<?= base_url('pos/pos/invoice_table_list');?>',
                    dataType: "html",
                    data: {
                        steward:$('.stewards :selected').val(),
                        table_id: $('.tables :selected').val(),
                    },
                    success: function (data) {
                    }
                });
