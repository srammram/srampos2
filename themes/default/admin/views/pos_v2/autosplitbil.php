...
<style>
	...
	table thead tr th{color: #fff;font-size: 16px;vertical-align: top;background-color: #380404;}
	.table-striped>tbody>tr:nth-child(odd)>td, .table-striped>tbody>tr:nth-child(odd)>th{background-color: transparent;}
	.modal-footer {border-top: none;}
	#submit-sale{background-color:#cf1f10;height: 34px;width: 120px;padding: 0px; border-color:#cf1f10; }
	.pos-variant-name1{     color: #e47345; position: relative;}
	.table_btn thead tr th,.table_btn tbody tr td{border: 1px solid #380404!important;}
	.table_btn>tbody+tbody{border-top: 2px solid #b58f8f;}
</style>
</head>
<body>
...
<section class="pos_bottom_s">
		<div class="container-fluid">
			<div class="row">
				...
				<div class="col-lg-10 col-md-10 col-sm-10 col-xs-12 text-left header_sec_me">
					<ul class="menu_nav">
						...
						<li>
						<a href="<?php echo base_url('pos/pos/payment/?type=1&table='.$table_id)   ?>">
			...
			<?php    
			 if(!empty($split_id)){ 
		       $sale_table=$this->site->get_order_details($split_id);   ?>
			   <div class="table_head" style="padding-left: 12px;">
                <div class="img_s"><img src="<?=$assets?>images/order-table.png" alt=""></div>
                <span class="odr_name"><?php echo   $sale_table->name ?></span><br>
...
echo form_open("pos/pos/billing?order_type=".$order_type."&bill_type=2&bils=".$bils."&table=".$table_id."&splits=".$split_id, $attrib);?>
...
<div class="quick-menu">
    <div id="proContainer">
        <div id="ajaxrecipe ">
        	...
        	<div id="item-list" class="row" style="max-height: 100%!important;">
				<?php 
                ...
                ?>
                    ...
                    <table id="example<?php echo $i; ?>" style="width: 100% ; float: left; margin: 0px 0px 0px 10px;" class="table table-bordered table-striped copytabl table_btn table_resolution">
                    	<thead>
                        	<tr class = "clickable" >
                    ...
                    <?php 
                    $recipe_variant_ids = array();$addon_subtotal = array();$manual_item_discount = array();
                    $recipeid_data = array();$recipeid_qty = array();
                        foreach($order_item as $salesitem) 
                        {
                            ...
                            $recipeid_data[] = $salesitem->recipe_id;
                             ...
                             $manual_item_discount[] = $salesitem->manual_item_discount/$split_count;
			                 $recipe_variant_ids[] = $salesitem->recipe_variant_id;
                            $discount_value = '';
                            if(!empty($discount)){
                            ...
                            <tr class = "clickable"  >
								...
								if($this->Settings->user_language == 'khmer'){
                                    ...
                                    $addon_subtotal[] = $itemaddonamt/$split_count;
                          ...
                          /*  echo"<pre>";   
                            ...
                            ?>
                            <input type="hidden" name="item_offer_dis[]" value="<?php echo $offer_dis;?>" id="recipe-offer-discount-<?=$salesitem->recipe_id?>-<?php echo $i;?>">
                            <input type="hidden"  id="recipe-input-discount-<?=$salesitem->recipe_id?>-<?php echo $i;?>">
                          ...
                          <td class="rec_price_che">
                              ...
                              <?php } $recipeids =  implode(',',$recipeid_data); $recipeqtys =  implode(',',$recipeid_qty);$recipe_variant_ids =  implode(',',$recipe_variant_ids); $addon_subtotal =  implode(',',$addon_subtotal);$manual_item_discount =  implode(',',$manual_item_discount);?>
                        </tbody>
                       <tbody>
                            <tr>
                       ...
                       /* echo "<pre>";
                        ...
                        {                
                             ...
                             if((!isset($discount['unique_discount']) || isset($discount['only_offer_dis'])) && $totdiscount1 < $value)
                             {  
                        ...
                        <tr>
                            ...
                            <div class="">
                                ...
                                <input type="hidden" name="split[<?php echo $i;?>][recipe_variant_ids]" id="recipe_variant_ids_<?php echo $i; ?>" value="<?php echo $recipe_variant_ids; ?>" >

                                <input type="hidden" name="split[<?php echo $i;?>][addon_subtotal]" id="addon_subtotal_<?php echo $i; ?>" value="<?php echo $addon_subtotal; ?>" >

                                <input type="hidden" name="split[<?php echo $i;?>][manual_item_discount]" id="manual_item_discount_<?php echo $i; ?>" value="<?php echo $manual_item_discount; ?>" >
								...
								<?php $member_dis= ($member_discount->discounttype =='percentage')? ($member_discount->discount/$bils).'%':($member_discount->discount/$bils); ?>
                                <input type="text" name="split[<?php echo $i;?>][order_discount_input]" autocomplete="off" class="kb-pad form-control pos-input-tip order_discount_input manual-discount" id="order_discount_input_<?php echo $i; ?>" value="<?php echo !empty($member_dis)?$member_dis:'';   ?>" count="<?php echo $i; ?>">
								<input type="hidden" name="member_dicount_card_number" value="<?php echo $member_discount->card_no ;  ?>">
								<input type="hidden" name="member_discount" value="<?php echo $member_dis ;  ?>">
...
<button class="btn btn-block center-block btn-lg btn-primary" id="submit-sale"><?=lang('send');?></button>
</div>
<?php
...
var recipe_variant_ids  = $('#recipe_variant_ids_'+ find_attr).val();
var addon_subtotal  = $('#addon_subtotal_'+ find_attr).val();
var manual_item_discount  = $('#manual_item_discount_'+ find_attr).val();
...
if(ds !=0){
        $.ajax({
                type: 'POST',
                url: '<?=admin_url('pos/autosplit_calculate_customerdiscount');?>',
                dataType: "json",
                 async : false,
                data: {
                    recipeids: recipeids,recipeqtys: recipeqtys,discountid:$this_obj.val(),divide: divide,recipe_variant_ids: recipe_variant_ids,addon_subtotal:addon_subtotal,manualitemdis:manual_item_discount
                },
