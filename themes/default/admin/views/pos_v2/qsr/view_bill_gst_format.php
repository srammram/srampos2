                ...
                <table class="table table-striped table-condensed">
                    ...
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr><td class="no-border"><span style="position: absolute;">' . $r . ':</span><span style="display: inline-block;padding-left: 15px;">' . ($row->recipe_name. ($printer ? null : null)) . ($row->recipe_variant ? ' [' . $row->recipe_variant . ']' : '') . '</span><span class="pull-right">' . ($row->tax_code ? '*'.$row->tax_code : '') . '</span></td>'; 
                        ...
                        if ($return_rows) {
                            ...
                            foreach ($return_rows as $row) {
                                ...
                                echo '<tr><td  class="no-border">#' . $r . ': &nbsp;&nbsp;' . product_name($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '') . '<span class="pull-right">' . ($row->tax_code ? '*'.$row->tax_code : '') . '</span></td></tr>';
                    ...
                    <tfoot>
                        ...
                        <?php
                        $grand_sub_total = $inv->total-$inv->total_discount-$inv->birthday_discount;
                        $dis = $this->site->getDiscountsAmt($inv->id);

                        if($inv->total_discount != 0){
                             echo '<tr><th colspan="3" class="text-right">' . lang("discount") . '('.$inv->discount_val.')</th><th class="text-right">' . $this->sma->formatMoney($inv->total_discount) . '</th></tr>';						
                        }
                    if ($pos_settings->display_tax == 1 && $inv->tax_rate != 0) {  
                        $tax_splits = $this->site->get_tax_splits($pos_settings->default_tax);

                        echo '<tr>';
                        echo '<th>' . $pos_settings->tax_caption. '</th>';
                        foreach ($tax_splits as $taxsplits) {
                            echo '<th>' . $taxsplits->name . '('.$taxsplits->rate.'%)</th>';
                        }

                        echo '</tr>';
                        echo '<tr>';
                        echo '<th>' . $this->sma->formatDecimal($inv->tax_rate). '</th>';
                        foreach ($tax_splits  as $tax_split) {                            
                            echo "<br>";
                            $tax_amount = ((($grand_sub_total) * $tax_split->rate) / 100);
                            echo '<th class="text-right">'. $this->sma->formatMoney($tax_amount) .'</th>';
                        }
                        // echo '<th colspan="1" class="text-right">' . lang("total_tax") . '</th>';
                        echo '<th colspan="1" class="text-right">' . $this->sma->formatMoney($inv->total_tax) . '</th>';
                        echo '</tr>';
                    }    
                        if ($inv->shipping != 0) {
                        ...
                        <?php
						if ($inv->tax_rate != 0) {
                           // echo '<tr><th colspan="3" class="text-right">' . lang("tax ".$taxname." (".$inv->tax_name.")") . '</th><th class="text-right">' . $this->sma->formatMoney($return_sale ? ($inv->order_tax+$return_sale->order_tax) : $inv->total_tax) . '</th></tr>';
							// echo '<tr><th colspan="4" class="text-left">*<small>' . lang("tax ".$taxname." (".$inv->tax_name.")") . '</small></th></tr>';
                        }
						?>
                        <?php
                        if ($inv->paid < ($inv->grand_total + $inv->rounding)) {
                            ?>
                          <!--   <tr>
                                <th colspan="5" class="text-right"><?=lang("paid_amount");?></th>
                                <th class="text-right"><?=$this->sma->formatMoney($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid);?></th>
                            </tr>
                            <tr>
                                <th colspan="5" class="text-right"><?=lang("due_amount");?></th>
    ...
    left: 0px;" class="no-print">
            ...
            if ($modal) {
                ...
                <span class="col-xs-12">
                    <a class="btn btn-block btn-warning" href="<?= admin_url('qsr'); ?>"><?= lang("back_to_qsr"); ?></a>
                </span>
                <?php
            }
            if ($pos->remote_printing == 1) {
