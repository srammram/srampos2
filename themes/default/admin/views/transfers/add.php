...
<script type="text/javascript">
    ...
    if (localStorage.getItem('material_request_no')) {
        localStorage.removeItem('material_request_no');
    }
    ...
    $(document).ready(function () {
        ...
        $("#material_request_no").val(localStorage.getItem('material_request_no'));
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="form-group">
                            ...
                            <?php echo form_input('reference_no', (isset($_POST['reference_no']) ? $_POST['purchases_order_no'] : ""), 'class="form-control input-tip" id="ref"'); ?>
                            ...
                            <div class="panel panel-warning">
                                ...
                                <div class="panel-body" style="padding: 5px;">
                                    ...
                                    <div class="col-md-4" style="padding: 5px;">
                                        <div class="form-group">
                                            <?= lang("material_request_no"); ?>
                                <?php                              
                                   $mrn[''] = '';
                                    foreach ($material_request as $material) {
                                        $mrn[$material->id] = $material->material_request_no;
                                    }
                                    echo form_dropdown('material_request_no', $mrn, (isset($_POST['material_request_no']) ? $_POST['material_request_no'] : '' ), ' class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("material_request_no") . '"style="width:100%;" id="material_request_no" required="required" ');
                                    ?>
                                        </div>
                                    </div>
                                </div>
...
<script>
    $(document).on('change', '#material_request_no', function(){
   ...
   /* if (localStorage.getItem('po_currency')) {
    ...
    var material_request_no = $(this).val();
    $.ajax({
        type: 'get',
        url: '<?= admin_url('transfers/material_request_list'); ?>',
        dataType: "json",
        data: {
            material_request_no: material_request_no
        },
        success: function (data) {
            var materialrequests_value = [];
            $(this).removeClass('ui-autocomplete-loading');
            var items = JSON.stringify(data.value['toitems']);
            var materialrequests = JSON.stringify(data.value['materialrequests']);
           materialrequests_value = $.parseJSON(materialrequests);

            localStorage.setItem('material_request_no',  materialrequests_value["id"]);

            localStorage.setItem('toshipping',  materialrequests_value["shipping"]);

            
            localStorage.setItem('from_warehouse', materialrequests_value["warehouse_id"]);

            localStorage.setItem('to_warehouse', materialrequests_value["to_warehouse_id"]);

            localStorage.setItem('tonote', materialrequests_value["note"]);


            localStorage.setItem('po_tax2', materialrequests_value["order_tax_id"]);
           
            
            localStorage.setItem('toitems', items);

            localStorage.setItem('todate', materialrequests_value["date"]);
            location.reload();
        }
    });
});
