...
die;*/
       ...
       // $getTax = $this->site->getTaxRateByID($getTax->id);
        ...
        //$itemtax = ($subval) * ($getTax->rate / 100);
                    ...
                    $kitchen_type_id = $this->site->getKitchenID_recipe($item_id);
                    $recipe = array(
                        'recipe_id'      => $item_id,
  ...
  print_r($recipe);
            ...
            $total_tax = $this->sma->formatDecimal(($recipe_tax + $order_tax));
                    ...
                    if($_POST['amount_KHR'][$r] == '' && $_POST['amount_'.$default_currency_data->code.''][$r] == '')
                    {
        				...
        				if(!empty($amount) || !empty($amount_exchange)){
            				$payment[] = array(
                                ...
                                'pos_balance'  => $_POST['balance_amount'],
            ...
            // var_dump($did);die;
                ...
                if ($sale = $this->qsr_model->addInsertbil($data, $recipes, $payment, $update_bill, $sales_bill, $multi_currency, $did,$order_data, $bill_recipes,$total,$customer_id,$loyalty_used_points,$taxation,$discountid)) {
                   ...
                   // $this->session->set_flashdata('message', $msg);                   
                     $redirect_to = $this->pos_settings->after_sale_page ? "pos" : "pos/qsr/view_bill/" . $sale['sale_id'];
...
echo "</pre>";die;*/
            $this->load->view($this->theme . 'pos_v2/qsr/qsr', $this->data);
        }
    }
    ...
    public function view_bill($billid = NULL, $modal = NULL)
    {
        ...
        $this->data['tax_rates'] = $this->site->getAllTaxRates();
         if($this->pos_settings->bill_print_format ==3){
            $this->load->view($this->theme . 'pos_v2/qsr/view_bill_gst_format', $this->data);
            // $redirect_to = $this->pos_settings->after_sale_page ? "pos" : "qsr/view_bill_gst_format/" . $sale['sale_id'];
        }else{
            $this->load->view($this->theme . 'pos_v2/qsr/view_bill', $this->data);
	...
	public function customer_bill()
    {
        //$this->sma->checkPermissions('index');
        $this->data['tax_rates'] = $this->site->getAllTaxRates();
        $this->load->view($this->theme . 'pos_v2/qsr/customer_bill', $this->data);
    }
   ...
   /* public function getrecipeDataByCode($code = NULL, $warehouse_id = NULL)
	...
	public function getrecipeDataByCode($recipe_id = NULL, $warehouse_id = NULL)
    {
       // $this->sma->checkPermissions('index');
        /* insted of recipe code used here recipe_id in same parametter $this->input->get('code') */
        if ($this->input->get('code')) {
            $recipe_id = $this->input->get('code', TRUE);
        }
        if ($this->input->get('warehouse_id')) {
            $warehouse_id = $this->input->get('warehouse_id', TRUE);
        }
        ...
        $row = $this->qsr_model->getWHrecipe($recipe_id, $warehouse_id);
        $option = false;
...
//$vari .= '<input class="kb-text1" id="myInput"style="width: 50%;height:36px;line-height:36px;padding:8px; border: 1px solid #ccc;"placeholder="Variants Search.." autocomplete ="off" >';
			...
			'{default} {space} {cancel} {accept}'
               ...
               /* $prods .= "<span><button id=\"recipe-" . $category_id . $count . "\" type=\"button\" value='" . $recipe->id . "' title=\"" . $recipe->name . "\" class=\"btn-prni btn-" . $this->pos_settings->recipe_button_color . " ".$class." recipe pos-tip\" data-container=\"body\"><img src=\"" . base_url() . "assets/uploads/thumbs/" . $recipe->image . "\" alt=\"" . $recipe->name . "\" class='img-rounded' />";*/
                if($this->pos_settings->sale_item_display == 0){
                  $prods .= "<span><button id=\"recipe-" . $category_id . $count . "\" type=\"button\" value='" . $recipe->id . "' title=\"" . $recipe->name . "\" class=\"btn-prni btn_item btn-" . $this->pos_settings->recipe_button_color . " ".$class." recipe pos-tip\" data-container=\"body\"><img src=\"" . base_url() . "assets/uploads/thumbs/" . $recipe->image . "\" alt=\"" . $recipe_name . "\" class='img-rounded' />";
                 }else{
				...
				$prods .=  "<span class='price_strong'> ".$default_currency_symbol ."" . $this->sma->formatDecimal($recipe->price). "</span>".$buyvalue." </button>";
                $prods .=$vari.'</span>';
   ...
   /* public function ajaxcategorydata($category_id = NULL)
    ...
    public function opened_bills($per_page = 0)
    {
        ...
        echo $this->load->view($this->theme . 'pos_v2/qsr/opened', $data, TRUE);
...
// var_dump($id);die;
    ...
    $variantsids = $this->input->post('variantsids');
...
/*echo "string";die;*/
        ...
        $variant_id = $variantsids[$key];
        
        // $price_total = $recipeDetails->cost;
       $recipe_Variant_Details = $this->qsr_model->getrecipeVarient($recipe_id,$variant_id);       
        if(!empty($recipe_Variant_Details)){  
           $price_total = (($recipe_Variant_Details->price*$current_qty));
          $price_total = $price_total;
        }else{
            $price_total = (($recipeDetails->cost*$current_qty));
            $price_total = $price_total;
        }
...
public function reprint_view($billid = NULL, $modal = NULL)
    ...
    public function cancel_bill(){
        ...
        $this->data['sales'] = $this->qsr_model->getAllBillingforReprint($start);              
        $this->load->view($this->theme . 'qsr/cancel_bill', $this->data);
    }
    public function resettle_bill(){
        // echo "string";die;
         $start = $this->input->get('date'); 
        if($start){
            $start = $start;
        ...
        $this->data['sales'] = $this->qsr_model->getAllSalesWithbiller($start);
        $this->data['currency'] = $this->site->getAllCurrencies();
        $this->load->view($this->theme . 'qsr/resettlebill', $this->data);
    } 
   public function cancel_sale($cancel_remarks = NULL, $sale_id = NULL)
    {        
        ...
        $notification_array['insert_array'] = array(            
         ...
         $result = $this->qsr_model->CancelSale($cancel_remarks, $sale_id, $this->session->userdata('user_id'));
        if($result == true){
           $msg = 'success'; 
...
//echo '33'.$_POST['amount_KHR'][$r];
                    ...
                    //}else{
					    foreach($currency as $currency_row){	
    						if($currency_row->rate == $default_currency_data->rate){						
    							$p = isset($_POST['amount_'.$currency_row->code][$r]) ? sizeof($_POST['amount_'.$currency_row->code]) : 0;							
    							echo $amount = $_POST['amount_'.$currency_row->code][$r];							
    						}else{							
    							echo $amount_exchange = $_POST['amount_'.$currency_row->code][$r];
    						}
					   }
			      ...
			      //   }
                   ...
                   redirect('pos/qsr/resettle_bill?date='.$date);
    }
	function  sales_list(){
		 $this->data['sales'] = $this->qsr_model->getAllSalesWithbiller();
         $this->data['currency'] = $this->site->getAllCurrencies();
		 $this->load->view($this->theme . 'pos_v2/qsr/sales_list', $this->data);
	}
