...
//	'archival_days' => $this->input->post('archival_days')
	    ...
	    $this->data['today_transac_date'] = $this->settings_model->getTransactionDate();
    ...
    public function deletebbqbuyxgetx()
    {
        // $this->sma->checkPermissions('index');
        $id =$this->input->post('id');
        if ($this->settings_model->deleteBBQBUYGET($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("suspended_sale_deleted")));
        }
    }

    public function bbq_lobster_discount()
    {
        $this->data['buyxgetx'] = $this->settings_model->getBBQbuyxgetx();
        $this->form_validation->set_rules('days[]', lang("days"), 'required');
        if ($this->form_validation->run() == true) {            
            if(!empty(implode(',', $this->input->post('discount_apply_type')))){                
                for($i=0; $i<count($this->input->post('days')); $i++){
                    $array_update[] = array(
                        'days' => $_POST['days'][$i],
                        'minimum_cover' => $_POST['minimum_cover'][$i], 
                        'discount_apply_type' => $_POST['discount_apply_type'][$i],
                        'discount_type' => $_POST['discount_type'][$i],
                        'adult_discount_val' => $_POST['adult_discount_val'][$i],
                        'child_discount_val' => $_POST['child_discount_val'][$i],
                        'kids_discount_val' => $_POST['kids_discount_val'][$i],                     
                        'created_by' => $this->session->userdata('user_id'),
                        'status' => 1,
                        'created_at' => date('Y-m-d H:i:s')
                    ); 
                ...
                $this->data['error'] = $this->session->set_flashdata('error', 'test');
                 admin_redirect("system_settings/bbq_lobster_discount/");
            }
        }
        if ($this->form_validation->run() == true && $this->settings_model->updateBBQLobsterDiscount($array_update)) {
            
            $this->session->set_flashdata('message', lang("bbq_lobster_discount_updated"));
            admin_redirect("system_settings/bbq_lobster_discount/");
        } else {
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('bbq_lobster_discount')));
            $meta = array('page_title' => lang('bbq_lobster_discount'), 'bc' => $bc);
            $this->page_construct('settings/bbq_lobster_discount', $meta, $this->data);
        }
    }
    public function deleteLobster()
    {
        // $this->sma->checkPermissions('index');
        $id =$this->input->post('id');
        if ($this->settings_model->deleteLobster($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("lobster_discounts_deleted")));
        }
    }
    public function bbq_daywise_discount()
    {        
        $this->sma->checkPermissions();
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('bbq_daywise_discount')), array('link' => '#', 'page' => lang('bbq_daywise_discount')));
        $meta = array('page_title' => lang('bbq_daywise_discount'), 'bc' => $bc);
        $this->page_construct('settings/bbq_daywise_discount', $meta, $this->data);
    }

  function get_daywise_discount()
    {
    // $this->sma->checkPermissions('currencies');
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',bbq_daywise_discount_hd.id, bbqm.image,bbqm.name, bbqm.name as sale_type")
            ->from("bbq_daywise_discount_hd")
            ->join("bbq_menu bbqm", 'bbqm.bbq_menu_id=bbq_daywise_discount_hd.bbq_menu_id')
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/view_bbq_daywise_discount/$1') . "' class='btn-primary btn-xs tip' title='" . lang("view_bbq_daywise_discount") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-eye\"></i></a> <a href='" . admin_url('system_settings/edit_bbq_daywise_discount/$1') . "' class='btn-warning btn-xs tip' title='" . lang("edit_bbq_daywise_discount") . "' ><i class=\"fa fa-edit\"></i></a> <a href='#' class='btn-danger btn-xs tip po' title='<b>" . lang("delete_bbq_daywise_discount") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_bbq_daywise_discount/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "bbq_daywise_discount_hd.id");
        //->unset_column('id');
        echo $this->datatables->generate();
    }
    public function add_bbq_daywise_discount()
    {
        $this->data['bbq_discounts'] = $this->settings_model->getBBQbuyxgetx();
        /*echo "<pre>";
        print_r($this->data['bbq_discounts']);die;*/
        $this->data['bbq_menu'] = $this->settings_model->getBBQMenus();
        if(!empty($_POST)){    
        $data = 0;
        $user = $this->input->post('days', TRUE);
        foreach ($user as $u) {
            if ($u != '')
            {
                $data++;
        ...
        // $this->form_validation->set_rules('bbq_menu_id', lang("bbq_menu"), 'required');
        // $this->form_validation->set_rules('from_date', lang("from_date"), 'required');
        $this->form_validation->set_rules('bbq_menu_id', lang("bbq_menu"), 'trim|is_unique[bbq_daywise_discount_hd.bbq_menu_id]|required');
        // $this->form_validation->set_rules('to_date', lang("to_date"), 'required');
        if ($data == 0) { $this->form_validation->set_rules('days', lang("days"), 'required');    }
        if ($this->form_validation->run() == true) {   
            $data = array('bbq_menu_id' => $this->input->post('bbq_menu_id'),
            ...
            if(!empty(implode(',', $this->input->post('discount_type')))){                
                for($i=0; $i<count($this->input->post('days')); $i++){
                    $bbq_daywise_discount[] = array(
                        'days' => $_POST['days'][$i],                                               
                        'discount_type' => $_POST['discount_type'][$i],
                        'adult_discount_val' => $_POST['adult_discount_val'][$i],
                        'child_discount_val' => $_POST['child_discount_val'][$i],
                ...
                $this->data['error'] = $this->session->set_flashdata('error', 'somthing is wrong');                 
                 admin_redirect("system_settings/add_bbq_daywise_discount/");
            }
        }
}        
        if ($this->form_validation->run() == true && $this->settings_model->AddBBQDaywiseDiscount($data,$bbq_daywise_discount)) {
            $this->session->set_flashdata('message', lang("bbq_daywise_discount_updated"));
            admin_redirect("system_settings/bbq_daywise_discount/");
        } else {
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('add_bbq_daywise_discount')));
            $meta = array('page_title' => lang('add_bbq_daywise_discount'), 'bc' => $bc);            
            $this->page_construct('settings/add_bbq_daywise_discount', $meta, $this->data);
        }
    }

    function edit_bbq_daywise_discount($id = NULL)
    {   
      $menu_details = $this->settings_model->getBBQProcessByID($id);
      if(!empty($_POST)){    
        /*echo "<pre>";
        ...
        if ($this->form_validation->run() == true) {
            $data = array('bbq_menu_id' => $this->input->post('bbq_menu_id'),
             ...
             if(!empty(implode(',', $this->input->post('discount_type')))){                
                ...
                $this->data['error'] = $this->session->set_flashdata('error', 'somthing is wrong');                 
                 admin_redirect("system_settings/edit_bbq_daywise_discount/");
            }
        } elseif ($this->input->post('bbq_menu_id')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/edit_bbq_daywise_discount");
        }
}   
    if ($this->form_validation->run() == true && $this->settings_model->updateBBQDaywiseDiscount($id, $data,$bbq_daywise_discount)) { 
            $this->session->set_flashdata('message', lang("edit_bbq_daywise_discount_updated"));
            admin_redirect("system_settings/bbq_daywise_discount");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['daywisediscount'] = $this->settings_model->getBBQdaywisediscount($id);
            $this->data['bbqdaywisediscount'] = $this->settings_model->getBBQDaywiseDiscountByID($id); 
            /*echo "<pre>";           
            print_r($this->data['daywisediscount']);die;*/
            $this->data['bbq_menu'] = $this->settings_model->getBBQMenus();            
            $this->data['id'] = $id;
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings/bbq_daywise_discount'), 'page' => lang('bbq_daywise_discount')), array('link' => '#', 'page' => lang('edit_bbq_daywise_discount')));
            $meta = array('page_title' => lang('edit_bbq_daywise_discount'), 'bc' => $bc);
            $this->page_construct('settings/edit_bbq_daywise_discount', $meta, $this->data);
      } 
}

public function view_bbq_daywise_discount($id = null)
    {
        $this->data['page_title'] = lang('view_bbq_daywise_discount');        
        $this->data['daywisediscount'] = $this->settings_model->getBBQdaywisediscount($id);
        $this->data['bbqdaywisediscount'] = $this->settings_model->getBBQDaywiseDiscountByID($id); 
        $this->data['bbq_menu'] = $this->settings_model->getBBQMenus();            
        /*echo "<pre>";
        ...
        $this->load->view($this->theme . 'settings/view_bbq_daywise_discount', $this->data);
    }    
    public function change_status_daywisediscount()
    {
        // $this->sma->checkPermissions('index');
        $id =$this->input->post('id');
        $status =$this->input->post('status');
        if ($this->settings_model->change_status_daywisediscount($id,$status)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("daywisediscount_deleted")));
        }
    }

    public function delete_bbq_daywise_discount($id = NULL)
    {
        // $this->sma->checkPermissions('index');
        // $id =$this->input->post('id');
        if ($this->settings_model->delete_bbq_daywise_discount($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("bbq_daywise_discount_deleted")));
        }
    }
	...
	$this->sma->checkPermissions();
		...
		if (!file_exists($this->upload_path . 'logos/')) {
		    mkdir($this->upload_path . 'logos/', 0777, true);
		}
                $this->load->library('upload');
            ...
            if ($_FILES['biller_logo']['size'] > 0) {
                ...
                $photo = $this->upload->file_name;
                $this->db->update('settings', array('logo3' => $photo), array('setting_id' => 1));
            }
    ...
    function permissions($id = NULL,$user_id =null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'recipe-list_ingredients' => $this->input->post('recipe-list_ingredients'),
                'recipe-add_ingredients' => $this->input->post('recipe-add_ingredients'),   
                'recipe-ingredients_edit' => $this->input->post('recipe-ingredients_edit'),             
                'recipe-ingredients_delete' => $this->input->post('recipe-ingredients_delete'),
                'recipe-ingredients_import_csv' => $this->input->post('recipe-ingredients_import_csv'),
                ...
                'reports-store_request_reports' => $this->input->post('reports-store_request_reports'), 
                'reports-quotes_request_reports' => $this->input->post('reports-quotes_request_reports'), 
                'reports-quotation_reports' => $this->input->post('reports-quotation_reports'), 
                'reports-purchase_order_reports' => $this->input->post('reports-purchase_order_reports'), 
                'reports-purchase_invoice_reports' => $this->input->post('reports-purchase_invoice_reports'), 
                'reports-purchase_return_reports' => $this->input->post('reports-purchase_return_reports'), 
                'reports-purchase_order_summary_reports' => $this->input->post('reports-purchase_order_summary_reports'), 
                'reports-purchase_invoice_summary_reports' => $this->input->post('reports-purchase_invoice_summary_reports'), 
                'reports-item_stock' => $this->input->post('reports-item_stock'), 
    ...
    public function sales_recipecategories_mapping()
    {
         
        // $this->data['buyxgetx'] = $this->settings_model->getBBQbuyxgetx();
        $this->data['recipe_groups'] = $this->site->getAllrecipeCategories();
        ...
        /*if ($this->form_validation->run() == true) {*/
         ...
         print_r($this->input->post());die;
            if(!empty(implode(',', $this->input->post('adult_buy')))){
                ...
                $this->data['error'] = $this->session->set_flashdata('error', 'test');
                 admin_redirect("system_settings/sales_recipecategories_mapping/");
            }
        }
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateBBQBUY($array_update)) {
            ...
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $this->data['recipe_groups'] = $this->site->getAllrecipeCategories();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('sales_recipecategories_mapping')));
            $meta = array('page_title' => lang('sales_recipecategories_mapping'), 'bc' => $bc);
            $this->page_construct('settings/sales_recipecategories_mapping', $meta, $this->data);
        }
    }
	...
	$this->sma->checkPermissions('recipecategories');
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',{$this->db->dbprefix('recipe_categories')}.id as id, {$this->db->dbprefix('recipe_categories')}.image, {$this->db->dbprefix('recipe_categories')}.code, {$this->db->dbprefix('recipe_categories')}.name,  c.name as parent,{$this->db->dbprefix('recipe_categories')}.status as status", FALSE)
            ->from("recipe_categories")
            ->join("recipe_categories c", 'c.id=recipe_categories.parent_id', 'left')
            ->group_by('recipe_categories.id')
	    ->where('recipe_categories.type',0)
	...
	$pid = $this->input->post('parent');
       ...
       // $this->form_validation->set_rules('slug', lang("slug"), 'required|is_unique[categories.slug]|alpha_dash');
        ...
        if ($this->form_validation->run() == true && $sid = $this->settings_model->addrecipeCategory($data)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
	    if(isset($_GET['sub'])){
		$this->load->view($this->theme . 'settings/add_recipeSubcategory', $this->data);
	    }else{
    ...
    $this->sma->checkPermissions();
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('tax_splits')));
        $meta = array('page_title' => lang('tax_splits'), 'bc' => $bc);
        $this->page_construct('settings/tax_splits', $meta, $this->data);
    }    

    function getTaxSplits()
    {
    // $this->sma->checkPermissions('tax_rates');
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',tax_rates.id, tax_rates.name, tax_rates.code")
            ->from("tax_rates")            
            ->join("tax_splits","tax_splits.tax_id = tax_rates.id")
            ->group_by('tax_rates.id')
        ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_tax_splits/$1') . "' ><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_tax_delete") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_tax_delete/$1') . "'>" . lang('i_m_sure') . "</a>  <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div> ", "tax_rates.id");

        echo $this->datatables->generate();
    }

    function edit_tax_splits($id = NULL)
    {
        // $this->sma->checkPermissions();
        $this->form_validation->set_rules('name', lang("name"), 'trim|required');
        $this->data['tax_rate'] = $this->settings_model->getAllTaxRates();        
        $tax_split_details = $this->settings_model->getTaxSplitsByTaxid($id);        
        if ($this->input->post('tax_id') != $tax_split_details->name) {            
            $this->form_validation->set_rules('tax_split_name', lang("tax_split_name"), 'required|is_unique[tax_splits.name]');
        }
        $this->form_validation->set_rules('tax_id', lang("tax_id"), 'required');        
        if ($this->form_validation->run() == true) {
            $data = array('name' => $this->input->post('name'),
                'code' => $this->input->post('code'),
                'type' => $this->input->post('type'),
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateTaxRate($id, $data)) { 
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => admin_url('system_settings/Tax_Splits'), 'page' => lang('Tax_Split')),array('link' => '#', 'page' => lang('Tax_Split_edit')));
            $meta = array('page_title' => lang('edit_tax_splits_edit'), 'bc' => $bc);
            $this->data['id'] = $id;        
            $this->page_construct('settings/edit_tax_splits', $meta, $this->data);    
        }
    }

/*service charge 04-08-2019 sivan*/   
    ...
    $this->sma->checkPermissions();
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('service_charge')));
        $meta = array('page_title' => lang('service_charge'), 'bc' => $bc);
        $this->page_construct('settings/service_charge', $meta, $this->data);
    }
    
    
    function getservice_charge()
    {
    $this->sma->checkPermissions('service_charge');
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',id, name, display_value, rate")
            ->from("service_charge")
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_service_charge/$1') . "' class='tip' title='" . lang("edit_service_charge") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_service_charge") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_service_charge/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
        echo $this->datatables->generate();
    }
    ...
    {
    $this->sma->checkPermissions();
        $this->form_validation->set_rules('name', lang("name"), 'trim|is_unique[service_charge.name]|required');
        //$this->form_validation->set_rules('type', lang("type"), 'required');
        $this->form_validation->set_rules('rate', lang("service_charge_rate"), 'required|numeric');

        if ($this->form_validation->run() == true) {
            $data = array('name' => $this->input->post('name'),
                'display_value' => $this->input->post('display_value'),
                'rate' => $this->input->post('rate'),
                'created_on' => date('Y-m-d-H-i-s'),
                'created_by' => $this->session->userdata('user_id'),
                'status' => 1,
            );
        } elseif ($this->input->post('add_service_charge')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/service_charge");
        }

        if ($this->form_validation->run() == true && $this->settings_model->addServiceCharge($data)) {
            $this->session->set_flashdata('message', lang("service_charge_added"));
            admin_redirect("system_settings/service_charge");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_service_charge', $this->data);
        }
    }
    
    function edit_service_charge($id = NULL)
    {
    $this->sma->checkPermissions();
        $this->form_validation->set_rules('name', lang("name"), 'trim|required');
        $service_charge_details = $this->settings_model->getSercideChargeByID($id);
        if ($this->input->post('name') != $service_charge_details->name) {
            $this->form_validation->set_rules('name', lang("name"), 'required|is_unique[service_charge.name]');
        }
        // $this->form_validation->set_rules('type', lang("type"), 'required');
        $this->form_validation->set_rules('rate', lang("service_charge_rate"), 'required|numeric');
        ...
        } elseif ($this->input->post('edit_service_charge')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/service_charge");
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateServiceCharge($id, $data)) { 
            $this->session->set_flashdata('message', lang("service_charge_updated"));
            admin_redirect("system_settings/service_charge");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['service_charge'] = $this->settings_model->getSercideChargeByID($id);
            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_service_charge', $this->data);
        }
    }
    
     function delete_service_charge($id = NULL)
    {
    $this->sma->checkPermissions();
        if ($this->settings_model->deleteSercideCharge($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("service_charge_deleted")));
        }
    }
/*service charge 04-08-2019 sivan*/    
...
$this->sma->checkPermissions('sales_type');
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',id, name,type")
            ->from("sales_type")
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/category_subcategory_sale_items_mapping/$1') . "' class='tip' title='" . lang("sales_items_mapping") . "'><i class=\"fa fa-list\"></i></a><a href='" . admin_url('system_settings/edit_sales_type/$1') . "' class='tip' title='" . lang("edit_sales_type") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_sales_type") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_sales_type/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
        //->unset_column('id');
        echo $this->datatables->generate();
    }
    ...
    function create_sale_item_mapping(){   
        ...
        } elseif ($this->input->post('create_sale_item_mapping')) {            
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/create_sale_item_mapping");
        }
        if ($this->form_validation->run() == TRUE && ($new_group_id = $this->settings_model->addSalesitems_mapping_head($data))) {
            $this->session->set_flashdata('message', lang('sale_item_mapping_by_saletype_added'));            
            admin_redirect("system_settings/sale_item_mapping_by_saletype_and_day/" . $new_group_id);
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/create_sale_item_mapping', $this->data);
        }
    }
function sale_item_mapping_by_saletype_and_day($id = NULL)
    {
        $this->form_validation->set_rules('group', lang("group"), 'is_natural_no_zero');
        if ($this->form_validation->run() == true) {               
            $data = $_POST;          
        }    
        if ($this->form_validation->run() == true && $this->settings_model->addsaleitemsmapping($id, $data)) {
            
        $this->session->set_flashdata('message', lang("category_subcategory_sale_items_mapping_created"));
        admin_redirect("system_settings/category_subcategory_sale_items_mapping");         
        } else {          
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['id'] = $id;        

        $this->data['group'] = $this->settings_model->getGroupByID($id);
        $sales_type = $this->settings_model->getSaleTypefromsalesmapping($id);        
        $this->data['recipe_groups'] = $this->site->getAllrecipeCategories_items($sales_type);        
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings/category_subcategory_sale_items_mapping'), 'page' => lang('category_subcategory_sale_items_mapping')), array('link' => '#', 'page' => lang('sale_item_mapping_by_saletype_and_day')));
        $meta = array('page_title' => lang('sale_item_mapping_by_saletype_and_day'), 'bc' => $bc);
        $this->data['page_title'] = $meta['page_title'];
        $this->data['edit_link'] = 'system_settings/sale_item_mapping_by_saletype_and_day/' . $id.'/'.$id;
        $this->page_construct('settings/sale_item_mapping_by_saletype_and_day', $meta, $this->data);
        }  
    }   
function edit_sale_item_mapping_by_saletype_and_day($id = NULL)
    {
        $this->form_validation->set_rules('sales_map_hd_id', lang("sales_map_hd_id"), 'is_natural_no_zero');
        if ($this->form_validation->run() == true) {               
            $data = $_POST;          
        }        
            $sales_map_hd_id = $this->input->post('sales_map_hd_id');
            if ($this->form_validation->run() == true && $this->settings_model->editsaleitemsmapping($sales_map_hd_id, $data)) {
            $this->session->set_flashdata('message', lang("edit_sale_item_mapping_by_saletype_and_day_updated"));
            ...
            $sales_type = $this->settings_model->getSaleTypefromsalesmapping($id);             
            $this->data['recipe_groups'] = $this->site->getAllrecipeCategories_items($sales_type);
            $this->data['saleitemsmaphead'] = $this->settings_model->getSaleTypeSaleitemMappinghead($id);            
            $this->data['customer_discounts'] = $this->settings_model->getSalesmapping($this->data['saleitemsmaphead']->id);
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings/category_subcategory_sale_items_mapping'), 'page' => lang('category_subcategory_sale_items_mapping')), array('link' => '#', 'page' => lang('edit_sale_item_mapping_by_saletype_and_day')));
            $meta = array('page_title' => lang('edit_sale_item_mapping_by_saletype_and_day'), 'bc' => $bc);
            $this->data['page_title'] = $meta['page_title'];
            $this->data['edit_link'] = 'system_settings/edit_sale_item_mapping_by_saletype_and_day/' . $id;
            $this->page_construct('settings/edit_sale_item_mapping_by_saletype_and_day', $meta, $this->data);
        }   
    
    }


    public function category_subcategory_sale_items_mapping($id = NULL)
    {   
        $this->form_validation->set_rules('items[]', lang("items"), 'required');
        $customer_discounts = $this->settings_model->getCustomerDiscount($id);
        if ($this->form_validation->run() == true) {
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateBBQItems($id, $data)) {
            ...
            $this->session->set_flashdata('message', lang("category_subcategory_sale_items_mapping_updated"));
            admin_redirect("system_settings/category_subcategory_sale_items_mapping/".$id);
        } else {

            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');

            if($id){
                $this->data['saletypes'] = $this->settings_model->getSaleTypeSaleitemMapping($id);
            }else{
                $this->data['saletypes'] = $this->settings_model->getALLSaleTypeSaleitemMapping($id);
            }            
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings/sales_type'), 'page' => lang('sales_type')), array('link' => '#', 'page' => lang('category_subcategory_sale_items_mapping')));
            $meta = array('page_title' => lang('category_subcategory_sale_items_mapping'), 'bc' => $bc);            
            $this->page_construct('settings/category_subcategory_sale_items_mapping', $meta, $this->data);
        }
    }

    function delete_sale_mapping($id = NULL,$day = NULL)
    {
        
        if ($this->settings_model->deletesalemapping($id)) {
            $this->session->set_flashdata('message', lang("sale_mapping_deleted"));
            admin_redirect("system_settings/category_subcategory_sale_items_mapping");
        }
    }
    ...
    $this->sma->checkPermissions();
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('shift_time')), array('link' => '#', 'page' => lang('shift_time')));
        $meta = array('page_title' => lang('shift_time'), 'bc' => $bc);
        $this->page_construct('settings/shift_time_index', $meta, $this->data);
    }

  function get_shift_times()
    {
    // $this->sma->checkPermissions('currencies');
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',id, name, start_time, end_time,status")
            ->from("shift_time")
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_shift_time/$1') . "' class='tip' title='" . lang("edit_shift_time") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_currency") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_shift_time/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id")
            ->edit_column('status', '$1__$2', 'status, id');
        //->unset_column('id');
    ...
    // $this->sma->checkPermissions();             
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/shift_time");
        }

        if ($this->form_validation->run() == true && $this->settings_model->addShifttime($data)) { //check to see if we are creating the customer
            $this->session->set_flashdata('message', lang("currency_added"));
            admin_redirect("system_settings/shift_time");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['page_title'] = lang("new_currency");
            $this->load->view($this->theme . 'settings/add_shift_time', $this->data);
        }
    }        

    function edit_shift_time($id = NULL)
    {         
      ...
      // $this->sma->checkPermissions();
       ...
       /* $cur_details = $this->settings_model->getCurrencyByID($id);
        if ($this->input->post('code') != $cur_details->code) {
            $this->form_validation->set_rules('start_time', lang("start_time"), 'required|is_unique[currencies.code]');
        }*/
        $this->form_validation->set_rules('end_time', lang("end_time"), 'required');        
        ...
        } elseif ($this->input->post('edit_shift_time')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/shift_time");
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateShifttime($id, $data)) { //check to see if we are updateing the customer
            $this->session->set_flashdata('message', lang("currency_updated"));
            admin_redirect("system_settings/shift_time");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['shifttime'] = $this->settings_model->getShifttimeByID($id);
            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_shift_time', $this->data);
        }
    }
      function delete_shift_time($id = NULL)
    {
        ...
        if ($this->settings_model->deleteShift($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("shift_deleted")));
        }
    }
    function bbq_menu()
    {
    $this->sma->checkPermissions();
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('bbq_menu')), array('link' => '#', 'page' => lang('bbq_menu')));
        $meta = array('page_title' => lang('bbq_menu'), 'bc' => $bc);
        $this->page_construct('settings/bbq_menu', $meta, $this->data);
    }

  function get_bbq_menu()
    {
    // $this->sma->checkPermissions('currencies');
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',bbq_menu.bbq_menu_id, bbq_menu.image,bbq_menu.name, st.name as sale_type")
            ->from("bbq_menu")
            ->join("sales_type st", 'st.id=bbq_menu.sale_type', 'left')
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/view_bbq_menu/$1') . "' class='btn-primary btn-xs tip' title='" . lang("view_gift_card") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-eye\"></i></a> <a href='" . admin_url('system_settings/edit_bbq_menu/$1') . "' class='btn-warning btn-xs tip' title='" . lang("edit_bbq_menu") . "' ><i class=\"fa fa-edit\"></i></a> <a href='#' class='btn-danger btn-xs tip po' title='<b>" . lang("delete_bbq_menu") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_bbq_menu/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "bbq_menu.bbq_menu_id");
        //->unset_column('id');
        echo $this->datatables->generate();
    }
        
    function add_bbq_menu()
    {
    // $this->sma->checkPermissions();  
      if(!empty($_POST)){   
        $data = 0;
        $user = $this->input->post('adult_price', TRUE);
        foreach ($user as $u) {
            if ($u != '')
            {
                $data++;
        ...
        $this->form_validation->set_rules('name', lang("name"), 'required|is_unique[bbq_menu.name]');  
        $this->form_validation->set_rules('sale_type', lang("sale_type"), 'required');                
        if ($data == 0) { $this->form_validation->set_rules('adult_price', lang("adult_price"), 'required');    }
        if ($this->form_validation->run() == true) {         
            $data = array('name' => $this->input->post('name'),
                'sale_type' => $this->input->post('sale_type'),                
        ...
        if ($this->form_validation->run() == true && $this->settings_model->addBBQProcess($data,$bbq_days_price)) { 
            $this->session->set_flashdata('message', lang("bbq_menu_added"));
            admin_redirect("system_settings/add_bbq_menu/");            
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));            
            $this->data['bbqsale_type'] = $this->settings_model->getBBQSalestypes();
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings/bbq_menu'), 'page' => lang('bbq_menu')), array('link' => '#', 'page' => lang('bbq_lobster_discount')));
            $meta = array('page_title' => lang('bbq_lobster_discount'), 'bc' => $bc);
            $this->page_construct('settings/add_bbq_menu', $meta, $this->data);

            // $this->load->view($this->theme . 'settings/add_bbq_menu', $this->data);
        }
    }        

    function edit_bbq_menu($id = NULL)
    {         
        
        $menu_details = $this->settings_model->getBBQProcessByID($id);
      if(!empty($_POST)){           
          if ($this->input->post('name') != $menu_details->name) {
            $this->form_validation->set_rules('name', lang("name"), 'required|is_unique[bbq_menu.name]');
        }
        // $this->form_validation->set_rules('name', lang("name"), 'trim|required');
         $data = 0;
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($_FILES['userfile']['size'] > 0) {
             ...
             for($i=0; $i<count($this->input->post('days')); $i++){
                ...
                /*echo "<pre>";
                print_r($bbq_days_price);die;*/
        } elseif ($this->input->post('edit_bbq_menu')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/bbq_menu");
        }
}
        if ($this->form_validation->run() == true && $this->settings_model->updateBBQProcess($id, $data,$bbq_days_price)) { 
            $this->session->set_flashdata('message', lang("bbq_menu_updated"));
            admin_redirect("system_settings/bbq_menu");
        } else {

            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['bbqmenu'] = $this->settings_model->getBBQProcessByID($id);
            $this->data['bbqmenudaywiseprice'] = $this->settings_model->getBBQDaywisepriceByID($id);
            /*echo "<pre>";
            print_r($this->data['bbqmenu']);die;*/
            $this->data['bbqsale_type'] = $this->settings_model->getBBQSalestypes();            
            $this->data['id'] = $id;
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings/bbq_menu'), 'page' => lang('bbq_menu')), array('link' => '#', 'page' => lang('edit_bbq_menu')));
            $meta = array('page_title' => lang('edit_bbq_menu'), 'bc' => $bc);
            $this->page_construct('settings/edit_bbq_menu', $meta, $this->data);

            /*$this->load->view($this->theme . 'settings/edit_bbq_menu', $this->data);*/
        }
    }    

    public function view_bbq_menu($id = null)
    {
        $this->data['page_title'] = lang('view_bbq_menu');        
        $this->data['bbqmenu'] = $this->settings_model->getBBQProcessByID($id);
        $this->data['bbqmenudaywiseprice'] = $this->settings_model->getBBQDaywisepriceByID($id);
        $this->data['bbqsale_type'] = $this->settings_model->getBBQSalestypes();            
        $this->data['id'] = $id;
        $this->data['modal_js'] = $this->site->modal_js();
        $this->load->view($this->theme . 'settings/view_bbq_menu', $this->data);
    }    
  function delete_bbq_menu($id = NULL)
    {
    // $this->sma->checkPermissions();
        if ($this->settings_model->deleteBBQmenu($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("bbq_menu_deleted")));
        }
    }
    ...
    function shift_time_deactivate($id){
    // $this->sma->checkPermissions('discounts');
    $this->settings_model->updateShiftTime_status($id,0);
    redirect($_SERVER["HTTP_REFERER"]);
    }
    function shift_time_activate($id){
    // $this->sma->checkPermissions('discounts');
    $this->settings_model->updateShiftTime_status($id,1);
    redirect($_SERVER["HTTP_REFERER"]);
    }

 function recipe_gategory_deactivate($id){
    // $this->sma->checkPermissions('discounts');
    $this->settings_model->updateRecipeCategory_status($id,0);
    redirect($_SERVER["HTTP_REFERER"]);
    }
    function recipe_gategory_activate($id){
    // $this->sma->checkPermissions('discounts');
    $this->settings_model->updateRecipeCategory_status($id,1);
    redirect($_SERVER["HTTP_REFERER"]);
    }
