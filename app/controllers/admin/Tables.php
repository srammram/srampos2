...
class Tables extends MY_Controller{
	...
	$this->sma->checkPermissions('index',true);
        ...
        $this->datatables
            ->select("'sno',restaurant_tables.id AS id, warehouses.name as branch,restaurant_tables.name AS name, restaurant_tables.native_name,restaurant_tables.max_seats AS max_seats,  restaurant_areas.name AS area, warehouses.name AS warehouse,users.username")
    ...
    $this->sma->checkPermissions();
        ...
        if ($this->form_validation->run() == true) {
			$area=$this->site->getAreaByID($this->input->post('area_id'));
            $data = array(
    ...
    $this->sma->checkPermissions();
        ...
        $this->form_validation->set_rules('steward_id', lang("steward_id"), 'required|callback_isTableProcessing['.$id.']');
         ...
         if($table_status !='' && $table_status ==0 ){            
            $current_order_status =$table_status;
            $current_order_user =$table_status;
         }else{           
            $current_order_status =$table_details->current_order_status;
            $current_order_user =$table_details->current_order_user;
         }
        if ($this->form_validation->run() == true) {
			$area=$this->site->getAreaByID($this->input->post('area_id'));
            $data = array(
                ...
                'current_order_user' => $current_order_user,
                );
	...
	function areas(){
		$this->sma->checkPermissions();
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('tables'), 'page' => lang('tables')), array('link' => '#', 'page' => lang('table_area')));
        $meta = array('page_title' => lang('table_area'), 'bc' => $bc);
    ...
    function add_Kitchen(){
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'kitchen_consolid_printer_id' => $this->input->post('kitchen_consolid_printer_id'),
