...
report_view_access
~~~~~~~~~~~~~~~~~~
1 = taxation all reports show (print with don't print reports).
2 = only taxation included (print) reports show.
3 = only taxation excluded (don't print) reports show.
*/

      $usergrop_permission = $this->site->getGroupPermissions($this->session->userdata('group_id')); 
      $include_reports_view_access = 1;//$usergrop_permission->include_reports_view_access;
...
print_r($usergrop_permission->include_reports_view_access);die;*/
        ...
        if (!$this->loggedIn && $this->router->fetch_method()!="curl_delete_bill") {
        ...
        $this->pos_settings = $this->reports_model->getPOSSetting(); 
        $this->data['pb'] = array(
            'cash' => lang('cash'),
            'CC' => lang('CC'),
            'Cheque' => lang('Cheque'),
    ...
    function getQuantityAlerts($warehouse_id = NULL, $pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if ($warehouse_id) {
                ...
                $this->datatables
                    ->select('"sno",recipe.image, recipe.code, recipe.name, sum(st.stock_in-st.stock_out) as quantity, recipe.minimum_quantity')
                    ->from('recipe')
                    ->join('pro_stock_master st','st.product_id = recipe.id')
                    // ->where('recipe.alert_quantity > quantity', NULL)
                    ->where('recipe.track_quantity', 0)
                    // ->group_by('recipe.id')
                    ->group_by('srampos_recipe.id HAVING sum(st.stock_in-st.stock_out) <= srampos_recipe.minimum_quantity',FALSE);

                    // ->having('sum(recipe.alert_quantity > st.stock_in-st.stock_out)'); 
            }
...
//print_R($this->db);
  ...
  function recipe()
    ...
    function getItem_report_SubCategories()
    {
        $category_id = $this->input->post('category_id');
        if($category_id){
          $rows = $this->reports_model->getItemSubCategories_by_cat($category_id);
        }else{
            $rows = $this->reports_model->getItemSubCategories();
        }    
        if ($rows) {
    ...
    function getItem_by_SubCategories()
    {
        $subcategory_id = $this->input->post('subcategory_id');    
        if($subcategory_id){
            $rows = $this->reports_model->getItemsfromsub_cate($subcategory_id);
        }else{
            $rows = $this->reports_model->getItems();
        }
        if ($rows) {
            $data = json_encode($rows);
            } else {
   ...
   public function get_itemreports($start_date = NULL, $end_date = NULL, $warehouse_id = NULL, $varient_id = NULL){
  ...
  function pos_settlement(){       
      ...
      public function get_settlementreports($start_date = NULL, $end_date = NULL, $warehouse_id = NULL, $varient_id = NULL, $defalut_currency = NULL){
 ...
 public function get_kotdetailsreports($start_date = NULL, $end_date = NULL, $kot = NULL, $warehouse_id = NULL, $varient_id = NULL){
        ...
        if ($start != '' && $end != '' && $kot != ''&& $varient_id != '') {
             if($kot == 'kot_cancel') 
             {                
                $data = $this->reports_model->getKotCancelReport($start,$end,$warehouse_id,$varient_id,$limit,$offset);
             }
             elseif ($kot == 'kot_pending')
             {
              $data = $this->reports_model->getKotPendingReport($start,$end,$warehouse_id,$varient_id,$limit,$offset);  
             }
             else{                
                $data = $this->reports_model->getKotDetailsReport($start,$end,$warehouse_id,$varient_id,$limit,$offset);
             }
    ...
    function user_reports()
    {
        ...
        if($this->Settings->qsr == 1){
             // echo "1";die;
          $this->page_construct('reports/qsr_user_reports', $meta, $this->data);
        }else{
             // echo "2";die;
 ...
 public function get_qsruser_reports($start = NULL, $end = NULL, $user = NULL){
        $this->sma->checkPermissions('user_reports',true);
        $start = $this->input->post('start');
        $end = $this->input->post('end');
        $user = $this->input->post('user');
        ...
        $data= '';
        if ($start != '' && $end != '') {
            $data = $this->reports_model->getQSRCashierReport($start,$end,$user,$limit,$offset,$group,$this->report_view_access,$this->report_show);
            if (!empty($data['data'])){
    ...
    function shifttime_reports()
    {
        // $this->sma->checkPermissions();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();        
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['shifttime'] = $this->reports_model->getshifttime();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $this->data['groups'] = $this->site->getAllGroups();  
        ...
        $this->page_construct('reports/shift_time_report', $meta, $this->data);
    }    
 public function get_shifttime_reports($start = NULL, $end = NULL, $user = NULL){
        // $this->sma->checkPermissions('user_reports',true);
        $start = $this->input->post('start');
        $end = $this->input->post('end');
        ...
        $data= '';
        if ($start != '' && $end != '') {
            $data = $this->reports_model->getShiftReport($start,$end,$user,$limit,$offset,$shift,$defalut_currency,$this->report_view_access,$this->report_show);
            /*echo "<pre>";
            print_r($data);die;*/
            if (!empty($data['data'])){
        ...
        $total = $data['total'];
        $pagination = $this->pagination('reports/get_user_reports',$limit,$offsetSegment,$total);
        $this->sma->send_json(array('shift_report' => $shift_report,'pagination'=>$pagination));
   }   
...
public function get_DiscountSummary($start = NULL, $end = NULL, $dis_type = NULL, $warehouse_id = NULL){
   ...
   function void_bills()
    {
        ...
        if($this->Settings->qsr == 1){
          $this->page_construct('reports/qsr_void_bills', $meta, $this->data);
        }else{
  ...
  public function get_voidbills_reports($start = NULL, $end = NULL, $warehouse_id = NULL){
        ...
        if ($start != '' && $end != '') {
            if($this->Settings->qsr == 1){
            $data = $this->reports_model->getQSRVoidBillsReport($start,$end,$warehouse_id,$limit,$offset,$this->report_view_access,$this->report_show);  
             }else{
 ...
 function bill_details(){
        ...
        if($this->Settings->qsr == 1){
             $this->page_construct('reports/qsr_bill_details', $meta, $this->data);
        }else{
...
public function get_bill_no($start = NULL, $end = NULL, $warehouse_id=NULL){    
 ...
 public function get_bill_details_reports($start = NULL, $end = NULL, $bill_no = NULL, $warehouse_id = NULL, $varient_id = NULL){
...
ELSE 0 END)) paid_by,{$this->db->dbprefix('bils')}.paid  as amount")
                ->from('bils')
...
WHEN ((srampos_payments.paid_by = 'credit' AND srampos_sale_currency.currency_id=1 AND srampos_sale_currency.amount!='')) THEN (srampos_sale_currency.amount*srampos_sale_currency.currency_rate) ELSE 0 END)) paid_by,{$this->db->dbprefix('bils')}.paid  as For_Ex,{$this->db->dbprefix('bils')}.balance, {$this->db->dbprefix('payments')}.id as id,{$this->db->dbprefix('payments')}.bill_id")
                ->from('bils')
                ->join('payments', 'payments.bill_id = bils.id')
                ->join('sale_currency', 'sale_currency.bil_id = bils.id')
...
// SUM(bils.total-B.total_discount+CASE WHEN (B.tax_type= 1) THEN B.total_tax ELSE 0 END) AS grand_total

            $s = "( SELECT customer_id,payment_status, count(B.id) as total, COALESCE(sum(grand_total), 0) as total_amount, COALESCE(sum(paid), 0) as paid, ( COALESCE(sum(grand_total), 0) - COALESCE(sum(paid), 0)) as balance from {$this->db->dbprefix('bils')} B  GROUP BY B.customer_id ) FS";
...
// sales/payment_note/
   ...
   function getCustomerPostpaid_bills(){
    $this->sma->checkPermissions('postpaid_bills',TRUE);
        ...
        //if ($start != '' && $end != '') {
            ...
            $data = $this->reports_model->customer_postpaid_bills($customer_id,$limit,$offset,$start,$end);
            $totalAmount = $this->reports_model->GetPostpaid_BillDetails($customer_id,$billId='',$start,$end);
   ...
   function postpaid_payment($id,$billId=false){
     $this->form_validation->set_rules('amount-paid', lang("amount"), 'required');
        ...
        if ($this->form_validation->run() == true && $this->reports_model->postpaid_Addpayment($payment,$billId)) {
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['bills'] = $this->reports_model->GetPostpaid_BillDetails($id,$billId,$start='',$end='');
 ...
 function getCurrentStock_excel(){
   ...
   function purchase_return_reports()
    {
        $this->sma->checkPermissions();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();        
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('purchase_return_reports')));
        $meta = array('page_title' => lang('purchase_return_reports'), 'bc' => $bc);
        
        $this->page_construct('reports/pro_purchase_return_reports', $meta, $this->data);
    }

   public function get_purchase_return_reports(){
        ...
        if ($start != '' && $end != '') {
            ...
            $data = $this->reports_model->getPurchaseReturn_Report($start,$end,$warehouse_id,$limit,$offset);
        ...
        $total = $data['total'];
        $pagination = $this->pagination('reports/get_purchase_return_reports',$limit,$offsetSegment,$total);
        $this->sma->send_json(array('report' => $report_details,'pagination'=>$pagination));
   }
   function purchase_order_summary_reports()
    {
        $this->sma->checkPermissions();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();        
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('purchase_order_summary_reports')));
        $meta = array('page_title' => lang('purchase_order_summary_reports'), 'bc' => $bc);
        
        $this->page_construct('reports/pro_purchase_order_summary_reports', $meta, $this->data);
    }

   public function get_purchase_order_summary_reports(){
        ...
        if ($start != '' && $end != '') {
            ...
            $data = $this->reports_model->getPurchaseOrderSummary_Report($start,$end,$warehouse_id,$limit,$offset);
        ...
        $total = $data['total'];
        $pagination = $this->pagination('reports/get_purchase_order_summary_reports',$limit,$offsetSegment,$total);
        $this->sma->send_json(array('report' => $report_details,'pagination'=>$pagination));
   }
    function purchase_invoice_summary_reports()
    {
        $this->sma->checkPermissions();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();        
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('purchase_invoice_summary_reports')));
        $meta = array('page_title' => lang('purchase_invoice_summary_reports'), 'bc' => $bc);
        
        $this->page_construct('reports/pro_purchase_invoice_summary_reports', $meta, $this->data);
    }

   public function get_purchase_invoice_summary_reports(){
        ...
        if ($start != '' && $end != '') {
            ...
            $data = $this->reports_model->getPurchaseInvoiceSummary_Report($start,$end,$warehouse_id,$limit,$offset);
        ...
        $total = $data['total'];
        $pagination = $this->pagination('reports/get_purchase_invoice_summary_reports',$limit,$offsetSegment,$total);
        $this->sma->send_json(array('report' => $report_details,'pagination'=>$pagination));
   }
    function bills()
    {
        ...
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('bills_report')));
        $meta = array('page_title' => lang('bills_report'), 'bc' => $bc);
        
        $this->page_construct('reports/bills', $meta, $this->data);
    }    

    public function get_bills_reports($start = NULL, $end = NULL, $bill_no = NULL, $warehouse_id = NULL){
        $this->sma->checkPermissions('bill_details',TRUE);
        $start = $this->input->post('start');
        $end = $this->input->post('end');
        $bill_no = $this->input->post('bill_no');
        ...
        $table_whitelisted = $this->input->post('table_whitelisted');
        if ($start != '' && $end != '') {
            $data = $this->reports_model->getBillsReport($start,$end,$bill_no,$warehouse_id,$limit,$offset,$this->report_view_access,$this->report_show);
            if (!empty($data['data'])){
                 $bill = $data['data'];
             }
             else{
        ...
        $total = $data['total'];
        $pagination = $this->pagination('reports/get_bills_reports',$limit,$offsetSegment,$total);
        $this->sma->send_json(array('bill_details' => $bill,'pagination'=>$pagination));
   ...
   function delete_bills(){
    ...
    $this->reports_model->DeleteBill_data($bills);
    echo json_encode(array('success'));
   }
   
   
  function modify_bills()
    {
        $this->sma->checkPermissions();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('modify_bills')));
        $meta = array('page_title' => lang('modify_bills'), 'bc' => $bc);
        
        
        $this->page_construct('reports/modify_bills', $meta, $this->data);
        
    }    
 

 public function get_bill_reports($start = NULL, $end = NULL, $bill_no = NULL, $warehouse_id = NULL, $varient_id = NULL){
        $this->sma->checkPermissions('modify_bills',TRUE);
        $start = $this->input->post('start');
        $end = $this->input->post('end');
        $bill_no = $this->input->post('bill_no');
        $warehouse_id = $this->input->post('warehouse_id');
        ...
        $total = $data['total'];
        $pagination = $this->pagination('reports/get_bill_reports',$limit,$offsetSegment,$total);
        $this->sma->send_json(array('bill_details' => $bill,'pagination'=>$pagination,'p_total'=>$p_total,'dp_total'=>$dp_total));
        
   }
   function deleteDontPrintBill($bill_id){
    $bill_ids[] = $bill_id;
    $this->reports_model->deleteDontPrintBill($bill_ids);
    echo 1;
   }
   function change_toPrint($bill_id){
   
    $this->reports_model->change_toPrintBill($bill_id);
    
   }
   function change_toDontPrint($bill_id){
    $this->reports_model->change_toDontPrintBill($bill_id);
   }    
   function edit_dontprint($bill_id){      
    //$this->sma->checkPermissions();		
	...
	if(isset($_POST['recipe_id'])){
		...
		$bill_items[$i]['net_unit_price'] = $_POST['net_unit_price'][$i];
		$bill_items[$i]['unit_price'] = $_POST['unit_price'][$i];
		...
		$bill_items[$i]['discount'] = $_POST['item_discount_id'][$i];
		$bill_items[$i]['service_charge_amount'] = (isset($_POST['item_sc'][$i]))?$_POST['item_sc'][$i]:0;
        $bill_items[$i]['service_charge_id'] = (isset($_POST['bill_sc_id']))?$_POST['bill_sc_id']:0;
		$bill_items[$i]['service_charge_id'] = $this->session->userdata('warehouse_id');

		//to update order items
		$order_items[$i]['update']['id'] = $_POST['order_item_id'][$i];
		$order_items[$i]['update']['sale_id'] = $_POST['order_id'][$i];	
		...
		if($_POST['ordered_quantity'][$i]>$_POST['quantity'][$i]){
		   // to insert cancelled order items
		    $order_items[$i]['insert'] = $this->reports_model->getOrderItemsDetails($_POST['order_item_id'][$i]);
	    ...
	    $bill_data['service_charge_amount'] = (isset($_POST['bill_sc']))?$_POST['bill_sc']:0;
	    $bill_data['service_charge_id'] = (isset($_POST['bill_sc_id']))?$_POST['bill_sc_id']:0;
	...
	//print_R($_POST);
	//exit;
	$this->reports_model->edit_bill($bill_id,$sale_id,$bill_data,$sale_data,$bill_items,$order_items,$payment,$multi_currency);
	
	$this->session->set_flashdata('message', lang("bill_edited"));
        admin_redirect('reports/modify_bills/');
        // admin_redirect('reports/edit_dontprint/'.$bill_id);
    }
    
    $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports/modify_bills'), 'page' => lang('bills')), array('link' => '#', 'page' => lang('modify_bill')));
    $meta = array('page_title' => lang('bill_details_report'), 'bc' => $bc);
    $this->data['bill'] = $this->reports_model->get_dontprintBillData($bill_id);
    $this->data['payments'] = $this->reports_model->get_BillPaymentData($this->data['bill']->id);
    $this->data['off_dis'] = $this->reports_model->getOfferdiscount($this->data['bill']->order_discount_id);
    $this->data['bill_tax'] = $this->reports_model->getBillTax($this->data['bill']->tax_id);
    $this->data['customer_discount'] = $this->site->GetAllcostomerDiscounts();
    $this->data['categories'] = $this->site->getAllrecipeMappedCategories();
    $this->data['all_subcategories'] = $this->site->getAllrecipeMappedSubCategories();
    //echo '<pre>';print_R($this->data['all_subcategories']);exit;
    $view_file = 'edit_bill';
    
    if($this->data['bill']->order_type==4){
	$view_file = 'edit_bbq_bill';
    }
    
    //echo '<pre>';print_R($this->data['bill']);exit;
    $this->page_construct('reports/'.$view_file, $meta, $this->data);
   }
   function getCustomerDiscount(){
	...
	$discount[$id] = $this->reports_model->getCategory_GroupDiscount($category_id,$subcategory_id,$id,$discountid);	
	//print_R($discount);
    }
    echo json_encode($discount);
    
   }
  function restore_bills()
    {
        $this->sma->checkPermissions();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('restore_bills')));
        $meta = array('page_title' => lang('restore_bills'), 'bc' => $bc);
        
        
        $this->page_construct('reports/archived_bills', $meta, $this->data);
        
    }    
 

 public function get_archived_bills($start = NULL, $end = NULL, $bill_no = NULL, $warehouse_id = NULL, $varient_id = NULL){
        $this->sma->checkPermissions('restore_bills',TRUE);
        $start = $this->input->post('start');
        $end = $this->input->post('end');
        $bill_no = $this->input->post('bill_no');
        $warehouse_id = $this->input->post('warehouse_id');
	...
	$this->session->set_userdata('list_limit',$this->input->post('pagelimit'));
        ...
        if ($start != '' && $end != '') {
            ...
            $data = $this->reports_model->getArchived_BillReports($start,$end,$bill_no,$warehouse_id,$bill_action,$limit,$offset);  
            $p_total = $data['print_total'];
	    $dp_total = $data['dontprint_total'];
            if (!empty($data['data'])){
                 $bill = $data['data'];
        ...
        $total = $data['total'];
        $pagination = $this->pagination('reports/get_archived_bills',$limit,$offsetSegment,$total);
        $this->sma->send_json(array('bill_details' => $bill,'pagination'=>$pagination));
        
   }
    function restore_deleted_bill($billid){
     
     $this->reports_model->restore_deleted_bill($billid);
     $this->session->set_flashdata('message', lang("bill_restored"));
     admin_redirect('reports/restore_bills');
    }
    function restore_modified_bill($billid){
     $this->reports_model->restore_modified_bill($billid);
     $this->session->set_flashdata('message', lang("bill_restored"));
     admin_redirect('reports/restore_bills');
    }
    function deletebills(){
        $bill_ids = $this->input->post('bills');
        $this->reports_model->updateDeleteAction_bill($bill_ids);
      
        $ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, admin_url('reports/curl_delete_bill'));
	curl_setopt($ch, CURLOPT_FRESH_CONNECT, true);
	curl_setopt($ch,CURLOPT_POST, 1);                //0 for a get request
	curl_setopt($ch,CURLOPT_POSTFIELDS,json_encode($bill_ids));
	...
	ob_end_clean();
        ...
        @ob_end_flush();
        flush();
	$bill_ids = json_decode(file_get_contents('php://input'));
	
	foreach($bill_ids as $k => $billid){
	    //file_put_contents('delete'.$k.'.txt',$billno);
	    $this->deleteDontPrintBill($billid);
	}	
    }
    function changeBills_toPrint(){
	$billids = $this->input->post('bills');
	foreach($billids as $k => $billid){
	   $this->change_toPrint($billid);
	}
 ...
 function bill_reprint()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('bill_reprint_report')));
        $meta = array('page_title' => lang('bill_reprint_report'), 'bc' => $bc);
        
       
        $this->page_construct('reports/bill_reprint_report', $meta, $this->data);
        
    }
    public function get_bill_reprint_reports($start = NULL, $end = NULL, $bill_no = NULL, $warehouse_id = NULL, $varient_id = NULL){
        //$this->sma->checkPermissions('bill_reprint',TRUE);
        $start = $this->input->post('start');
        $end = $this->input->post('end');
        $bill_no = $this->input->post('bill_no');
        $warehouse_id = $this->input->post('warehouse_id');
        ...
        if ($start != '' && $end != '') {
            ...
            $data = $this->reports_model->getBillReprintReport($start,$end,$bill_no,$warehouse_id,$limit,$offset,$this->report_view_access,$this->report_show);  
        ...
        $total = $data['total'];
        $pagination = $this->pagination('reports/get_bill_reprint_reports',$limit,$offsetSegment,$total);
        $this->sma->send_json(array('reports' => $bill,'pagination'=>$pagination));
        
   }
   
    function auto_modify_bills()
    {
        $this->sma->checkPermissions();
	
	if(isset($_POST['submit_search'])){
	    $this->session->unset_userdata('bill_no');
	    $this->session->unset_userdata('bill_no_from');
	    $this->session->unset_userdata('bill_no_to');
	    
	    
	    $search['start_date'] = $this->input->post('start_date');
	    
	    $search['end_date'] = $this->input->post('end_date');
	    $search['target_amount'] = $this->input->post('target_amount');
	    $search['bill_search_type'] = $this->input->post('bill_search_type');
	    $this->session->set_userdata('start_date',$this->input->post('start_date'));
	    $this->session->set_userdata('end_date',$this->input->post('end_date'));
	    $this->session->set_userdata('target_amount',$this->input->post('target_amount'));
	    $this->session->set_userdata('bill_search_type',$this->input->post('bill_search_type'));
	    if($search['bill_search_type']=='single'){
		$search['bill_no'] = $this->input->post('bill_no');
		$this->session->set_userdata('bill_no',$this->input->post('bill_no'));
	    }else if($search['bill_search_type']=='range'){
		$search['bill_no_from'] = $this->input->post('bill_no_from');
		$search['bill_no_to'] = $this->input->post('bill_no_to');
		$this->session->set_userdata('bill_no_from',$this->input->post('bill_no_from'));
		$this->session->set_userdata('bill_no_to',$this->input->post('bill_no_to'));
	    }
	    ...
	    $item_ids = array();
	    if(isset($_POST['group'][0]['recipe_group_id'])){
		foreach($_POST['group'][0]['recipe_group_id'] as $k => $category){
		   
		    foreach($category['sub_category'] as $kk => $subcategory){//echo '<pre>';print_R($subcategory);
			
			foreach($subcategory['recipes'] as $kkk => $item){//echo '<pre>';print_R($item);
			    array_push($item_ids,$item);
			    if(!in_array($subcategory['id'],$subcategory_ids)){
		...
		$search['subcategory_ids'] = $subcategory_ids;
	    }
	    if(!empty($item_ids)){
	    ...
	    $data = $this->reports_model->GetBillsToModify($search);
	    
	$d['target_amount'] = $search['target_amount'];
	    $d['bill_list'] = array();
	    $cnt = 0;
	    $bill_items = array();
	    $bill =  array();	   
	    foreach($data as $k => $row){
		$bill[$row->bill_id]['delete_bill_items'][] = $row->id;
	    }
	    foreach($bill as $k => $row){
		$d['bill_list'][$cnt] = $this->reports_model->getBillDetails($k);
		$d['bill_list'][$cnt]->del_bill_items = $this->reports_model->getBillItems_ids($k,$row['delete_bill_items']);
		$d['bill_list'][$cnt]->remaining_bill_items = $this->reports_model->getBillItems_not_in_ids($k,$row['delete_bill_items']);
		
		$cnt++;
	    }
	    
	    $list_html = $this->load->view($this->theme . 'reports/modify_bill_list', $d, TRUE);
	    echo json_encode(array('data'=>$data,'list'=>$list_html));exit;
	}
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('auto_modify_bills')));
        $meta = array('page_title' => lang('auto_modify_bills'), 'bc' => $bc);
	...
	$this->data['recipe_groups_json'] = json_encode($this->data['recipe_groups']);
        $this->page_construct('reports/auto_modify_bills', $meta, $this->data);
        
    }
    function initiate_auto_modification(){
	$bills =  $_POST['bill'];
	$this->site->auto_modify_bills($bills);	
	echo json_encode(array('status'=>'success'));
    }
	...
	$data = $this->reports_model->fetch_recipe($category_id,$subcategory_id);
...
GROUP BY B.id, PD.identify*/
    ...
    function items_mapping_for_modify_bills(){
    $this->sma->checkPermissions();
    if ($this->input->post('update')) {         
        ...
        foreach($group['sub_category'] as $s_k => $subgroup){
            ...
            $recipe_array[$index]['category_id'] = $group['id'];
	    $recipe_array[$index]['subcategory_id'] = $subgroup['id'];
	    $recipe_array[$index]['recipe_id'] = $recipe;
	    $index++;
            }
        }
        }
       
        $return = $this->reports_model->updateRecipe_feedbackMapping($recipe_array);
        if ($return) {
        $this->session->set_flashdata('message', lang("recipe_mapping_updated"));
        admin_redirect("reports/items_mapping_for_modify_bills");
        }
        } 
    $this->data['recipe_groups'] = $this->site->getAllrecipeCategories_items();
    $this->data['recipe_groups_json'] = json_encode($this->data['recipe_groups']);
    $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
    $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('items_mapping_for_modify_bills')));
    $meta = array('page_title' => lang('items_mapping_for_modify_bills'), 'bc' => $bc);
  ...
  function wastage(){        
        ...
        $this->data['billers'] = $this->site->getAllCompanies('biller');
		$this->data['varients'] = $this->recipe_model->getAllvarients();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('wastage_report')));
        $meta = array('page_title' => lang('wastage_report'), 'bc' => $bc);
        
        $this->page_construct('reports/wastage', $meta, $this->data);
    }
