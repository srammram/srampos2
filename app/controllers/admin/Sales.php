...
class Sales extends MY_Controller
{
    ...
    public function suggestions()
    {
        ...
        if($this->pos_settings->sales_item_in_pos == 1){
            ...
            $rows = $this->sales_model->getProductNames_new($sr, $warehouse_id, $recipe_standard);
        }		
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                if($row->variant_id != 0 ){
                    $addons = $this->sales_model->getrecipeVariantAddons($row->variant_id,$row->id);
                    $customizable = $this->sales_model->getrecipeVariantCustomizable($row->variant_id,$row->id);
                }else{
                    $addons = $this->sales_model->getrecipeAddons($row->id);  
                    $customizable = $this->sales_model->getrecipeCustomizable($row->id);
                }				
                if ($options) {
               ...
               // $row->price = $row->price + (($row->price * $customer_group->percent) / 100);
                ...
                if($row->price !=0){
                    ...
                    $row->base_unit = $row->variant_price;
                    $row->base_unit_price = $row->variant_price;
                    $row->unit = $row->variant_price;
                    $row->price =$row->variant_price + $row->tax_amount;
                    $row->real_unit_price = $row->variant_price;
                }
                $row->comment = '';
                $combo_items = false;				
                if ($row->type == 'combo') {
                ...
                $pr[] = array('id' => ($c + $r), 'item_id' => $row->id.$variant_id, 'label' => $row->recipe_name . " (" . $row->code . ")", 'category' => $row->category_id,
                    'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'addons' => $addons,'variant'=>$row->variant_name,'variant_id'=>$row->variant_id, 'customizable_ingrediends' => $customizable);
                $r++;
            }
