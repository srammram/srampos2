...
class Purchase_orders extends MY_Controller{
	...
	public function purchase_orders_list(){
		...
		foreach ($inv_items as $item) {
            ...
            $row->item_tax_rate = '0';
            $row->item_selling_price = $item->selling_price ? $item->selling_price:0;
            $row->discount = '0';
    	    $row->item_dis_type='p';
    ...
    public function add($purchase_invoices_id = null){
        ...
        if ($this->form_validation->run() == true) {  
	     //echo '<pre>';print_R($_POST);exit;
            ...
            'note' => $this->sma->clear_tags($this->input->post('note')),
            'tax_method' => $this->input->post('tax_method'),
            'shipping' => ($this->input->post('feright_chargers_shipping'))?$this->input->post('feright_chargers_shipping'):0,
            'bill_disc' => $this->input->post('bill_disc'),		
            'round_off' => $this->input->post('round_off'),
            'supplier_address' => $this->input->post('supplier_address'),
            'status' => $this->input->post('status'),
   ...
   public function delete($id = null)
    {
        ...
        $delete_check = $this->purchase_orders_model->checkpurchase_ordersstatus($id);
        if($delete_check == FALSE){
          ...
          $this->sma->send_json(array('error' => 1, 'msg' => lang("could_not_be_delete_this_purchase_orders")));  
        }    
    }
