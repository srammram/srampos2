...
class Customers extends MY_Controller
{
    ...
    function view_customer($id = NULL)
    {
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['customer'] = $this->companies_model->getCompanyByID($id);
        $this->load->view($this->theme.'customers/view_customer',$this->data);
    }
    ...
    function add()
    {
		...
		'allow_loyalty'=>$this->input->post('allow_loyalty'),
            );
        ...
        if ($this->form_validation->run() == true && $cid = $this->companies_model->addCompany($data)) {
			...
			if(!empty($ref[0]) && !empty($ref[1]) && !empty($ref[2])){
				admin_redirect($ref[0].'?'.$ref[1].'&'.$ref[2].'&customer=' . $cid);
			}elseif(!empty($ref[0]) && !empty($ref[1]) && empty($ref[2])){
				admin_redirect($ref[0].'?'.$ref[1].'&customer=' . $cid);
			}elseif(!empty($ref[0]) && empty($ref[1]) && empty($ref[2])){
	...
	function new_customer()
    {
        ...
        if ($this->form_validation->run() == true && $cid = $this->companies_model->addCompany($data)) {
           $customer_details = $this->site->getCompanyByID($cid);
		   $result['success'] = array("new_customer_id"=>$cid,"name"=>$customer_details->name);
		  /* $result['new_customer_id'] = $cid;
			   $result['name'] = $customer_details->name;
			   $result['msg'] = 'success';*/
            ...
            $this->data['price_groups'] = $this->companies_model->getAllPriceGroups();
			 ...
			 $this->load->view($this->theme . 'customers/new_customer', $this->data);
    ...
    function add_pos()
    {
        ...
        if ($this->form_validation->run() == true && $cid = $this->companies_model->addCompany($data)) {
            $this->session->set_flashdata('message', lang("customer_added"));
             ...
             $response['success'] = array("id"=>$cid,"name"=>$data['name']);
             //$response['cname'] = $data['name'];
            ...
            $this->data['customer_groups'] = $this->companies_model->getAllCustomerGroups();
            $this->data['price_groups'] = $this->companies_model->getAllPriceGroups();
            $this->load->view($this->theme . 'customers/add_pos', $this->data);
        }
    }
    ...
    {//print_R($_POST);exit;
        ...
        if($_POST['customer_type']=="postpaid"){
    ...
    { 
      ...
      //  $this->sma->checkPermissions(false, true);
        ...
        if ($this->form_validation->run() == true && $this->companies_model->updateCompany($id, $data)) {
           ...
           $result['success'] = array("new_customer_id"=>$id,"name"=>$customer_details->name);
            ...
            $this->load->view($this->theme . 'customers/edit_customer', $this->data);
        }
    }
...
function suggestions_new($term = NULL, $limit = NULL)
    {
        // $this->sma->checkPermissions('index');
        if ($this->input->get('term')) {
            $term = $this->input->get('term', TRUE);
        ...
        $results = $this->companies_model->getCustomerSuggestions($term, $limit);
               ...
               $row->price =$row->price + $row->tax_amount;
                ...
                $pr[] = array('id' => ($c + $r), 'item_id' => $row->id, 'label' => $row->text,
                    'row' => $row);
                $r++;
            }
    ...
    function get_deposits($company_id = NULL)
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("deposits.id as id, date, credit_amount,credit_balance,credit_used,return_amount, paid_by, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as created_by", false)
            ->from("deposits")
    ...
    function add_deposit($company_id = NULL)
    {
        ...
        $this->form_validation->set_rules('credit_amount', lang("credit_amount"), 'required|numeric');
		...
		'credit_balance' => $this->input->post('credit_amount'),
    ...
    function edit_deposit($id = NULL)
    {
        ...
        $this->form_validation->set_rules('credit_amount', lang("credit_amount"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                'date' => $date,
                'credit_amount' => trim($this->input->post('credit_amount'),'$'),
                'paid_by' => $this->input->post('paid_by'),
                'note' => $this->input->post('note'),
	    ...
	    $rdata = array();
	    if($data['paid_by']=="prepaid"){
		
		$rdata['return_amount'] = $this->input->post('return_amount');
        ...
        if ($this->form_validation->run() == true && $this->companies_model->updateDeposit($id, $data, $cdata,$rdata)) {
