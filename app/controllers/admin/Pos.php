...
public function bbq_reprint_view()
  ...
  function kot_print_copy($split_id,$kitchen_id=false){
  ...
  function kitchen_kot_print_copy($order_id,$kitchen_id){
	...
	if(!empty($table_id)){
		...
		foreach($split->split_order as $kk => $split_order){
			...
			foreach($order->item as $ik => $item){
			    ...
			    if($item_data->item_status=="Cancel") continue;
			    $customer_id = $this->site->getOrderItemCustomer($order_item_id);
				 ...
				 if(!empty($varients)){
				 	...
				 	if($this->pos_settings->variant_display_option == 0){						
						...
						foreach($varients as $k => $varient){
							...
							$vari .= "<td><span class='name_strong'>" .$varient->variant_code. "</span></td>";
...
public function getSalesItems($sale_id = NULL)
	...
	function gatdata_sugar_plam_print_billing()
	{	
...
//    }
    ...
    function manualsplit_calculate_customerdiscount(){
...
print_r($data);die;*/
 ...
 public function get_bill_reports($start = NULL, $end = NULL, $bill_no = NULL, $warehouse_id = NULL, $varient_id = NULL){
   ...
   function deleteDontPrintBill($bill_no){
    $bill_nos[] = $bill_no;
    $this->pos_model->deleteDontPrintBill($bill_nos);
    echo 1;
   }
   function change_toPrint($bill_no){
   
    $this->pos_model->change_toPrintBill($bill_no);
    echo 1;
   }
   function edit_dontprint($bill_no){      
    //$this->sma->checkPermissions();		
