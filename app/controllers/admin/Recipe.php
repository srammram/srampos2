            ...
            if ($this->input->post('type') == 'addon' || $this->input->post('type') == 'production' || $this->input->post('type') == 'semi_finished') {
				for($j=0; $j<count($this->input->post('purchase_item[id]')); $j++){
					$recipe_pro[] = array(
						...
						'cm_id' => $this->input->post('purchase_item[cm_id]['.$j.']'),
...
function clean($string) {
   ...
   return preg_replace('/-+/', '-', $string); // Replaces multiple hyphens with single one.
       ...
       /* $this->form_validation->set_rules('slug', lang("slug"), 'required|alpha_dash');
        ...
        if ($this->form_validation->run('recipe/edit') == true) {
           $data = array(
				...
				'khmer_name' => $this->input->post('khmer_name'),
                // 'khmer_image' => str_replace(' ', '-',$this->input->post('name')).'.png',
				'khmer_image' => $this->clean($this->input->post('name')).'.png',
                'barcode_symbology' => $this->input->post('barcode_symbology') ? $this->input->post('barcode_symbology') : 'code25',
				'name' => $this->input->post('name'),
				'currency_type' => $this->input->post('currency_type'),
	    ...
	    if($this->input->post('type') == 'combo'){
            ...
            $filename = 'assets/language/'.$this->clean($_POST['name']).'.png';
            $this->base64ToImage($_POST['recipe_name_img'],$filename);
    	    /*$filename = 'assets/language/'.str_replace(' ', '-',$_POST['name']).'.png';
    	    $this->base64ToImage($_POST['recipe_name_img'],$filename);*/
	...
	function mapping_download_sample(){
		$filename = 'ingredients-mapping-import.csv';
		header('Content-Type: application/csv');
			header('Content-Disposition: attachment; filename="'.$filename.'";');
		$header = array('item_name', 'ingredients_name', 'item_type','quantity','uom');
		$fp = fopen('php://output', 'w');       
			fputcsv($fp, $header);
		fclose($fp);    
    }
...
/*recipe variant start*/
   ...
   function add_varient(){
    ...
    //$this->sma->checkPermissions('categories');        
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',{$this->db->dbprefix('recipe_variants_values')}.id as id,r.name as item_name,rv.name AS variant_name, r.khmer_name as item_native_name, rv.native_name AS variant_native_name", FALSE)
            ->from("recipe_variants_values")
            ->join('recipe_variants as rv','rv.id=recipe_variants_values.attr_id','left')
            ->join('recipe as r','r.id=recipe_variants_values.recipe_id','left')
            ->add_column("Actions", "<div class=\"text-center\"> <a href='" . admin_url('recipe/edit_item_with_varient/$1') . "'  class='tip' title='" . lang("edit_item_with_varient") . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_category") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('recipe/delete_varient/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
...
function edit_item_with_varient($id){
    $this->sma->checkPermissions();
        ...
        if ($this->form_validation->run() == true) {
            ...
            $image_name =$this->input->post('item_name') . '' . $this->input->post('variant_name');
            $data = array(                
        ...
        if ($this->form_validation->run() == true &&  $this->recipe_model->update_recipe_variants_values($id,$data)) {
            $data['id'] = $id;
            $response['varient'] = $data;
        $this->session->set_flashdata('message', lang("Varient_updated"));

            $filename = 'assets/language/'.str_replace(' ', '-',$image_name).'.png';
            $this->base64ToImage($_POST['recipe_name_img'],$filename);
            echo json_encode($response);exit;
            ...
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['modal_js'] = $this->site->modal_js();
        $this->data['variant'] = $this->recipe_model->getItemVariantbyID($id);
           $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('recipe/variants'), 'page' => lang('variants')), array('link' => '#', 'page' => lang('edit_varient')));
            $meta = array('page_title' => lang('edit_item_with_variant'), 'bc' => $bc);
            $this->page_construct('recipe/edit_item_with_variant', $meta, $this->data);
        }
    }
	...
	function search_purchase_items_new(){
		$term = $this->input->post('term');
		$type = $this->input->post('item_type');
		$existing = $this->input->post('existing');
		//echo '<pre>';print_r($_POST);exit;
		$result = $this->recipe_model->getPurchase_items_new($term,$existing,$type);
		$this->sma->send_json($result);
    }
 ...
 function edit_ingredients($id = NULL){
		...
		$this->form_validation->set_rules('type', lang("type"), 'xss_clean');		
			if ($this->form_validation->run('recipe/edit_ingredients') == true) {                     
				...
				$this->page_construct('recipe/edit_ingredients', $meta, $this->data);
    ...
    public function ingredients_view($id = null){        
        ...
        $this->load->view($this->theme . 'recipe/ingredients_view', $this->data);
...
/*recipe and variant add on mapping start*/    
   function recipe_and_variant_addon(){
    $this->sma->checkPermissions();
        ...
        $meta = array('page_title' => lang('recipe_and_variant_addon'), 'bc' => $bc);
    ...
    function recipe_and_variant_addon_add(){        
        ...
        if ($this->form_validation->run() == true && $this->recipe_model->recipe_variant_addoninsert($add_on,$addon_items)){            
            $this->session->set_flashdata('message', lang("recipe_and_variant_addon_added"));
            admin_redirect('recipe/recipe_and_variant_addon');
        } else {

            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['Allrecipe'] = $Allrecipe;
            $this->data['units'] = $this->site->getAllUnits();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('recipe'), 'page' => lang('recipe')), array('link' => '#', 'page' => lang('recipe_and_variant_addon_add')));
            $meta = array('page_title' => lang('recipe_and_variant_addon_add'), 'bc' => $bc);
            $this->page_construct('recipe/recipe_and_variant_addon_add', $meta, $this->data);
    ...
    function edit_recipe_and_variant_addon($id = NULL){
        ...
        if ($this->form_validation->run() == true && $this->recipe_model->updaterecipe_variant_addon($id, $add_on,$addon_items)) {              
            $this->session->set_flashdata('message', lang("recipe_and_variant_addon_updated"));
            admin_redirect('recipe/recipe_and_variant_addon');
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['Allrecipe'] = $Allrecipe;  
            $this->data['addon'] = $recipevariantaddon;
            $this->data['recipevariants'] = $recipevariants;              
            $this->data['addon_item'] = json_encode($item);    
            /*echo "<pre>";       
            print_r($this->data['addon_item']);die;*/
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('recipe/recipe_and_variant_addon'), 'page' => lang('edit_recipe_and_variant_addon')), array('link' => '#', 'page' => lang('edit_recipe_and_variant_addon')));            
            $meta = array('page_title' => lang('edit_recipe_and_variant_addon'), 'bc' => $bc);
            $this->page_construct('recipe/edit_recipe_and_variant_addon', $meta, $this->data);
        }
    }
   function delete_recipe_and_variant_addon($id){
    $this->recipe_model->delete_recipe_and_variant_addon($id);
    $this->session->set_flashdata('message', lang("Varient_added"));
    $this->sma->send_json(array('error' => 0, 'msg' => lang("varient_deleted")));
    }
   ...
   public function getrecipevariants(){
    ...
    public function get_addon_suggestions(){
        ...
        if ($rows) {
            foreach ($rows as $row) {
                $pr[] = array('id' => $row->id, 'label' => $row->name,  'name' => $row->name, 'code' => $row->code);
            }
            $this->sma->send_json($pr);
        } else {
            $this->sma->send_json(array(array('id' => 0, 'label' => lang('no_match_found'), 'value' => $term)));
