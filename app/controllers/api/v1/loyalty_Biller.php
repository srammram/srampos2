...
//die;	
	...
	/*hide before loyalty 18-12-2018*/
	public function consolidatedbildata_post_before_loyalty()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			if($devices_check == $devices_key){	
				...
				if(!empty($data)){
					$result = array( 'status'=> true , 'message'=> lang('biller_list_in_data'),'message_khmer'=> html_entity_decode(lang('biller_list_in_data_khmer')), 'data' => $data, 'customer_request' => $customer_request, 'dine_bil' => $dine_bil ? $dine_bil : 0, 'bbq_bil' => $bbq_bil ? $bbq_bil : 0, 'automatic' => $automatic, 'automatic_discount' => $automatic_discount, 'is_unique' => $is_unique, 'taxation' => $taxation);
	...
	public function consolidatedbildata_post()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			if($devices_check == $devices_key){	
				...
				if(!empty($data)){
					$result = array( 'status'=> true , 'message'=> lang('biller_list_in_data'),'message_khmer'=> html_entity_decode(lang('biller_list_in_data_khmer')), 'data' => $data, 'customer_request' => $customer_request, 'dine_bil' => $dine_bil ? $dine_bil : 0, 'bbq_bil' => $bbq_bil ? $bbq_bil : 0, 'automatic' => $automatic, 'automatic_discount' => $automatic_discount, 'is_unique' => $is_unique, 'taxation' => $taxation, 'loyalty_available' => $loyalty_available);
