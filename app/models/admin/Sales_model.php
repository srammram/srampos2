...
class Sales_model extends CI_Model
{
    ...
    {        
        ...
        $this->db->select('recipe.*,CONCAT('.$this->db->dbprefix("recipe").'.name," ",IFNULL('.$this->db->dbprefix("recipe_variants").'.name,"")) as recipe_name, recipe_categories.id as category_id, recipe_categories.name as category_name,recipe_variants.name as variant_name,recipe_variants.id as variant_id,recipe_variants.native_name as variant_khmer_name, recipe_variants_values.price as variant_price', FALSE)
	        ...
	        ->join('recipe_variants_values', 'recipe_variants_values.recipe_id=recipe.id', 'left')
            ->join('recipe_variants', 'recipe_variants.id=recipe_variants_values.attr_id', 'left');
            //->group_by('recipe.id');
     ...
     public function getProductNames_new($term, $warehouse_id,$recipe_standard,$sale_type=1)
    {
        ...
        $this->db->select('recipe.*,CONCAT('.$this->db->dbprefix("recipe").'.name," ",IFNULL('.$this->db->dbprefix("recipe_variants").'.name,"")) as recipe_name,'.$this->db->dbprefix("recipe_variants").'.name as variant,'.$this->db->dbprefix("recipe_variants").'.id as variant_id,'.$this->db->dbprefix("recipe_variants").'.native_name as variant_khmer_name, recipe_categories.id as category_id, recipe_categories.name as category_name, recipe_variants_values.price as variant_price', FALSE);
        ...
        $this->db->join('recipe_categories', 'recipe_categories.id=recipe.category_id', 'left');      
        $this->db->join('recipe_variants_values', 'recipe.id=recipe_variants_values.recipe_id', 'left');
        $this->db->join('recipe_variants', 'recipe_variants.id=recipe_variants_values.attr_id', 'left');
...
// print_r($this->db->last_query());die;
        ...
        /*$limit = 5;
	$where_in = array('standard','production','quick_service','combo');
        $this->db->select('recipe.*,CONCAT('.$this->db->dbprefix("recipe").'.name," ",IFNULL('.$this->db->dbprefix("recipe_variants").'.name,"")) as recipe_name,'.$this->db->dbprefix("recipe_variants").'.name as variant,'.$this->db->dbprefix("recipe_variants").'.id as variant_id, recipe_categories.id as category_id, recipe_categories.name as category_name, recipe_variants_values.price as variant_price', FALSE)
          
            ->join('recipe_categories', 'recipe_categories.id=recipe.category_id', 'left')
	    ->join('recipe_variants_values', 'recipe.id=recipe_variants_values.recipe_id', 'left')
	    ->join('recipe_variants', 'recipe_variants.id=recipe_variants_values.attr_id', 'left')
	    ->where_in('recipe.type',$where_in)
            ->group_by('recipe.id');
