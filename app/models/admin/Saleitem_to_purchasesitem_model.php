...
class Saleitem_to_purchasesitem_model extends CI_Model
{
	...
	public function getAllrecipe(){		
		...
		$sql = "SELECT 'sno',w.name branch,recipe.id, recipe.name, recipe.code, GROUP_CONCAT( DISTINCT CONCAT(products.name,' - ',recipe_products.quantity,'(',units.code,')') ORDER BY recipe.id SEPARATOR '<br> ') AS product_details
		...
		LEFT JOIN ".$this->db->dbprefix('units')."  AS units ON units.id IN (recipe_products.unit_id)
