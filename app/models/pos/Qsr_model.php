...
class Qsr_model extends CI_Model
{
    ...
    public function getWHrecipe($recipe_id, $warehouse_id)
    {
        $this->db->select('recipe.*, warehouses_recipe.quantity, categories.id as category_id, categories.name as category_name')
            ...
            ->group_by('recipe.id');
        $q = $this->db->get_where("recipe", array('recipe.id' => $recipe_id));
	...
	public function addInsertbil($data, $recipes, $payment, $update_bill, $sales_bill, $multi_currency, $did,$order_data, $bill_recipes,$total,$customer_id,$loyalty_used_points,$taxation,$discountid){
	  ...
	  // $this->db->update('bils', array('bill_number' => $bill_number), array('id' => $bill_id));
		...
		'tax_type'         => $this->pos_settings->tax_type,
                ...
                'discount_val' => $discountid ? $discountid : 0,
...
/**/				
	    ...
	    //echo '<pre>';print_r($kit);exit;
	    //////////// kitchen data end //////
			 return array('sale_id' => $bill_id,'kitchen_data' => $kit, 'consolid_kitchen_data' => $consolid_kit);
                   //   print_r($this->db->error());die;
			 return TRUE;
		}
...
function recipe_customer_discount_calculation($itemid,$groupid,$subgroup_id,$finalAmt,$discountid){
    ...
    $query = "SELECT count(P.id) total_transaction,SUM(P.round_total) gross_total1,SUM(P.total-P.total_discount+CASE WHEN (P.tax_type= 1) THEN P.total_tax ELSE 0 END) AS gross_total,SUM(P.round_total-P.total_discount) net_total1,SUM(P.total-P.total_discount+CASE WHEN (P.tax_type= 1) THEN P.total_tax ELSE 0 END) AS net_total
        FROM " . $this->db->dbprefix('bils') . "  P
            WHERE DATE(P.date) BETWEEN '".$start."' AND '".$end."' AND
            P.payment_status ='Completed' ".$where." ";//echo $query;exit;
...
public function getRoundamount($start,$end)
    ...
    public function CancelSale($cancel_remarks, $sale_id, $user_id){
    ...
    public function getAllSalesWithbiller(){
		$current_date = date('Y-m-d');
		$current_date = $this->site->getTransactionDate();
		$this->db->select("sales.*");
		$this->db->where('sales.sales_type_id', 0);	
	...
	$this->db->where(array('sale_id'=>$sale_id,'bil_id'=>$bill_id));
